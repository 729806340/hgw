...
final class Base
{
    ...
    private static function parse_conf(&$setting_config)
    {
        ...
        $setting['shopwwi_version'] = '<span class="vol"><font class="b">Hango</font><font class="o">Wa</font></span>';
...
//        echo $act_file;
     ...
     *
     */
    private static function start_session($config="")
    {
        if (SUBDOMAIN_SUFFIX) {
        ...
        if ( C('session_type') == 'memcache' || $config['session_type'] == 'memcache' ) {
        	$host = C('memcache.1.host') ? C('memcache.1.host') : ( $config['memcache']['1']['host'] ? $config['memcache']['1']['host'] : '' ) ;
        	$port = C('memcache.1.port') ? C('memcache.1.port') : ( $config['memcache']['1']['port'] ? $config['memcache']['1']['port'] : '' ) ;
        ...
        session_start();
        /*echo session_id();
        $memcache = memcache_connect('172.16.40.250', 11211); 
		var_dump($memcache->get($_COOKIE['PHPSESSID']));*/
    }
    ...
    private static function cp()
    {
        ...
        if (strpos(self::CPURL, '||') !== false) {
            ...
            header('location: http://www.shopwwi.com');
            exit();
        } else {
    ...
    public static function load_config($file, $key = '', $default = '', $reload = false)
    {
        ...
        $path = BASE_DATA_PATH . '/config/' . $file . '.config.php';
        if (file_exists($path)) {
	...
	public static function sess_start($config)
    {
        //session start
