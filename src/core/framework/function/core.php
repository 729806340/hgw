...
function setNcCookie($name, $value, $expire = '3600', $path = '', $domain = '', $secure = false)
{
    ...
    //默认用全域域名，如要用二级域名，请在$domai参数中精确指定
    if (empty($domain)) {
        if (SUBDOMAIN_SUFFIX) {
            $subdomain_suffix = SUBDOMAIN_SUFFIX;
        } else {
            if (preg_match("/^[\w.]*[:0-9.]+$/", $_SERVER['HTTP_HOST'])) {
        ...
        $domain = str_replace('http://', '', $subdomain_suffix);
        $pos= strpos($domain, ':');
        if (false !== $pos) {
...
function getRegisterNchash($act = '', $op = '')
{
    $act = $act ? $act : $_GET['act'];
    $op = $op ? $op : $_GET['op'];
    return substr(md5(SHOP_SITE_URL . $act . $op), 0, 8);
...
function Model($model = null, $base_path = null)
{
    ...
    if (!file_exists($file_name)) {
        ...
        if (!class_exists($class_name)) {
            ...
            return $_cache[$cache_key] = new $class_name($model);
        }
    }
}
 ...
 * @return TModel
 */
function TModel($name = '', $tablePrefix = '', $connection = '', $db = 0)
{
    static $_model = array();
    if (strpos($name, ':')) {
    ...
    if (!isset($_model[$guid]))
        $_model[$guid] = new $class($name, $tablePrefix, $connection, $db);
    return $_model[$guid];
}

function ecModel($name = '')
{
    return TModel($name, 'sdb_', 'DB_CONFIG1', 999);
}
 ...
 * @param string $base_path 起始路径
 * @param string $ext 导入的文件扩展名
 * @return boolean
 */
function Service($class, $base_path = '', $ext = '.php')
{
    static $_cache = array();
    $cache_key = $class . '.' . $base_path;
    if (!is_null($class) && isset($_cache[$cache_key])) return $_cache[$cache_key];
    $base_path = $base_path == null ? BASE_DATA_PATH : $base_path;
    $file_name = $base_path . '/service/' . $class . $ext;
    $class_name = $class . 'Service';
    if (!file_exists($file_name)) {
        $error = 'Service Error:  File ' . $file_name . ' is not exists!';
        throw_exception($error);
    } else {
        require_once($file_name);
        if (!class_exists($class_name)) {
...
function showEditor($id, $value = '', $width = '700px', $height = '300px', $style = 'visibility:hidden;', $upload_state = "true", $media_open = false, $type = 'all')
{
    ...
    switch ($type) {
        ...
        default:
            ...
            'italic', 'underline', 'strikethrough', 'lineheight', 'removeformat', '|', 'image'" . $media . ", 'table', 'hr', 'emoticons', 'link', 'unlink', '|', 'about', '|', 'insertcontent']";
            break;
    }
    
    //专题关闭标签过滤模式
    $filterMode = ('special_content' == $id) ? 'false' : 'true';
...
<script>
  ...
  KindEditor.ready(function(K) {
        KE = K.create("textarea[name=\'' . $id . '\']", {
						...
						filterMode : ' . $filterMode . ',//用 filterMode 参数关闭过滤模式，保留所有标签;另外可用 htmlTags 参数定义要保留的标签和属性，具体使用请百度
						syncType:"form",
 ...
 * @param string $upload_state 上传状态，默认是开启
 */
function showAdminEditor($id, $value = '', $width = '700px', $height = '300px', $style = 'visibility:hidden;', $upload_state = "true", $media_open = false, $type = 'all')
{
    //是否开启多媒体
    $media = '';
    if ($media_open) {
    ...
    //专题关闭标签过滤模式
    $filterMode = ('special_content' == $id) ? 'false' : 'true';
    $admin_resource = str_replace('www.hangowa.com', 'admin2018.hangowa.com', RESOURCE_SITE_URL);
    //图片、Flash、视频、文件的本地上传都可开启。默认只有图片，要启用其它的需要修改resource\kindeditor\php下的upload_json.php的相关参数
    echo '<textarea id="' . $id . '" name="' . $id . '" style="width:' . $width . ';height:' . $height . ';' . $style . '">' . $value . '</textarea>';
    echo '
<script src="' . $admin_resource . '/kindeditor/kindeditor-min.js" charset="utf-8"></script>
<script src="' . $admin_resource . '/kindeditor/lang/zh_CN.js" charset="utf-8"></script>
<script>
	var KE;
  KindEditor.ready(function(K) {
        KE = K.create("textarea[name=\'' . $id . '\']", {
...
function showB2BEditor($id, $value = '', $width = '700px', $height = '300px', $style = 'visibility:hidden;', $upload_state = "true", $media_open = false, $type = 'all')
{
    //是否开启多媒体
    $media = '';
    if ($media_open) {
    ...
    echo '<textarea id="' . $id . '" name="' . $id . '" style="width:' . $width . ';height:' . $height . ';' . $style . '">' . $value . '</textarea>';
    echo '
<script src="/data/resource/kindeditor/kindeditor-min.js" charset="utf-8"></script>
<script src="/data/resource/kindeditor/lang/zh_CN.js" charset="utf-8"></script>
<script>
	var KE;
  KindEditor.ready(function(K) {
        KE = K.create("textarea[name=\'' . $id . '\']", {
 ...
 */
function ncPriceFormat($price, $num=2)
{
    $price_format = number_format($price, $num, '.', '');
    return $price_format;
}
...
function str_sub($string, $sublen, $start = 0, $code = 'UTF-8')
{
    if ($code == 'UTF-8') {
...
function getMemberAvatar($member_avatar)
{
    if (empty($member_avatar)) {
        ...
        }elseif (substr($member_avatar,0,7)=='http://' || substr($member_avatar,0,8)=='https://'){
 ...
 *
 * @param str $tplpath
 * @param str $tpl_quna module名 add by guiyj on 2016.7.27
 * @return string
 */
function template($tplpath, $tpl_quna = '')
{
    ...
    if (strpos($tplpath, ':') !== false) {
        ...
        if (empty($tpl_quna)) {
            $tpl_file = BASE_PATH . '/templates/' . TPL_NAME . '/' . $tplpath . '.php';
        } else {
            $tpl_file = BASE_PATH . '/modules/' . $tpl_quna . '/templates/' . TPL_NAME . '/' . $tplpath . '.php';
        }
...
//    $site_url = empty($site_url) ? SHOP_SITE_URL : $site_url;
    ...
    $reg_match_from = array(
        ...
        '/^special-show-special_id-(\d+)\.html$/',
        '/^category-index\.html$/',
...
function urlB2b($act = '', $op = '', $args = array(), $store_domain = '')
{
...
//        'search:index',
    ...
    return url($act, $op, $args, $rewrite_flag, B2B_SITE_URL);
}
...
/**
 * b2b后台shop使用的URL链接函数，强制使用动态传参数模式
 *
 * @param string $act control文件名
 * @param string $op op方法名
 * @param array $args URL其它参数
 * @return string
 */
function urlAdminB2b($act = '', $op = '', $args = array())
{
    return url($act, $op, $args, false, B2B_modules_URL);
}
 ...
 * @return string
 */
function passwordHash($source, $salt = null, $account = null, $createTime = null)
{
    if (!$account || !$createTime) {
        return empty($salt) ? md5($source) : md5(md5($source) . $salt);
    }

    if ($salt == '' && $salt == null) return extendsMd5($source, $account, $createTime);
    ...
    if ($salt == 0) {
        ...
        return md5($source_md5 . $salt);
    }
}

function extendsMd5($source, $username, $createTime)
{
    if (32 != strlen($source)) {
        $source_md5 = md5($source);
    } else {
    ...
    $string_md5 = md5($source_md5 . $username . $createTime);
    $front_string = substr($string_md5, 0, 31);
    $end_string = 's' . $front_string;
    return $end_string;
}

function copyTo($from, $map)
{
    $res = array();
    foreach ($map as $k => $v) {
