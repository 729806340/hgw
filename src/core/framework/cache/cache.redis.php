...
class CacheRedis extends Cache {
    ...
    private function init_master(){
    	...
    	if (isset($_cache)){
            ...
            !empty($this->config['master']['auth']) && $this->handler->auth($this->config['master']['auth']);
            //切换到指定的数据库
            $db_index = empty($this->config['master']['db_index']) ? 0 : $this->config['master']['db_index'];
    ...
    private function init_slave(){
    	...
    	if (isset($_cache)){
	        ...
	        !empty($this->config['slave']['auth']) && $this->handler->auth($this->config['slave']['auth']);
            //切换到指定的数据库
            $db_index = empty($this->config['slave']['db_index']) ? 0 : $this->config['slave']['db_index'];
            $this->handler->select($db_index);
