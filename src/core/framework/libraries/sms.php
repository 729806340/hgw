...
class Sms {
    ...
    public function send($mobile,$content) {
       $shopwwi_sms_type=C('shopwwi_sms_type');
		...
		/*Log::selflog(json_encode($_GET));*/
       return $this->mysend_haiyan($mobile,$content);
    }

    //玄武短信
    public function mysend_xuanwu($mobile,$content)
    {

        $url = "http://116.31.71.146:9051/api/v1.0.0/message/mass/send";
        ...
        import('Curl');
        $Curl = new Curl();
        $authorization = base64_encode($account . ":" . MD5($password));
        ...
        $Curl->setHeader('Content-Length', $content_length);
    ...
    //海岩短信
    private function mysend_haiyan($mobile,$content)
    {
    	$url = 'http://www.duanxin10086.com/sms.aspx?action=send';
    	
    	$account = 'hangowa';
    	$password = 'hango2015WA';
    	$userid = 12072;
    	...
    	$post_data['content'] = $this->stripSmsCoentent($content); // 短信内容需要用urlencode编码下
    	$post_data['mobile'] = $mobile;
    	$post_data['sendtime'] = ''; // 不定时发送，值为0，定时发送，输入格式YYYYMMDDHHmmss的日期值
    	
    	define('SCRIPT_ROOT',  BASE_DATA_PATH.'/api/taobao/');
    	require_once SCRIPT_ROOT.'TopClient.php';
    	$client = new TopClient() ;
    	
    	$result = $client->curl($url, $post_data);
    	return true;
    }
    
    private function stripSmsCoentent($content) {
    	$search = array(
    			'私募',
    			'优惠'
    	);
