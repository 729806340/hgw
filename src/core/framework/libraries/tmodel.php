...
class TModel {
     ...
     */
    public function __construct($name='',$tablePrefix='',$connection='',$db=0) {
        // 模型初始化
        $this->_initialize();
        // 获取模型名称
        if(!empty($name)) {
        ...
        // 获取数据库操作对象
        // 当前模型有独立的数据库连接信息
        $this->db($db,empty($this->connection)?$connection:$this->connection);
    ...
    public function db($linkNum='',$config='',$params=array()){
        ...
        if(!isset($_db[$linkNum]) || (isset($_db[$linkNum]) && $config && $_linkNum[$linkNum]!=$config) ) {
            ...
            $_db[$linkNum]            =    TDb::getInstance($config);
        }elseif(NULL === $config){
