...
//引用配置文件
//用户唯一key
define('WEBSCAN_U_KEY', '90814cc2c703bf2cf7381dea8048374c');
//数据回调统计地址
 ...
 */
function webscan_getRealIp()
{
    if ($_SERVER["HTTP_FW_ADDR"])
        $ip = trim($_SERVER["HTTP_FW_ADDR"]);
    else if (getenv("HTTP_CLIENT_IP") && strcasecmp(getenv("HTTP_CLIENT_IP"), "unknown") && webscan_checkIP(getenv("HTTP_CLIENT_IP")))
        $ip = trim(getenv("HTTP_CLIENT_IP"));
    else if (getenv("REMOTE_ADDR") && strcasecmp(getenv("REMOTE_ADDR"), "unknown") && webscan_checkIP(getenv("REMOTE_ADDR")))
        $ip = trim(getenv("REMOTE_ADDR"));
    else if (isset ($_SERVER ['REMOTE_ADDR']) && $_SERVER ['REMOTE_ADDR'] && strcasecmp($_SERVER ['REMOTE_ADDR'], "unknown") && webscan_checkIP($_SERVER ["REMOTE_ADDR"]))
 ...
 * @return bool ，是否为内部ip
 */
function webscan_checkIP($ip)
{
    if (preg_match("/^(192\.168\.[5|6]|127\.0\.0)\./", $ip) && getenv("HTTP_X_FORWARDED_FOR")) {
        return false;
    } else
        return true;
}
...
function webscan_pape()
{
  ...
  <center>请求信息包含非法字符</center>
  </body>
  </html>
HTML;
    header('HTTP/1.1 403 Forbidden');
    header("Cache-Control: no-cache");
    header("Expires: " . gmdate("r", 0) . " GMT");
...
function webscan_StopAttack($StrFiltKey, $StrFiltValue, $ArrFiltReq, $method)
{
    ...
    if (preg_match("/" . $ArrFiltReq . "/is", $StrFiltValue) == 1) {
        webscan_slog(array('ip' => webscan_getRealIp(), 'time' => strftime("%Y-%m-%d %H:%M:%S"), 'page' => $_SERVER["PHP_SELF"], 'method' => $method, 'rkey' => $StrFiltKey, 'rdata' => $StrFiltValue, 'user_agent' => $_SERVER['HTTP_USER_AGENT'], 'request_url' => $_SERVER["REQUEST_URI"]));
        exit(webscan_pape());
    }
    if (preg_match("/" . $ArrFiltReq . "/is", $StrFiltKey) == 1) {
        webscan_slog(array('ip' => webscan_getRealIp(), 'time' => strftime("%Y-%m-%d %H:%M:%S"), 'page' => $_SERVER["PHP_SELF"], 'method' => $method, 'rkey' => $StrFiltKey, 'rdata' => $StrFiltKey, 'user_agent' => $_SERVER['HTTP_USER_AGENT'], 'request_url' => $_SERVER["REQUEST_URI"]));
        exit(webscan_pape());
    }
...
} elseif ($webscan_action == "_ckinstall") {
    //验证安装与版本信息
    if (!function_exists('curl_init')) {
