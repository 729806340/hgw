...
<?php 
if( $output['order_info']['shipping_code'] ) {
	$shipping_code = explode(",", $output['order_info']['shipping_code']);
}
?>
    ...
    <div class="ncm-order-condition">
      ...
      <ul>
        <li>1. 商品已发出；
          ...
          物流公司：<?php echo $output['order_info']['express_info']['e_name']?>；
          单号：
         <?php 
         if( is_array($shipping_code) && count($shipping_code)>1 ){
         	$string = "";
         	foreach ($shipping_code as $k => $code) {
         		if( $k>0 )$string .= ",";
         		$string .= "包裹".($k+1)."：".$code;
         	}
         	echo $string ;
    ...
    <table class="ncm-default-table order">
      <thead>
        <tr>
          ...
          <th class="w200" style="border-right: solid 1px #E7E7E7;">交易状态</th>
          <th class="w120" style="display: none;" >交易操作</th>
        </tr>
      </thead>
      <tbody>
        <?php if (!empty($output['order_info']['shipping_code'])) { ?>
        <tr>
          <th colspan="7" style="border-right: none;"> 
          ...
          <span> 
          <?php echo $lang['member_show_order_shipping_no'].$lang['nc_colon'];?> <?php echo count($shipping_code)>1?$shipping_code[0]:$output['order_info']['shipping_code'];?> 
          <?php if(count($shipping_code)>1){?>
          		<span><a href="javascript:void(0);" id="shipping_code_more">更多<i class="icon-angle-down"></i>
              <div class="more"><span class="arrow"></span>
                <ul id="shipping_code_ul">
                	<?php foreach ($shipping_code as $k => $code) {?>
                  <li>包裹<?php echo $k+1;?>：<?php echo $code;?></li>
                  <?php }?>
                </ul>
              </div>
          		<?php }?>
          ...
          <th colspan="3" style=" border-left: none;" class="bdr"><?php if(!empty($output['daddress_info'])) { ?>
        ...
        <tr class="bd-line">
          ...
          <td class="tl"><dl class="goods-name">
              <dt><a target="_blank" href="<?php echo $goods['goods_url']; ?>"><?php echo $goods['goods_name']; ?></a><span class="rec"><a style="display: none;" target="_blank" href="<?php echo urlShop('snapshot', 'index', array('rec_id' => $goods['rec_id']));?>">[交易快照]</a></span></dt>
          ...
          <!-- S 合并TD -->
          <?php if ((true||$output['order_info']['goods_count'] > 1 && $k ==0) || ($output['order_info']['goods_count'] == 1)){ ?>
          <td class="bdl bdr"><?php echo $output['order_info']['state_desc']; ?></td>
          <td class="bdr" style="display: none;"><?php if ($output['order_info']['if_lock']) { ?>
            <p>退款退货中</p>
            <?php } ?>
            ...
            <?php if ( false&&$output['order_info']['if_evaluation']) { ?>
            ...
            <?php if (false &&$output['order_info']['if_share']) { ?>
        ...
        <!-- E 赠品列表 -->
            <?php
            foreach ($output['order_info']['address_list'][$goods['rec_id']] as $address) { ?>
                <tr>
                    <td colspan="1"></td>
                    <td colspan="3"><?php echo $address['buyer_name']; ?>（<?php echo $address['buyer_phone']; ?>） <?php echo $address['address']; ?></td>
                    <td><?php echo $address['rec_price']; ?></td>
                    <td>x<?php echo $address['rec_num']; ?></td>
                    <td class="bdl bdr" colspan="1"><?php if($address['status']){echo '已发货 ('.$address['logi_name'].':'.$address['logi_sn'].')';}else{ echo '待发货';} ?></td>
                </tr>
            <?php } ?>
        <?php } ?>
      </tbody>
