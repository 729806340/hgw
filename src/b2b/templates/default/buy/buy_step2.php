...
<div class="ncc-main">
    ...
    <?php } else { ?>
        <div class="ncc-receipt-info mb30">
            <div class="ncc-finish-a"><i></i>订单提交成功！</div>
            <div class="ncc-finish-b">可通过用户中心<a href="<?php echo SHOP_SITE_URL ?>/index.php?act=member_order">已买到的商品</a>查看订单状态。
            </div>
            <div class="ncc-finish-c mb30"><a href="<?php echo B2B_SITE_URL ?>" class="ncbtn-mini ncbtn-mint mr15"><i
                            class="icon-shopping-cart"></i>继续购物</a><a
                        href="<?php echo SHOP_SITE_URL ?>/index.php?act=member_order" class="ncbtn-mini ncbtn-aqua"><i
                            class="icon-file-text-alt"></i>查看订单</a></div>
        </div>


    <?php } ?>
    <form action="index.php?act=payment&op=order" method="POST" id="buy_form">
        ...
        <div class="ncc-receipt-info">
            ...
            <dl class="ncc-pd-pay">
                ...
                <?php } ?>
                <?php if (false&&$output['pay']['if_show_pdrcb_select']) { ?>
                    <dt>使用余额支付</dt>
                    <dd>
            ...
            <div class="ncc-receipt-info">
                ...
                <ul class="ncc-payment-list">
                    <?php foreach ($output['payment_list'] as $val) {
                        if($val['payment_code'] != 'alipay') continue;
                        ?>
...
<script type="text/javascript">
    ...
    var invoice = <?php echo $output['invoice'] ? 'true' : 'false'; ?>;
    $(function () {
        ...
        $('input[name="jicai"]').on('click', function () {
            if ($(this).attr('checked')) {
                $('input[name="pd_pay"]').attr('checked', false);
                $('input[name="rcb_pay"]').attr('checked', false);
                $('.ncc-payment-list > li').removeClass('using');
                $('#payment_code').val('');
            }
        });
        ...
        $('#next_button').on('click', function () {
            ...
            if ($('#payment_code').val() == '' && parseFloat($('#api_pay_amount').html()) > 0 && !$('input[name="jicai"]').attr('checked')) {
