...
<div class="ncc-main">
  <div class="ncc-title">
    ...
    <h5>查看采购商品清单，增加减少商品数量，并勾选想要的商品进入下一步操作。</h5>
  </div>
  <form action="<?php echo urlB2b('buy','buy_step1');?>" method="POST" id="form_buy" name="form_buy">
    ...
    <table class="ncc-table-style" nc_type="table_cart">
      ...
      <tbody>
        ...
        <!-- S one store list -->
        <?php
        foreach($output['cart_list'] as $cart_info) {
            ?>
...
<?php } ?> /></td>
          ...
          <td class="w100"><a href="<?php echo urlB2b('goods','index',array('goods_commonid'=>$cart_info['goods_commonid']));?>" target="_blank" class="ncc-goods-thumb"><img src="<?php echo thumb($cart_info);?>" alt="<?php echo $cart_info['goods_name']; ?>" /></a></td>
          <?php } ?>
          <td class="tl" <?php if ($cart_info['bl_id'] != '0') {?>colspan="2"<?php }?>>
              <dl class="ncc-goods-info">
              <dt>
                <a href="<?php echo urlB2b('goods','index',array('goods_commonid'=>$cart_info['goods_commonid']));?>" target="_blank" title="<?php echo $cart_info['goods_name']; ?>"><?php echo $cart_info['goods_name'].'('.$cart_info['goods_calculate'].')'; ?></a></dt>
          ...
          <td class="ws0"><a href="JavaScript:void(0);" onclick="decrease_quantity(<?php echo $cart_info['cart_id']; ?>);" title="<?php echo $lang['cart_index_reduse'];?>" class="add-substract-key tip">-</a>
            ...
            <a href="JavaScript:void(0);" onclick="add_quantity(<?php echo $cart_info['cart_id']; ?>);" title="<?php echo $lang['cart_index_increase'];?>" class="add-substract-key tip" >+</a>
          </td>
          <?php } else {?>
          <td>无效
