...
class Control{
    ...
    protected function showLayout() {
        ...
        Tpl::output('goods_category',$goods_category);
...
class BaseHomeControl extends Control {
    ...
    private function _is_purchaser(){
        //验证自己是否是采购商
        $res = Model('b2b_purchaser')->isPurchaser();
        if(!empty($res)){
...
class BaseBuyControl extends Control {
    ...
    protected function __construct(){
        ...
        if(!C('site_status')) halt(C('closed_reason'));
        Model('b2b_purchaser')->purchaserFlag();
        //获取导航
        Tpl::output('nav_list', rkcache('nav',true));
...
class BaseMemberControl extends Control {
    ...
    private function _getMenuList() {
        $menu_list = array(
            'purchase_center' => array('name' => '采购中心', 'child' => array(
                'member_order'    => array('name' => '采购订单', 'url'=>urlB2b('member_order', 'index')),
                'member_purchase'    => array('name' => '收货地址', 'url'=>urlB2b('member_purchase', 'address')),
                'member_invoice'    => array('name' => '发票管理', 'url'=>urlB2b('member_invoice', 'address')),
            ))
        );
