...
class paymentControl extends BaseHomeControl
{
    ...
    public function orderOp()
    {
        ...
        //取订单列表
        /** @var b2b_paymentLogic $logic_payment */
        $logic_payment = Logic('b2b_payment');
        ...
        if (empty($api_pay_amount)) {
            redirect(B2B_SITE_URL . '/index.php?act=buy&op=pay_ok&pay_sn=' . $order_pay_info['data']['pay_sn'] . '&pay_amount=' . ncPriceFormat($pay_amount));
        }

        if ($order_list[0]['payment_code'] == 'jicai') { //集采订单再次选取第三方支付，更新payment_code和order_from
            $result = Model('b2b_order')->editOrder(array('api_pay_time' => TIMESTAMP, 'payment_code' => $payment_code, 'order_from' => '1'), array('order_id' => array('in', $pay_order_id_list)));
        } else {
            $result = Model('b2b_order')->editOrder(array('api_pay_time' => TIMESTAMP), array('order_id' => array('in', $pay_order_id_list)));
        }
        ...
        if ($order_pay_info['data']['if_buyer_repay']) {
            $update = Model('b2b_order')->editOrderPay(array('api_pay_state' => 0), array('pay_id' => $order_pay_info['data']['pay_id']));
            if (!$update) {
    ...
    public function vr_orderOp()
    {
        ...
        //计算所需支付金额等支付单信息
        $result = Logic('b2b_payment')->getVrOrderInfo($order_sn, $_SESSION['member_id']);
        if (!$result['state']) {
            showMessage($result['msg'], $url, 'html', 'error');
        }
        ...
        $result = Logic('b2b_payment')->getPaymentInfo($payment_code);
        if (!$result['state']) {
            showMessage($result['msg'], $url, 'html', 'error');
        }
        $payment_info = $result['data'];
    ...
    public function returnOp()
    {
        ...
        $url = B2B_SITE_URL . '/index.php?act=' . $act;
        ...
        //支付成功后跳转
        if ($order_type == 'real_order') {
            $pay_ok_url = B2B_SITE_URL . '/index.php?act=buy&op=pay_ok&pay_sn=' . $out_trade_no . '&pay_amount=' . ncPriceFormat($api_pay_amount);
        }
    ...
    {
        $result = Logic('b2b_payment')->getPaymentInfo('wxpay');
        if (!$result['state']) {
    ...
    public function wxpay_notifyOp()
    {
        ...
        $paymentLogic = Logic('b2b_payment');
    ...
    public function yeepay_callbackOp()
    {
        ...
        if ($order_type == 'real_order') {
            ...
            $pay_ok_url = B2B_SITE_URL . '/index.php?act=buy_virtual&op=pay_ok&order_sn=' . $out_trade_no . '&order_id=' . $order_pay_info['order_id'] . '&order_amount=' . ncPriceFormat($api_pay_amount);
        } elseif ($order_type == 'pd_order') {
    ...
    public function query_stateOp()
    {
        if ($_GET['pay_id'] && intval($_GET['pay_id']) > 0) {
            $info = Model('b2b_order')->getOrderPayInfo(array('pay_id' => intval($_GET['pay_id']), 'buyer_id' => intval($_GET['buyer_id'])));
