...
<?php } ?> /></td>
          ...
          <td class="tl" <?php if ($cart_info['bl_id'] != '0') {?>colspan="2"<?php }?>><dl class="ncc-goods-info">
              <dt>
                ...
                <strong>【优惠套装】</strong>
                <?php }?>
                <a href="<?php echo urlShop('goods','index',array('goods_id'=>$cart_info['goods_id']));?>" target="_blank" title="<?php echo $cart_info['goods_name']; ?>"><?php echo $cart_info['goods_name']; ?></a></dt>
          ...
          <td><!-- S 商品单价 -->
            ...
            <?php } ?>
            <em id="item<?php echo $cart_info['cart_id']; ?>_price" class="goods-price">
                <?php
                if ($cart_info['ifxianshi'] && $cart_info['xianshi_price'] > 0 && $cart_info['goods_num'] <= $cart_info['xianshi_num']) {
                    echo $cart_info['xianshi_price'];
                } else {
                    echo $cart_info['goods_price'];
                }
                ?>
            </em>
            <!-- E 商品单价 --> 
