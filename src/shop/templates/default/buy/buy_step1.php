...
function iniRpt(order_total) {
	...
	for (i = 0; i < rpt_list_json.length; i++) {
	    ...
	    if(rpt_list_json[i]['rpacket_range']>0&&rpt_list_json[i]['rpacket_skus']!=''&&rpt_list_json[i]['rpacket_skus']!=null){
            orderAmount = sumSku(rpt_list_json[i]['rpacket_skus'],rpt_list_json[i]['rpacket_range']==2);
        }
		_tmp = parseFloat(rpt_list_json[i]['rpacket_limit']);
        orderAmount = parseFloat(orderAmount);
    	if (orderAmount > 0 && parseFloat(orderAmount.toFixed(2)) >= parseFloat(_tmp.toFixed(2))) {
...
function sumSku(sku,exclude) {
    ...
    $('em[nc_type="eachStoreTotal"]').each(function() {
        ...
        $('em[nc_type="eachGoodsTotal' + store_id + '"]').each(function () {
            ...
            if(($.inArray(goodsId,sku)==-1) === exclude)
            storeSum += parseFloat($(this).html());
        });
        sum += storeSum;
    });
    return sum;
...
<div class="ncc-main">
  <div class="ncc-title">
    ...
    <?php } else if($output['current_goods_info']['is_pintuan']){
        echo '拼团';?>
        <?php include template('buy/buy_goods_pintuan');?>
    <?php } else { ?>
