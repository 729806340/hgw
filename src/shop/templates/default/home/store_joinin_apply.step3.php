      ...
      <tbody>
        ...
        <tr>
          ...
          <td><select name="sg_id" id="sg_id">
              ...
              <option value="<?php echo $v['sg_id'];?>" <?php echo $output['joinin_detail']['sg_id']==$v['sg_id']?"selected=''":'';?> >
              <?php echo $v['sg_name'];?> (<?php echo $explain;?>)
              </option>
              <?php } ?>
              <?php } ?>
            </select>
        ...
        <tr>
          ...
          <td><select name="joinin_year" id="joinin_year">
              <option value="1" <?php echo $output['joinin_detail']['joinin_year']=='1'?"selected=''":'';?>>1 年</option>
              <option value="2" <?php echo $output['joinin_detail']['joinin_year']=='2'?"selected=''":'';?>>2 年</option>
            </select></td>
        </tr>
        <tr>
          <th><i>*</i>店铺分类：</th>
          <td><select name="sc_id" id="sc_id">
              ...
              <option value="<?php echo $v['sc_id'];?>" <?php echo $output['joinin_detail']['sc_id']==$v['sc_id']?"selected=''":'';?> >
              <?php echo $v['sc_name'];?> (保证金：<?php echo $v['sc_bail'];?> 元)
              </option>
              <?php } ?>
              <?php } ?>
            </select>
        ...
        <tr>
          ...
          <td><a href="###" id="btn_select_category" class="btn">+选择添加类目</a>
            ...
            <input id="store_class" name="store_class" type="hidden" value='<?php echo $output['joinin_detail']['sc_id'];?>' />
            <span></span></td>
        </tr>
        <tr>
          <td colspan="2"><table border="0" cellpadding="0" cellspacing="0" id="table_category" class="type">
              ...
              <?php 
        	if( $output['joinin_detail']['store_class_ids'] && $output['joinin_detail']['store_class_names'] ) {
        		
        		$store_class_ids = unserialize($output['joinin_detail']['store_class_ids']) ;
        		$store_class_names = unserialize($output['joinin_detail']['store_class_names']) ;
        		$store_class_commis_rates = explode(",", $output['joinin_detail']['store_class_commis_rates']) ;
        		foreach ($store_class_ids as $k => $sc_id) {
        			list($c1,$c2,$c3) = explode(",", $store_class_names[$k]);
        ?>
        			<tr class="store-class-item">
        			<td><?php echo $c1;?></td>
        			<td><?php echo $c2;?></td>
        			<td><?php echo $c3;?> (分佣比例：<?php echo $store_class_commis_rates[$k];?>%)</td>
        			<td><a nctype="btn_drop_category" href="javascript:;">删除</a></td>
        			<input name="store_class_ids[]" value="<?php echo $store_class_ids[$k];?>," type="hidden">
        			<input name="store_class_names[]" value="<?php echo $store_class_names[$k];?>," type="hidden">
        			</tr>
        <?php 
        		}
        	}
