...
<div class="eject_con">
  ...
  <form method="post" action="index.php?act=store_deliver_set&op=daddress_add" id="address_form" target="_parent">
    ...
    <dl>
      ...
      <dl>
          <dt><i class="required">*</i><?php echo $lang['store_daddress_is_receipt'].$lang['nc_colon'];?></dt>
          <dd>
              <div>
                  <label class="mr20">
                      <input name="is_receipt" class="vm" type="radio" <?php if ($output['address_info']['is_receipt'] == 1) { echo 'checked';};?> value="1">
                      同意</label>
                  <label>
                      <input name="is_receipt" class="vm" type="radio" <?php if ($output['address_info']['is_receipt'] == 0) { echo 'checked';};?> value="0">
                      放弃</label>
                  <p class="hint">如果选择同意，买家退货时，可以使用此地址。</p>
              </div>
          </dd>
      </dl>
      <dl>
