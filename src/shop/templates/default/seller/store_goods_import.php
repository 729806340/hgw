...
<!-- 		<h6>STIP.1</h6> -->
		<h2>导入CSV4.0</h2> <!-- <i class="arrow icon-angle-right"> --></i></li>
	<!--<li class="<?php $output['step']=="4" ? print "current" : print "";?>"><i
		class="icon icon-camera-retro "></i>
		<h6>STIP.2</h6>
		<h2>上传图片</h2></li>
	<li class="<?php $output['step']=="5" ? print "current" : print "";?>"><i
...
<form method="post" action="index.php?act=taobao_import&op=index"
	...
	<div class="ncsc-form-goods" <?php if($output['step'] != '1'){?>
		...
		<dl>
			<dt>
				<i class="required">*</i><?php echo $lang['store_goods_album_goods_pic'].$lang['nc_colon'];?></dt>
			<dd>
				<div class="handle">
		...
		<dl>
			...
			<dd>
				<span class="new_add"><a href="javascript:void(0)"
					id="add_sgcategory" class="ncsc-btn"><?php echo $lang['store_goods_index_new_class'];?></a>
				</span>
        ...
        <select name="sgcate_id[]" class="sgcategory">
          ...
          <option value="<?php echo $child_val['stc_id']; ?>"
						...
						<?php } ?>>&nbsp;&nbsp;&nbsp;&nbsp;<?php echo $child_val['stc_name']; ?></option>
          <?php }?>
          <?php }?>
          <?php } ?>
        </select>
...
?>
<iframe
	<?php if($output['step']=="4"){echo 'style="display:block"';}else{echo 'style="display:none"';} ?>
	id="uploadwjc" src="<?php echo $src; ?>" height="800" width='100%'
