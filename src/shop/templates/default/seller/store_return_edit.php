        ...
        <form id="post_form" method="post" action="index.php?act=store_return&op=edit&return_id=<?php echo $output['return']['refund_id']; ?>">
          ...
          <dl>
            ...
            <dd>
              <div>
                <label class="mr20">
                  <input type="radio" class="vm" id="ok_seller_message" name="seller_state" value="2" />
                  同意</label>
                <label>
          ...
          <dl>
            ...
            <dd id="seller_message">
              <textarea name="seller_message" rows="2" class="textarea w300"></textarea>
              <span class="error"></span>
                <div>
                <select onchange="putvalue(this.value);"><option value="">-请选择-</option>
                    <?php foreach ($output['receipt_list'] as $list){?>
                    <option value="<?php echo $list['seller_name'] . ',' . $list['telphone'] . ',' . $list['area_info'].$list['address']?>"><?php echo $list['seller_name'] . ',' . $list['telphone'] . ',' . $list['area_info'].$list['address']?></option>
                    <?php }?>
                </select>
                </div>
...
<script type="text/javascript">
    function putvalue(value) {
        $('#seller_message').children('.textarea').text(value);
    }
$(function(){
