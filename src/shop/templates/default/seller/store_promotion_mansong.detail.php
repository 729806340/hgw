    ...
    <tr class="bd-line"><td></td>
      ...
      <td><ul class="ncsc-mansong-rule-list">
          ...
          <li>
              ...
              <?php }
              if($val['rule_repeat']) echo '可累计';
              else echo '不可累计';
              echo '<br />';
              if($val['rule_range'] ==0) echo '全场通用';
              elseif ($val['rule_range']==1) echo '仅选定商品适用';
              else  echo '除选定商品外适用';
              if ($val['rule_range']>0){
                foreach ($val['sku'] as $sku){
                  $sku['goods_url'] = urlShop('goods', 'index', array('goods_id' => $sku['goods_id']));
                  //v($sku);
                  ?>
                  <a href="<?php echo $sku['goods_url'];?>" title="<?php echo $sku['goods_name'];?>" target="_blank" class="goods-thumb"> <img src="<?php echo cthumb($sku['goods_image']);?>"/> </a>
                <?php }
              } ?>
          </li>
          <?php }?>
