...
<style>
    .explanation li {
        background: url(../admin/templates/default/images/macro_arrow.gif) no-repeat 0 10px;
        padding-left: 10px;
        margin-bottom: 4px;
        line-height:25px;
        margin-left:10px;
    ...
    .explanation .title span {
        background: url(../admin/templates/default/images/combine_img.png) no-repeat -580px -200px;
        width: 20px;
        height: 20px;
        position: absolute;
        z-index: 1;
...
<div class="explanation" id="explanation" style="color: rgb(44, 188, 163); background-color: rgb(237, 251, 248); width: 99%; height: 100%;color: #2CBCA3;padding: 6px 9px;border-radius: 5px; position: relative;overflow: hidden;">
    <div class="title" id="checkZoom"><i class="fa fa-lightbulb-o"></i>
        <h4 title="提示相关设置操作时应注意的要点" style="font-size:15px;margin-bottom:5px;">&hearts; 温馨提示：</h4>
        <span id="explanationZoom" title="收起提示" style="display: block;"></span> </div>
    <ul>
        <li>48小时内未处理系统将自动同意退款处理.</li>
        <li>请在规定时间处理完所有售后订单.</li>
    </ul>
</div>
<div class="tabmenu">
  ...
  <table class="search-form">
    <tr>
      ...
      <td class="w70 tc"><label class="submit-border"><input type="submit" class="submit" value="<?php echo $lang['nc_search'];?>" /></label></td>
        <td class="w70 tc"><label class="submit-border"><input type="button" class="export" value="导出" /></label></td>
...
<table class="ncsc-default-table">
  ...
  <tbody>
    ...
    <tr class="bd-line" >
      ...
      <td><?php
          $seller_state = $val['seller_state']==2&&$val['agree_role']==1?4:$val['seller_state'];
          echo $output['state_array'][$seller_state];
          ?></td>
      <td><?php echo ($val['seller_state'] == 2 && $val['refund_state'] >= 2) ? $output['admin_array'][$val['refund_state']]:'无'; ?></td>
      <td class="nscs-table-handle">
        <?php if ($val['is_operate'] == 1) { ?>
...
<script>
	$(function(){
        ...
        $('.export').click(function(){
            ...
            if(add_time_from==""&&add_time_to!==""&&key==""){
                alert("请填写筛选结束时间");
                return false;
            }
            if(add_time_from!==""&&add_time_to==""&&key==""){
            ...
            location.href="index.php?act=store_return&op=exportCvs&add_time_from="+add_time_from+"&add_time_to="+add_time_to+"&state="+state+"&type="+type+"&key="+key+"&lock="+"<?php echo $_GET['lock']; ?>";
        })
