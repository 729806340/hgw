...
<div class="ncsc-oredr-show">
  <div class="ncsc-order-info">
    <div class="ncsc-order-details">
      <div class="title"><?php echo $lang['store_show_order_info'];?> <span style="float: right;"><a href="javascript:;" onclick="ajax_form('bill_remark', '添加账单备注', 'index.php?act=store_order&op=bill_remark&order_id=<?php echo $_GET['order_id']; ?>', 640,0);" class="blue" >添加账单备注</a></span></div>
      <div class="content">
        <dl>
        ...
        <dl>
          ...
          <dd><?php echo $output['order_info']['extend_order_common']['order_message']; ?><?php echo  !empty($output['order_info']['extend_order_common']['distribution_channel']) ? "【".$output['order_info']['extend_order_common']['distribution_channel']."】":'' ?></dd>
        </dl>
        <dl class="line">
            <?php
            $hango = in_array($_SESSION['store_id'],array(10,15,48,138,171,174,213));
            if ($output['order_info']['order_from']==3 &&$hango) { ?>
                <dt>渠道单号：</dt>
                <dd><?php echo$output['order_info']['fx_order_id'] . '【'. orderFrom($output['order_info']['order_from'],$output['order_info']['buyer_name']) .'】'; ?> </dd>
                <!--<dd><?php /*echo $output['order_info']['fx_order_id'] . '【'.str_replace( array(1,2,3,4,5), array('PC端','移动端','分销','集采','b2b'), $output['order_info']['order_from'] ) .'】'; */?> </dd>-->
            <?php } ?>
        ...
        <dl>
          <dt>发货备注：<?php echo $output['order_info']['extend_order_common']['deliver_explain']; ?></dt>
          <dd></dd>
        </dl>
      </div>
    </div>
    ...
    <div class="ncsc-order-condition">
      ...
      <ul>
        <li>1. 商品已发出；
          ...
          物流公司：<span class='e_name'><?php echo $output['order_info']['express_info']['e_name']?></span>；
          单号：<span class='s_code'>
          <?php 
          $shipping_code = explode(",", $output['order_info']['shipping_code']);
          if(count($shipping_code)>1){
          		$string = "";
          ...
          查看 <a href="#order-step" class="blue">“物流跟踪”</a> 情况。填错了快递单号也可以<a href="javascript:;" onclick="ajax_form('edit_deliver', '修改快递', 'index.php?act=store_order&op=edit_deliver&order_id=<?php echo $_GET['order_id']; ?>', 640,0);" class="blue" >“修改物流”</a>。
          <?php } ?>
          <?php } else { ?>
          无需要物流。
          <?php } ?>
      ...
      <tbody>
        ...
        <tr>
          <th colspan="6" style="border-right: 0;"> 
          		...
          		<span>物流公司：<em class="e_name">
          		<a target="_blank" href="<?php echo $output['order_info']['express_info']['e_url'];?>"><?php echo $output['order_info']['express_info']['e_name'];?></a></em>
          		</span> 
          		...
          		<span><?php echo $lang['store_order_shipping_no'].$lang['nc_colon'];?> <em class="s_code"><?php echo $shipping_code[0]; ?></em>
