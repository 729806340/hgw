...
<script type="text/javascript">
    $(document).ready(function(){
        ...
        $('#div_goods_search_result').on('click', '#btn_submit', function() {
            ...
            var xianshi_storage = Number($('#dialog_xianshi_storage').val());
            var xianshi_limit = Number($('#dialog_xianshi_limit').val());
            var first_order = Number($('#dialog_first_order:checked').val())
            if(!isNaN(xianshi_price) && xianshi_price > 0 && xianshi_price < goods_price) {
                $.post('<?php echo urlShop('store_promotion_xianshi', 'xianshi_goods_add');?>', 
                    {goods_id: goods_id, xianshi_id: xianshi_id, xianshi_price: xianshi_price, xianshi_storage: xianshi_storage, xianshi_limit: xianshi_limit, first_order: first_order},
                    function(data) {
                        if(data.result) {
        ...
        $('#xianshi_goods_list').on('click', '[nctype="btn_edit_xianshi_goods"]', function() {
            ...
            var xianshi_storage = $edit_item.find('[nctype="xianshi_storage"]').text();
            var xianshi_limit = $edit_item.find('[nctype="xianshi_limit"]').text();
            var first_order = $edit_item.find('[nctype="first_order"]').text() == '是';
            ...
            $('#dialog_edit_xianshi_storage').val(xianshi_storage);
            $('#dialog_edit_xianshi_limit').val(xianshi_limit);
            if(first_order) $('#dialog_edit_first_order').prop('checked',true);
        ...
        $('#btn_edit_xianshi_goods_submit').on('click', function() {
            ...
            var xianshi_storage = Number($('#dialog_edit_xianshi_storage').val());
            var xianshi_limit = Number($('#dialog_edit_xianshi_limit').val());
            var first_order = Number($('#dialog_edit_first_order:checked').val())
            if(!isNaN(xianshi_price) && xianshi_price > 0 && xianshi_price < goods_price) {
                $.post('<?php echo urlShop('store_promotion_xianshi', 'xianshi_goods_price_edit');?>',
                    {xianshi_goods_id: xianshi_goods_id, xianshi_price: xianshi_price, xianshi_storage: xianshi_storage, xianshi_limit: xianshi_limit, first_order: first_order},
                    function(data) {
                        if(data.result) {
                            ...
                            $edit_item.find('[nctype="xianshi_storage"]').text(data.xianshi_storage);
                            $edit_item.find('[nctype="xianshi_limit"]').text(data.xianshi_limit);
                            $edit_item.find('[nctype="first_order"]').text(data.first_order);
...
<table class="ncsc-default-table">
  ...
  <tbody id="xianshi_goods_list">
    ...
    <tr class="bd-line">
        ...
        <td><span nctype="xianshi_storage"><?php echo $val['xianshi_storage'];?></span></td>
        <td><span nctype="xianshi_limit"><?php echo $val['xianshi_limit'];?></span></td>
        <td><span nctype="first_order"><?php echo $val['first_order']?'是':'否';?></span></td>
...
<div id="dialog_edit_xianshi_goods" class="eject_con" style="display:none;">
    ...
    <dl><dt>限时库存：</dt><dd><input id="dialog_edit_xianshi_storage" type="text" class="text w70"><em class="add-on">件</em>
            <p id="dialog_edit_xianshi_goods_sold_error" style="display:none;"><label for="dialog_edit_xianshi_goods_error" class="error"><i class='icon-exclamation-sign'></i>限时折扣库存应低于正常商品库存</label></p>
    </dl>
    <dl><dt>限购数量：</dt><dd><input id="dialog_edit_xianshi_limit" type="text" class="text w70"><em class="add-on">件</em>
            <p id="dialog_edit_xianshi_goods_limit_error" style="display:none;"><label for="dialog_edit_xianshi_goods_error" class="error"><i class='icon-exclamation-sign'></i></label></p>
    </dl>
    <dl><dt></dt><dd><input id="dialog_edit_first_order" type="checkbox" class="" value="1"><label for="dialog_edit_first_order">仅限首单</label>
            <p id="dialog_edit_xianshi_first_order_error" style="display:none;"><label for="dialog_edit_xianshi_goods_error" class="error"><i class='icon-exclamation-sign'></i></label></p>
    </dl>
    <div class="eject_con">
...
<tr class="bd-line">
    ...
    <td><span nctype="xianshi_storage"><%=xianshi_storage%></span></td>
    <td><span nctype="xianshi_limit"><%=xianshi_limit%></span></td>
    <td><span nctype="first_order"><%=first_order_text%></span></td>
    <td class="nscs-table-handle">
    <?php if($output['xianshi_info']['editable']) { ?>
