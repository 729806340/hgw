    ...
    <dl>
      ...
      <dd>
        ...
        <span>送礼品&nbsp;<a href="javascript:void(0);" id="btn_show_search_goods" class="ncbtn"><i class="icon-gift"></i>选择礼品</a></span> <div id="mansong_goods_item" class="gift"></div>
            ...
            <span>适用范围 <select name="mansong_range" id="mansong_range">
                    <option value="0">全场通用</option>
                    <option value="1">仅选定商品适用</option>
                    <option value="2">除选定商品外适用</option>
                </select>
                <a href="javascript:void(0);" class="ncbtn" id="btn_show_search_sku" style="display: none;">选择商品</a>
            </span>
            <span><input type="checkbox" name="mansong_repeat" id="mansong_repeat"> <label for="mansong_repeat">是否累计计算</label></span>
                <div id="mansong_sku_item" class="gift"></div>
        <div id="div_search_sku" class="div-goods-select mt10" style="display: none;">
                    <table class="search-form">
                        <tr>
                            <th class="w150">
                                <strong>第一步：搜索店内商品</strong>
                            ...
                            <td class="w160">
                                <input id="search_sku_name" type="text w150" class="text" name="sku_name" value=""/>
                            </td>
                            <td class="w70 tc">
                                <a href="javascript:void(0);" id="btn_search_sku" class="ncbtn"/><i class="icon-search"></i><?php echo $lang['nc_search'];?></a>
                            </td>
                            <td class="w10"></td>
                            <td>
                                <p class="hint">不输入名称直接搜索将显示店内所有出售中的商品</p>
                    ...
                    <a id="btn_hide_search_sku" class="close" href="javascript:void(0);">X</a>
                    <div id="div_sku_search_result" class="search-result" style="width:739px;"></div>
                </div>
            </div>
        ...
        <span class="error-message"></span>
        <p class="hint">设置当单笔订单满足金额时（必填选项），减免金额（选填）或赠送的礼品（选填）；留空为不做减免金额或赠送礼品处理。<br/>系统最多支持设置三十组规则。</p>
      </dd>
    </dl>
    <dl>
...
<%}%>
    ...
    <a href="<%=sku[i].url%>" title="<%=sku[i].name%>" class="goods-thumb" target="_blank">
        <img src="<%=sku[i].cover%>"/>
    </a>
    <input nctype="mansong_sku_id[]" type="hidden" class="mansong_sku_id" value="<%=sku[i].id%>">
    </div>
    ...
    <span><%if(repeat){%>可累加<%}else{%>不可累加<%}%></span>
<input type="hidden" name="mansong_rule[]" value="<%=price%>,<%=discount%>,<%=goods_id%>,<%=range%>,<%=skuIds%>,<%=repeat%>">
...
<script id="mansong_sku_template" type="text/html">
    <div class="selected-mansong-sku">
    <div nctype="mansong_goods" class="selected-mansong-goods">
    <a href="<%=goods_url%>" title="<%=goods_name%>" class="goods-thumb" target="_blank">
        <img src="<%=goods_image_url%>"/>
    </a>
    <input nctype="mansong_sku_id[]" type="hidden" class="mansong_sku_id" value="<%=goods_id%>">
    </div><a nctype="btn_del_mansong_sku" data-key="<%=goods_id%>" href="javascript:void(0);" class="ncbtn-mini ncbtn-grapefruit"><i class="icon-trash"></i>删除已选择的商品</a>
    </div>
</script>
...
<?php } ?>
    ...
    $('#btn_add_rule').on('click', function() {
        ...
        $('#mansong_range').val(0);
        var mansong_sku_item= $('#mansong_sku_item');
        mansong_sku_item.html('');
        mansong_sku_item.data('sku','');
        $('#mansong_goods_item').html('');
        $('#mansong_price_error').hide();
        $('#div_search_sku').hide();
        $('#div_sku_search_result').html('');
        $('#btn_show_search_sku').hide();
    ...
    $('#btn_save_rule').on('click', function() {
        ...
        var mansongItem=$('#mansong_goods_item');
        mansong.goods = mansongItem.find('[nctype="mansong_goods"]').html();
        mansong.goods_id = Number(mansongItem.find('[nctype="mansong_goods_id"]').val());
        mansong.range = Number($('#mansong_range').val());
        mansong.sku = new Array();
        mansong.skuIds=new Array();
        $('.selected-mansong-sku').each(function (index, item) {
            var $item = $(item);
            var goods = {
                id:$item.find('.mansong_sku_id').val(),
                name:$item.find('[nctype="mansong_goods"] a').attr('title'),
                url:$item.find('[nctype="mansong_goods"] a').attr('href'),
                cover:$item.find('[nctype="mansong_goods"] img').attr('src'),
            };
            mansong.sku.push(goods);
            mansong.skuIds.push(goods.id);
        });
        mansong.repeat = $('#mansong_repeat').prop('checked')?1:0;
        mansong.skuIds = mansong.skuIds.join('|');
        ...
        if(mansong.range>0&&mansong.skuIds=='') {
            alert('适用范围为非全场通用时必须选择商品！');
            return false;
        }
    ...
    function updateSelectedSku() {
        var mansong_sku_item = $('#mansong_sku_item');
        var sku = mansong_sku_item.data('sku');
        if(typeof sku == 'object') {
            for (var i in sku){
                if(sku.hasOwnProperty(i)){
                    $(document).find('#result-sku-'+sku[i]).html('<i class="icon-ok-circle "></i>已经选择');
                }
            }
        }
        return true;
    ...
    $('#mansong_range').on('change',function (e) {
        var $this = $(this);
        if($this.val()>0){
            $('#div_search_sku').fadeIn();
            $('#mansong_sku_item').fadeIn();
            $('#btn_show_search_sku').fadeIn();
        }else{
            $('#div_search_sku').fadeOut();
            $('#mansong_sku_item').fadeOut();
            $('#btn_show_search_sku').fadeOut();
        }
    });
    $('#btn_show_search_sku').on('click',function (e) {
    ...
    $('#btn_search_sku').on('click', function() {
        var url = "<?php echo urlShop('store_promotion_mansong', 'search_sku');?>";
        url += '&' + $.param({goods_name: $('#search_sku_name').val()});
        $('#div_sku_search_result').load(url,function (e) {
            updateSelectedSku();
        });
    });

    var div_goods_search_result = $('#div_goods_search_result');
    var div_sku_search_result = $('#div_sku_search_result');
    // 搜索商品翻页
    div_goods_search_result.on('click', 'a.demo', function() {
        $('#div_goods_search_result').load($(this).attr('href'));
        return false;
    });
    div_sku_search_result.on('click', 'a.demo', function() {
        $('#div_sku_search_result').load($(this).attr('href'),function (e) {
            updateSelectedSku();
        });
        return false;
    });
    ...
    $('#btn_hide_search_sku').on('click', function() {
        $('#div_search_sku').hide();
    });
    ...
    div_sku_search_result.on('click', '[nctype="btn_add_mansong_goods"]', function() {
        var mansong_sku_item = $('#mansong_sku_item');
        var sku = mansong_sku_item.data('sku');
        var goods = {};
        var $this = $(this);
        goods.goods_id = $this.attr('data-goods-id');
        if(typeof sku != 'object') sku = [];
        if($.inArray(goods.goods_id,sku)>-1){
            alert('该商品已经选择！');
            return;
        }
        sku.push(goods.goods_id);
        mansong_sku_item.data('sku',sku);
        goods.goods_name = $this.attr('data-goods-name');
        goods.goods_image_url = $this.attr('data-goods-image-url');
        goods.goods_url = $this.attr('data-goods-url');
        var mansong_goods_item = template.render('mansong_sku_template', goods);
        mansong_sku_item.append(mansong_goods_item);
        $this.html('<i class="icon-ok-circle "></i>已经选择');
    });
    ...
    // 删除以选的商品
    $('#mansong_sku_item').on('click', '[nctype="btn_del_mansong_sku"]', function() {
        var $this = $(this);
        var key = $this.data('key')+'';
        $this.parents('.selected-mansong-sku').remove();
        // 从数组中删除key
        var mansong_sku_item=$('#mansong_sku_item');
        var sku = mansong_sku_item.data('sku')+'';
        sku = sku.split(',');
        var index = $.inArray(key,sku);
        if(index>-1){
            sku.splice(index,1);
            sku = sku.join(',')+'';
            mansong_sku_item.data('sku',sku);
        }
        $('#result-sku-'+key).html('<i class="icon-circle "></i>选择为条件商品');
        console.log(sku);
    });
