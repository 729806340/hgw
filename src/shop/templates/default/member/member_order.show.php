...
<div class="ncm-oredr-show">
  <div class="ncm-order-info">
    ...
    <?php } ?>
    <?php if ($output['order_info']['order_state'] == ORDER_STATE_TUAN_PAY) { ?>
    <div class="ncm-order-condition">
      <dl>
        <dt><i class="icon-ok-circle green"></i>订单状态：</dt>
        <dd>
      ...
      <ul>
        ...
        <li>2. 此订单为拼团订单，拼团成功后商家进行备货发货准备。</li>
        <li>3. 拼团失败系统会自动退款到原支付账户。</li>
        <li>4. 组团中的订单不支持退款操作。</li>
      </ul>
    </div>
    <?php } ?>
    ...
    <?php } ?>
    <?php } ?>
    <div class="mall-msg">有疑问可咨询<a href="javascript:void(0);" id="chat_show_user" onclick="chat_show_user();"><i class="icon-comments-alt"></i>平台客服</a></div>
  </div>
...
var NTKF_PARAM = {
    ...
    "uname":""/*会员名*/,
    "userlevel": "0"/*会员等级*/,
    "erpparam": 'hango:order:<?php echo intval($_GET['order_id'])?>'
}
</script>
