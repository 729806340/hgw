$(function(){
    ...
    var certificationImages = $("#certification_images");
    $('#certification').fileupload({
        dataType: 'json',
        url: SITEURL + '/index.php?act=store_goods_add&op=image_upload&upload_type=uploadedfile',
        formData: {name:'certification'},
        add: function (e,data) {
        	//$('img[nctype="certification"]').attr('src', SHOP_TEMPLATES_URL + '/images/loading.gif');
            data.submit();
        },
        done: function (e,data) {
            var param = data.result;
            if (typeof(param.error) != 'undefined') {
                alert(param.error);
                //$('img[nctype="certification"]').attr('src',DEFAULT_GOODS_IMAGE);
            } else {
                $('input[nctype="certification"]').val(param.name);
                //$('img[nctype="certification"]').attr('src',param.thumb_name);
                var name=[];
                var img = '<img nctype="certification" data-name="'+param.name+'" src="'+param.thumb_name+'" style="margin-right: 5px;"/>';
                certificationImages.append(img);
                certificationImages.find('img').each(function (index,item) {
                    name.push($(item).data('name'));
                    console.log(name.join(','));
                });
                $('input[nctype="certification"]').val(name.join(','));

            }
        }
    });
    certificationImages.on('click','img',function (e) {
        $(this).remove();
        var name=[];
        certificationImages.find('img').each(function (index,item) {
            name.push($(item).data('name'));
    ...
    // 资质证明使用
    $('a[nctype="show_certification"]').unbind().ajaxContent({
        event:'click', //mouseover
        loaderType:"img",
        loadingMsg:SHOP_TEMPLATES_URL+"/images/loading.gif",
        target:'#certification_demo'
    }).click(function(){
        $(this).hide();
        $('a[nctype="del_certification_demo"]').show();
    });
    $('a[nctype="del_certification_demo"]').unbind().click(function(){
        $('#certification_demo').html('');
        $(this).hide();
        $('a[nctype="show_certification"]').show();
    });
    // 商品描述使用
...
function otherCategorySelect(obj)
{
	var id = $(obj).attr('id');
	//console.log(index);
	layer.open({
	  ...
	  maxmin: true,
	  title:'选择更多分类',
	  content: 'index.php?act=store_goods_online&op=more_class&id='+id
	});
}

function addMoreClass()
{
	var num = $('#other-category').children('select').length;
	if( num == 0 ) {
		index = 1;
	} else {
		last = $('#other-category').children('select').last();
		index = Number($(last).attr('data')) + 1;
	}

	new_obj = $('<select onclick="otherCategorySelect(this)" id="oc_'+index+'" data="'+index+'" name="class_rels[]"><option value="">请选择</option></select><span style="cursor: pointer ;" onclick="delMoreClass(this)">&nbsp; x &nbsp;</span>');
	$("#other-category").append(new_obj);
}

function delMoreClass(obj)
{
	var pre = $(obj).prev();
	$(pre).remove();
	$(obj).remove();
