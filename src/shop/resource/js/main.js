...
var Hangowa = function () {
    ...
    var initialUser = function () {
        ...
        if(hango.userId&&hango.userId>0) {
            ...
            $('#n-top-username').text(hango.username);
            $('#user-info').show();
            $('#n-user-info').show();
            $('#purchase-btn1').show();
            $('#bar-user-info').show();
        ...
        $.get(url,function (res) {
            if(res.error==0){
                ...
                if(goods.promotion_type=='xianshi'){
                    var end_time = goods.xianshi_info.end_time;
                    xianshiTick(end_time);
                }
                if(fn&&typeof fn == 'function') fn(goods);
                //console.log(goods);
            }
        },'json');
    };
    var xianshiTick = function (endTime) {
        // TODO 限时倒计时
        ...
        var second = time%60;
        $('#xiansh-hour').text(pad(hour,2));
        $('#xiansh-minute').text(pad(minute,2));
        $('#xiansh-second').text(pad(second.toFixed(0),2));
        setTimeout(function () {
            xianshiTick(endTime);
        },1000);
    ...
    var initialGoods = function (id,fn) {
        hango.goods = {id:id};
        loadGoodsInfo(id,fn);
