...
class cartControl extends BaseBuyControl {
    ...
    public function indexOp() {
        ...
        // 购物车列表 [得到最新商品属性及促销信息]
        $cart_list = $logic_buy_1->getGoodsCartList($cart_list, $jjgObj,$_SESSION['member_id']);
        // 加价购活动
        ...
        foreach ($cart_list as $cart) {
            if ($cart['ifxianshi']) {
                if ($cart['goods_num'] <= $cart['xianshi_num']) {
                    $cart['goods_total'] = bcmul($cart['xianshi_price'], $cart['goods_num'], 2);
                } else {
                    $cur_num = $cart['goods_num'] - $cart['xianshi_num'];
                    $cart['goods_total'] = bcadd(bcmul($cart['xianshi_price'], $cart['xianshi_num'], 2),  bcmul($cur_num, $cart['goods_price'], 2), 2);
                }
            } else {
    ...
    public function add_combOp() {
        ...
        //限时折扣
        $logic_buy_1->getXianshiCartList($goods_list,$_SESSION['member_id']);
    ...
    public function updateOp() {
        ...
        if ($update) {
            ...
            $return['state'] = 'true';
            $return['goods_price'] = $goods_info['goods_price'];
            if ($goods_info['ifxianshi']) {
                if ($quantity <= $goods_info['xianshi_num']) {
                    $return['goods_price'] = $goods_info['xianshi_price'];
                    $return['subtotal'] = $goods_info['xianshi_price'] * $quantity;
                } else {
                    $cur_num = $quantity - $goods_info['xianshi_num'];
                    $return['subtotal'] = $goods_info['xianshi_price'] * $goods_info['xianshi_num'] + $cur_num * $goods_info['goods_price'];
                }
            } else {
