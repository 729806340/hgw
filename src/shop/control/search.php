...
class searchControl extends BaseHomeControl {
    ...
    public function indexOp() {
		...
		//$goods_num= $model_goods->getGoodsCommonOnlineCount($condition);
        $goods_num= $model_goods->getGoodsOnlineCount($condition,"distinct goods_commonid");
		Tpl::output('goods_num',  $goods_num);
        Tpl::output('goods_list', $goods_list);//v($goods_list);
        if ($_GET['keyword'] != ''){
        ...
        // 小能客服系统
        Tpl::output('is_gallery', 1);
        Tpl::output('cat_id', $default_classid);
    ...
    public function auto_completeOp() {
        ...
        if (!C('fullindexer.open')) 
        {
        	if( isset($_GET['term']) && !empty($_GET['term']) )
        	{
        		...
        		$model_goods = Model('goods');
        		$condition = array() ;
        		$condition['goods_name|goods_name_py'] = array('like', '%' . $_GET['term'] . '%');
        		$list = $model_goods->getGoodsCommonList($condition, 'goods_name', 6);
        		$data = array();
                foreach ($list as $row)
                {
                    $row['id'] = mb_substr($row['goods_name'], 0,28,'utf-8');
                    $row['label'] = mb_substr($row['goods_name'], 0,28,'utf-8');
                    $row['value'] = mb_substr($row['goods_name'], 0,28,'utf-8');
                    $data[] = $row;
                }
                
                //追加店铺
                $string = "搜索店铺：".$_GET['term'];
                $row['id'] = $string;
                $row['label'] = $string;
                $row['value'] = $string;
                $data[] = $row;
