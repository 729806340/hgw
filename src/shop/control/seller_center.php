...
class seller_centerControl extends BaseSellerControl {
    ...
    public function indexOp() {
        ...
        /** @var articleModel $model_message */
        $model_message  = Model('article');
        // 店铺信息
        $store_info = $this->store_info;

        //系统公告
        $notice_read_condition = array(
            'shop_id' => $store_info['store_id'],
        );
        $notice_read_list = Model('notice_read')->field('notice_id')->where($notice_read_condition)->select();
        $notice_read_id = array_column($notice_read_list, 'notice_id');
        $notice_condition = array(
            'ac_id' => 1,
            'article_show' => 1,
            'article_position' => 3,
        );
        if (!empty($notice_read_id)) {
            $notice_condition['article_id'] = array('not in', implode(',', $notice_read_id));
        }
        $notice_article = Model('article')->where($notice_condition)->order('article_id ASC')->find();
        Tpl::output('notice_article',$notice_article);
...
//         Tpl::output('help_list',$help_list);
    ...
    public function statisticsOp() {
        ...
        $all = $model_order->getOrderCount($condition);
        // 交易中的订单
    ...
    public function read_article_ajaxOp() {
        /** @var articleModel $model_article */
        $model_article  = Model('article');

        $article_id = intval($_GET['article_id']);
        $article_data = $model_article->getOneArticle($article_id);
        ...
        $next_article = Model('article')->where($article_condition)->order('article_id ASC')->find();
        Tpl::output('article_data',$article_data);
        Tpl::output('article_next',$next_article);
        Tpl::showpage('read_article_ajax.iframe', 'null_layout');
    }

    public function read_articleOp(){
        $notice_id = intval($_POST['article_id']);

        if (!$notice_id) {
            exit(json_encode(array('status'=>true,'msg'=>'参数错误')));
        }
        $store_info = $this->store_info;
        ...
        $status =  Model('notice_read')->insert($insert_data);

        exit(json_encode(array('status'=>$status,'msg'=>'结果')));
    }
