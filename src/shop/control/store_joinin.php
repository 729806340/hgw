...
class store_joininControl extends BaseHomeControl {
        ...
        if(!empty($joinin_detail)) {
            ...
            switch (intval($joinin_detail['joinin_state'])) {
                ...
                case STORE_JOIN_STATE_PAY:
                    $this->show_join_message('已经提交，请等待管理员核对后为您开通店铺', FALSE, '5');
                    break;
    ...
    public function step0Op() {
        ...
        if(isset($_GET['storetype'])){
            $_SESSION['storetype']=$_GET['storetype'];
        }
    ...
    public function step2Op() {
        if(!empty($_POST)) {
            ...
            if(isset($_SESSION['storetype'])) {
             $param['manage_type'] = $_SESSION['storetype'] == 1 ? 'co_construct' : 'platform';
            }
            $model_store_joinin = Model('store_joinin');
    ...
    private function step3_save_valid($param) {
        ...
        $obj_validate->validateparam = array(
            ...
            array("input"=>$param['bank_account_number'], "require"=>"true","validator"=>"Length","min"=>"1","max"=>"32","message"=>"银行账号不能为空且必须小于32个字"),
            array("input"=>$param['bank_name'], "require"=>"true","validator"=>"Length","min"=>"1","max"=>"50","message"=>"开户银行支行名称不能为空且必须小于50个字"),
            array("input"=>$param['bank_code'], "require"=>"true","validator"=>"Length","min"=>"1","max"=>"32","message"=>"支行联行号不能为空且必须小于32个字"),
            ...
            array("input"=>$param['settlement_bank_account_number'], "require"=>"true","validator"=>"Length","min"=>"1","max"=>"32","message"=>"银行账号不能为空且必须小于32个字"),
            array("input"=>$param['settlement_bank_name'], "require"=>"true","validator"=>"Length","min"=>"1","max"=>"50","message"=>"开户银行支行名称不能为空且必须小于50个字"),
            array("input"=>$param['settlement_bank_code'], "require"=>"true","validator"=>"Length","min"=>"1","max"=>"32","message"=>"支行联行号不能为空且必须小于32个字"),
    ...
    public function step4Op() {
        ...
        $condition = array();
        $condition['model'] = 'store_joinin';
        $condition['type'] = $service::TYPE_STORE_JOIN;
        $condition['model_id'] = $_SESSION['member_id'];
        $workflow = $wkModel->getWorkflowInfo($condition);
        if( $workflow ) 
        {
        	$service->init($workflow, $_SESSION['member_name'], '商家');
        	$service->updateWorkflow('运营部');
        	$service->log(1,'重新申请');
        }
        else 
        {
        	$service->init(null,$_SESSION['member_name'],'商家',$service::ROLE_SELLER);
        	try{
        		$service->launch($service::TYPE_STORE_JOIN,$_SESSION['member_id'],array('status'=>STORE_JOIN_STATE_NEW),array(),true);
        	}catch (Exception $e){log::selflog($e->getMessage());
    ...
    public function pay_saveOp() {
        ...
        {
        	$service->init($workflow, $_SESSION['member_name'], '商家');
        	$service->updateWorkflow('出纳');
        	$service->log(1,'提交保证金审核');
        }
        else
        {
        	showMessage("提交审核失败");
