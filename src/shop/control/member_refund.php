...
class member_refundControl extends BaseMemberControl {
    ...
    public function add_refundOp(){
        ...
        $reason_list = $model_refund->getReasonList($condition,"",false);//退款退货原因
        ...
        $refund_state = $model_refund->getRefundState($order,true);//根据订单状态判断是否可以退款退货
...
//            $goods_num = $goods['goods_num'];
            ...
            if ($state) {
                ...
                //客服退货提醒
                $params = array(
                		'return_type' => $refund_array['refund_type'],
                		'uname' => $_SESSION['member_name'],
                		'goods_name' => $goods['goods_name'],
                		'reason_info' => $refund_array['reason_info'],
                		'buyer_message' => $refund_array['buyer_message'],
                		'order_sn' => $order['order_sn']
                ) ;
                $model_refund -> sendNotice( $params ) ;
