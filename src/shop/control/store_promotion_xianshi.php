...
class store_promotion_xianshiControl extends BaseSellerControl {
    ...
    public function xianshi_listOp() {
        ...
        $xianshi_list = $model_xianshi->getXianshiList($condition, 10, 'xianshi_id desc');
    ...
    public function xianshi_add_configOp() {
        ...
        Tpl::showpage('store_promotion_xianshi_config.add');

    }

    public function xianshi_list_configOp() {
        ...
        $condition = array();
        $condition['store_id'] = $_SESSION['store_id'];
        $condition['config_xianshi_id'] = array('gt', 0);
        $xianshi_list = $model_xianshi->getXianshiList($condition, 9999, 'state desc, end_time desc');
        $config_xianshi_id = array_column($xianshi_list, 'config_xianshi_id');
        $w = array();
        if ($type == 1) {
            if (!empty($config_xianshi_id)) {
                $w['config_xianshi_id'] = array('in', $config_xianshi_id);
                $xianshi_list = $model_xianshi->getXianshiConfigList($w);
                if (!empty($xianshi_list)) {
                    $xianshi_config_ids = array_column($xianshi_list, 'config_xianshi_id');
                    if (!empty($xianshi_config_ids)) {
                        $xian_list = $model_xianshi->getXianshiList(array('config_xianshi_id' => array('in', $xianshi_config_ids), 'store_id' => $_SESSION['store_id']), 100, 'state desc, end_time desc');
                        $xian_list = array_under_reset($xian_list, 'config_xianshi_id');
                        if (!empty($xian_list)) {
                            foreach ($xianshi_list as $k => $v) {
                                $xianshi_list[$k]['xianshi_id'] = $xian_list[$v['config_xianshi_id']]['xianshi_id'];
                            }
                        }
                    }
                }
            ...
            if (!empty($config_xianshi_id)) {
                $w['config_xianshi_id'] = array('not in', $config_xianshi_id);
            }
            $xianshi_list = $model_xianshi->getXianshiConfigList($w);
        }

        // 输出导航
        $profile_menu = $type == 1 ? 'xianshi_show_config' : 'xianshi_add_config';
        self::profile_menu($profile_menu);
        Tpl::output('list', $xianshi_list);
        Tpl::output('show_page', $model_xianshi->showpage());
        Tpl::showpage('store_promotion_xianshi_config.list');
    ...
    public function xianshi_saveOp() {
        ...
        $param['config_xianshi_id'] = $_POST['config_xianshi_id'] ? $_POST['config_xianshi_id'] : 0;
    ...
    public function xianshi_goods_addOp() {
        ...
        $xianshi_storage = $_POST['xianshi_storage'];
        $xianshi_limit = $_POST['xianshi_limit'];
        $first_order = $_POST['first_order'];
        ...
        $param['xianshi_storage'] = $xianshi_storage;
        $param['xianshi_limit'] = $xianshi_limit;
    ...
    public function xianshi_goods_price_editOp() {
        ...
        $xianshi_storage = intval($_POST['xianshi_storage']);
        $xianshi_limit = intval($_POST['xianshi_limit']);
        $first_order = intval($_POST['first_order']);
        ...
        $update['xianshi_storage'] = $xianshi_storage;
        $update['xianshi_limit'] = $xianshi_limit;
        ...
        if($result) {
            ...
            $data['xianshi_storage'] = $xianshi_storage;
            $data['xianshi_limit'] = $xianshi_limit;
            $data['first_order'] = $first_order == 1 ? '是' : '否';
    ...
    private function profile_menu($menu_key='') {
        $menu_array = array(
            ...
            2=>array('menu_key'=>'xianshi_show_config','menu_name'=>'已参加活动','menu_url'=>'index.php?act=store_promotion_xianshi&op=xianshi_list_config&type=1'),
            3=>array('menu_key'=>'xianshi_add_config','menu_name'=>'活动报名','menu_url'=>'index.php?act=store_promotion_xianshi&op=xianshi_list_config&type=2'),
        );
        switch ($menu_key){
            case 'xianshi_add':
