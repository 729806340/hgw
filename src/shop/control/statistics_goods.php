...
class statistics_goodsControl extends BaseSellerControl {
    ...
    public function __construct(){
        ...
        //处理搜索时间
        if (in_array($this->search_arr['op'],array('price','hotgoods','channel'))){
    ...
    public function channelOp()
    {
        ...
        $templatesname = 'stat.goods.channel';
        ...
        //查询商品信息
        /** @var goodsModel $gooddModel */
        $gooddModel = Model('goods');
        $goods_info = $gooddModel->getGoodsInfoByID($goods_id, 'goods_id,goods_name');
        if (!$goods_info){
            Tpl::output('stat_msg','参数错误');
            Tpl::showpage($templatesname,'null_layout');
        }
        ...
        $fenxiaoService = Service('Fenxiao');
        $fenxiaoMemebrs = $fenxiaoService->getFenxiaoMembers();
        $fenxiaoUids = array_keys($fenxiaoMemebrs);
        //查询订单商品表下单商品数
        $where = array();
        $where['goods_id'] = $goods_id;
        $where['order_isvalid'] = 1; //计入统计的有效订单
        $where['store_id'] = $_SESSION['store_id'];
        $where['buyer_id'] = array('in',$fenxiaoUids);
        $where['order_add_time'] = array('between',array($searchtime_arr[0],$searchtime_arr[1]));

        $field = 'buyer_id,COUNT(DISTINCT order_id) as ordernum,SUM(goods_num) as ordergoodsnum,SUM(goods_pay_price) as ordergamount ';
        $_group = 'buyer_id';
        $stat_ordergoods = $model->statByStatordergoods($where, $field, 0, 0, '',$_group);

        //v($model->getLastSql(),0);
        //v($stat_ordergoods,0);
        Tpl::output('fenxiaoMemebrs',$fenxiaoMemebrs);
        Tpl::output('stat_ordergoods',$stat_ordergoods);
        Tpl::output('goods_info',$goods_info);
        Tpl::showpage($templatesname);
