...
class store_goods_onlineControl extends BaseSellerControl {
    ...
    public function more_classOp()
    {
    	...
    	Tpl::showpage('add_more_class', 'null_layout');
    }
    ...
    public function edit_goodsOp() {
        ...
        $model_goodsclass = Model('goods_class') ;
        $goods_class = $model_goodsclass->getGoodsClassLineForTag($goodscommon_info['gc_id']);
        //商家审核通过的商品分类
        $seller_goods_class = $model_goodsclass->getGoodsClass($_SESSION['store_id'],0,1,$_SESSION['seller_group_id'],$_SESSION['seller_gc_limits']);
        Tpl::output('goods_class', $goods_class);
        Tpl::output('seller_goods_class', $seller_goods_class);
        ...
        //更多分类
        $result = Model("goods_class_rel")->where( array('goods_commonid' => intval($_GET['commonid'])) )->select() ;
        $more_class_list = array() ;
        foreach ($result as $k => $row)
        {
        	$gc_name = $model_goodsclass->getGoodsClassNav( $row['gc_id'] ) ;
        	$gc_name = array_column(array_values($gc_name), 'title');
        	unset($gc_name[0]);
        	$more_class_list[ $k ] = array(
        			'gc_id' => $row['gc_id'],
        			'gc_name' => implode("<", $gc_name)
        	) ;
        }
        Tpl::output('more_class_list', $more_class_list);
   ...
   * 设置商品佣金
	...
	public function maker_qrcodeOp()
	{
		...
		$lst=$model_goods->getGoodsList($where,'goods_id,store_id');
		if(empty($lst))
		{
			echo '未找到商品信息';
			retrun;
    ...
    //修改成本价
    public function update_goodscostOp(){
        if($_POST['isajax']){
            $goods_id=$_POST['goods_id'];
            $goods_cost=$_POST['goods_cost'];
            $goods_old_cost=$_POST['goods_old_cost'];
            $new_value = array('goods_cost'=>$goods_cost);
            $old_value = array('goods_cost'=>ncPriceFormat($goods_old_cost));
            $storeModel = Model('store');
            /** @var goodsModel $goodsModel */
            $goodsModel = Model('goods');
            $goods_info = $goodsModel->getGoodsInfo(array('goods_id' =>$goods_id));
            ...
            $service->init(null,$_SESSION['member_name'],"商家",'1');
            try{
                $res=$service->launch(54,$goods_id,$new_value,$old_value);
            }catch (Exception $e){
        ...
        $model_goods=Model("goods");
        $goodsdata=$model_goods->getGoodsInfo($where,'goods_name,goods_price,goods_cost,goods_id');
        tpl::output('goodsdata',$goodsdata);
        tpl::showpage('store_update_goodscost','null_layout');
    }
}
