...
class indexControl extends BaseHomeControl{
    ...
    public function index_oldOp(){
        ...
        //自营区块信息
        $self_web_html = $model_web_config->getSelfWebHtml();
        Tpl::output('self_web_html',$self_web_html);

        Model('seo')->type('index')->show();
        Tpl::showpage('new_index');
    }
    ...
    public function indexOp() {
        ...
        //处理秒杀
        $xianshi_list = array();
        $xianshi_config_condition = array(
            'state' => 1,
            'start_time' => array('lt', TIMESTAMP),
            'end_time' => array('gt', TIMESTAMP),
           // 'config_xianshi_id' => array('gt', 0)
        );
        //$xianshi_list = Model('p_xianshi')->getXianshiList($xianshi_config_condition);
        if (!empty($xianshi_list)) {
            $xinashi_ids = array_column($xianshi_list, 'xianshi_id');
            $xianshi_goods_list = Model('p_xianshi_goods')->getXianshiGoodsList(array('xianshi_id' => array('in', $xinashi_ids)));
            if (!empty($xianshi_goods_list)) {
                $xianshi_list['current_time'] = 0;
                $xianshi_list['end_time'] = 0;
                $xianshi_list['text_name'] = '';
                $xianshi_list['goods_list'] = array();
            }
            $xianshi_i = 1;
            foreach($xianshi_goods_list as $xianshi_val) {
                if ($xianshi_i > 3) {
                    break;
                }
                if (!$xianshi_list['current_time']) {
                    $xianshi_list['current_time'] = TIMESTAMP;
                    $xianshi_list['text_name'] = date('H:i', $xianshi_val['start_time']). '秒杀专场';
                    $xianshi_list['end_time'] = $xianshi_val['end_time'];
                }
                $xianshi_list['goods_list'][] = array(
                    'goods_name' => $xianshi_val['goods_name'],
                    'goods_price' => $xianshi_val['goods_price'],
                    'xianshi_price' => $xianshi_val['xianshi_price'],
                    'goods_image' => thumb($xianshi_val, 240),
                    'goods_url' => 1,
                );
                $xianshi_i++;
            }
