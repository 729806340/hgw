...
class member_addressControl extends mobileMemberControl {
    ...
    public function address_listOp() {
        ...
        $city_province = $area_model->getCityProvince();
        foreach ($address_list as $k1=>$v1) {
            $data_list[$k1] = $v1;
            $data_list[$k1]['province_id'] = $city_province[$v1['city_id']];
        }

        output_data(array('address_list' => $data_list));
    }
    ...
    public function address_add_weiOp() {

        $address = $_POST['address'];
        $true_name = $_POST['true_name'];
        $mob_phone = $_POST['mob_phone'];
        if (empty($address) || empty($true_name) || empty($mob_phone)) {
            output_error('参数不正确');
        }
        //$_POST['province_name'] = trim('广东省', '省');
        $province_name = trim($_POST['province_name']);
        $province_name = str_replace('省', '', $province_name);
        /** @var areaModel $area_model */
        $area_model = Model('area');
        $area_info_one = $area_model->getAreaInfo(array('area_name' => $province_name, 'area_parent_id' => 0));
        if (empty($area_info_one)) {
            output_error('请手动填写,暂不支持该地区');
        }
        $city_name = trim($_POST['city_name']);
        $area_info_two = $area_model->getAreaInfo(array('area_name' => $city_name, 'area_parent_id' => $area_info_one['area_id']));
        if (empty($area_info_two)) {
            output_error('请手动填写,暂不支持该地区!'.$city_name);
        }
        $area_name = trim($_POST['area_name']);
        $area_info_three = $area_model->getAreaInfo(array('area_name' => $area_name, 'area_parent_id' => $area_info_two['area_id']));
        if (empty($area_info_three)) {
            output_error('请手动填写,暂不支持该地区!!'.$area_name);
        }

        $_POST['area_id'] = $area_info_three['area_id'];
        $_POST['city_id'] = $area_info_two['area_id'];
        $_POST['province_id'] = $area_info_one['area_id'];
        $_POST['is_default'] = 1;
...
//         );
    ...
    private function _address_valid() {
        ...
        if (!$_POST['area_info']) {
            /** @var areaModel $area_model */
            $area_model = Model('area');
            $area_list = $area_model->getAreaNames();
            if (array_key_exists($_POST['area_id'], $area_list) && array_key_exists($_POST['city_id'], $area_list) && array_key_exists($_POST['province_id'], $area_list)) {
                $_POST['area_info'] = $area_list[$_POST['province_id']]. ' '. $area_list[$_POST['city_id']]. ' '. $area_list[$_POST['area_id']];
            }
        }
        ...
        if (!preg_match('/^1[0-9]{10}$/', $_POST['mob_phone'])) {
