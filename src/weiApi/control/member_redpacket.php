...
class member_redpacketControl extends mobileMemberControl {
    ...
    public function redpacket_listOp(){
        ...
        $rp_state_select = intval($_POST['rp_state_select']);
        $rp_state_select = !in_array($rp_state_select, array(1,2,3)) ? 1 : $rp_state_select;
        $where['rpacket_state'] = $rp_state_select;

        //分页
        $page = new Page();
        $list = $model_redpacket->getRedpacketList($where, '*', 0, $page, 'rpacket_id desc');
