...
class member_refundControl extends mobileMemberControl
{
    ...
    public function refund_all_formOp()
    {
        ...
        if (!empty($order_info) && is_array($order_info)) {
            ...
            $order['order_add_time'] = date('Y-m-d H:i:s', $order_info['add_time']);
    ...
    public function refund_formOp()
    {
        ...
        $goods['is_xianshi'] = ($goods_info['goods_type'] == 3 ) ? true : false;
    ...
    public function get_refund_listOp()
    {
        ...
        $condition = array();
        $condition['buyer_id'] = $member_id;
        $tuanzhang_id = intval($_POST['tz_id']);
        $member_tuanzhang_id = $this->member_info['tuanzhang_id'];
        $show_refund_tzid = 0;
        if ($tuanzhang_id && $member_tuanzhang_id == $tuanzhang_id) {
            unset($condition['buyer_id']);
            $condition['shequ_tz_id'] = $member_tuanzhang_id;
            $show_refund_tzid = $member_tuanzhang_id;
        }
        $refund_state = $_POST['refund_state'];
        if (in_array($refund_state, array(1,2,3))) {
            $condition['refund_state'] = $refund_state;
        }
        ...
        if (!empty($list) && is_array($list)) {
            ...
            foreach ($list as $k => $v) {
                ...
                $val['order_add_time'] = date("Y-m-d H:i:s", time());
                ...
                $val['shequ_tuan_member'] = '';
                if ($show_refund_tzid) {
                    $member_info = $member_model->getMemberInfoByID($val['buyer_id']);
                    $val['shequ_tuan_member'] = array(
                ...
                if ($v['goods_id'] > 0) {
                    ...
                    $order_info = $model_order->getOrderInfo($condition);
                    $val['order_add_time'] = date("Y-m-d H:i:s", $order_info['add_time']);
                }
    ...
    public function get_refund_infoOp()
    {
        ...
        if (!empty($refund_info) && is_array($refund_info)) {
            ...
            $refund['shequ_return_amount'] = $show_refund_tzid ? $refund_info['shequ_return_amount'] : 0;
            ...
            if ($refund_info['seller_state'] == 1) {
                $refund['refund_show_state'] = 0;
            } elseif ($refund_info['seller_state'] == 3) {
                $refund['refund_show_state'] = 1;
            } elseif ($refund_info['seller_state'] == 2 && $refund_info['refund_state'] != 3) {
                $refund['refund_show_state'] = 2;
            } elseif ($refund_info['seller_state'] == 2 && $refund_info['refund_state'] == 3) {
                $refund['refund_show_state'] = 3;
            }
