...
class member_favorites_storeControl extends mobileMemberControl {
    ...
    public function favorites_listOp() {
        ...
        foreach ($favorites_list as $v) {
            ...
            $item['store_slogan'] = $store['store_slogan'];
            ...
            if ($store['is_own_shop']) {
                $item['store_credit'] = array(
                ...
                foreach ((array)$store['store_credit'] as $k => $v) {
                    $v['percent_text'] = $percentClassTextMap[$v['percent_class']];
                    $storeCredit[$k] = $v;
                }
                $item['store_credit'] = $storeCredit;
            }
    ...
    public function favorites_delOp() {
        //$_POST['store_id_list'] = '[13]';
        $fav_id_list = html_entity_decode($_POST['store_id_list']);
        $fav_id_list = json_decode($fav_id_list,true);
        if(count($fav_id_list)<1){
            output_error('非法参数');
        }
        $model_favorites = Model('favorites');
        $model_store = Model('store');
        ...
        $condition['fav_id'] = array('in' , $fav_id_list);
