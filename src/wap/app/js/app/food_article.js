...
//数据回调
    function getPostData(data) {
        ...
        if (result.code == '200') {
            $("#article_publisher_avatar").attr('src', list.article_publisher_avatar);
            $("#article_image").attr('src', list.article_image);
            if (!list.article_publisher_name) {
                list.article_publisher_name = "农谷鲜";
            }
            $("#article_publisher_name").html(list.article_publisher_name);
            $("#article_content").html(list.article_content);
            $("#article_click").html(list.article_up);
        } else {
            console.log('error');
        }
    }
    ...
    function getCommentData(data) {
        ...
        if (result.code == '200') {
            ...
            for (var i = 0; i < max_num; i++) {
                ...
                commentHtml += '    <a class="fr cob9 btn-comment font10" href="comment_list_more.html?key=' + key + '&id=' + list[i].comment_id + '">';
                commentHtml += '     <img src="images/icon-evaluate1.png"/>';
                commentHtml += '     <span class="cogreen">' + list[i].comment_quotes.length + '</span>';
                commentHtml += '     </a>';
...
//文章点赞数据回调
    function clickAssist(data) {
        var result = eval('(' + data + ')');
        if (result.code == '200') {
            var article_up = $("#article_click").html();
            article_up = parseInt(article_up) + 1;
            $("#article_click").html(article_up);
        } else {
            //信息框
            layer.open({
...
//评论点赞的回调
    ...
    $("#more_comment").click(function () {
        window.location.href = "comment_list.html?key=" + key + "&id=" + id;
    })
    //返回
    $("#jump_history").bind('click', function () {
        var article_type = _getUrlString('type');
        if (article_type == 'travel') {
            history.go(-1);
        } else {
            window.location.href = "http://www.finish.com";
        }
    })
