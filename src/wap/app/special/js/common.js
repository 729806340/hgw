...
// function buildAppUrl(e, t) {
//     switch (e) {
//         case "keyword":
//             return buildAppSite + "?" + encodeURIComponent(t);
//         case "special":
//             return buildAppSite + "/special.html?special_id=" + t;
//         case "goods":
//             return buildAppSite + "/tmpl/product_detail.html?goods_id=" + t;
//         case "url":
//             return t
//     }
//     return WapSiteUrl
...
//现联跳转js
function redirect_xianlian(e) {
    var t = getCookie("key");
    if (!t) {
        alert('请先注册成为汉购网会员，才能享受该项福利哦。如果您已注册，请登录！');
        checkLogin(0);
        return
    }
    $.ajax({
        type: "post",
        url: ApiUrl + "/index.php?act=member_index&op=xianlian",
        data: {
            tid: e,
            key: t
        },
