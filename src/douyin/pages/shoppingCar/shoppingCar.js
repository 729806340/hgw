...
Page({
  ...
  add: function (e) {
    ...
    request.postUrl('cart.add', { goods_id: item, quantity: goods.goods_num}, function (res) {  
      that.setData({
        shop_list: that.data.shop_list
      })
  ...
  list_wa: function(){
    var that = this;
    if(wx.getStorageSync('user_token')){
      request.postUrl("cart.list", {}, function (res) {
        if (!res.data.code) {
          return;
        }
  ...
  goSureOrder: function () {
    ...
    if(wx.getStorageSync('user_token')){
      ...
      wx.navigateTo({
        url: '../sureOrder/sureOrder?cart_id=' + cart_id + '&ifcart=' + 1,
      })
    }else{
      wx.switchTab({
          url: '../me/me'
