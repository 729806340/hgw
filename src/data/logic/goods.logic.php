...
class goodsLogic
{
    ...
    const LOG_TYPE_SHOW = 3;
    const LOG_TYPE_HIDE = 4;
    const LOG_TYPE_DELETE= 5;
    ...
    public function saveGoods($param, $store_id, $store_name, $store_state, $seller_id, $seller_name, $bind_all_gc)
    {
        ...
        $newValue = array(
            'tax_input'=>$common_array['tax_input'],
            'tax_output'=>$common_array['tax_output'],
            'certification'=>$common_array['certification'],
        );
        //判断店铺信息，根据店铺类型移除税率信息
        /** @var storeModel $storeModel */
        $storeModel =  Model('store');
        $storeInfo = $storeModel->getStoreInfo(array('store_id'=>$store_id));
        if($storeInfo['manage_type']=='platform') unset($newValue['tax_input'],$newValue['tax_output']);
        unset($common_array['tax_input'],$common_array['tax_output']);//生成商品基本信息时不要添加税率信息
        // 生成通用商品返回通用商品编号
        ...
        //保存更多分类
        if (isset($param['class_rels']) && !empty($param['class_rels'])) {
            $res = $this->_saveMoreCate($param['class_rels'], $common_id);
            if (!$res) {
                return callback(false, '更多分类保存失败');
            }
        }

        // 记录日志
        $this->_recordLog('添加商品，SPU:' . $common_id, $seller_id, $seller_name, $store_id,1,self::LOG_TYPE_ADD);

        /** 启动审批流程 */
        /** @var WorkflowService $workflow */
        $workflow = Service('Workflow');
        $workflow->init(null,$seller_name,'商家',$workflow::ROLE_SELLER);
        try{
            $res = $workflow->launch($workflow::TYPE_GOODS_PUBLISH,$common_id,$newValue,array(),true);
        }catch (Exception $e){
            return callback(false, '提交申请失败');
        }
    ...
    public function updateGoods($param, $store_id, $store_name, $store_state, $seller_id, $seller_name, $bind_all_gc)
    {
        ...
        // 根据参数初始化通用商品数据
        $common_array = $this->_initCommonGoodsByParam($param, $store_id, $store_name, $store_state);
        unset($common_array['tax_input'],$common_array['tax_output']);//更新商品基本信息时不要添加税率信息
        ...
        // 记录日志
        $this->_recordLog('编辑商品，SPU:' . $common_id, $seller_id, $seller_name, $store_id,1,self::LOG_TYPE_EDIT);

        return callback(true, '', $common_id);
    }

    function _saveMoreCate($cat_ids, $common_id)
    {
        $model = Model('goods_class_rel');
        $delWhere['goods_commonid'] = intval($common_id);
        $model->where($delWhere)->delete();

        $data = array();
        foreach ($cat_ids as $gc_id) {
            $data[] = array(
    ...
    {
        // 分类信息
        /** @var goods_classModel $goods_classModel */
        $goods_classModel = Model('goods_class');
        $goods_class = $goods_classModel->getGoodsClassLineForTag(intval($param['cate_id']));
        
        //转换商品名为拼音
        import('pinyin');
    	$py = new CUtf8_PY;
    	$name_py = $py->encode($param['g_name'], 'all');
    	$name_py = str_replace(' ','',$name_py);

        $common_array = array();
        $common_array['goods_name'] = $param['g_name'];
        $common_array['goods_jingle'] = $param['g_jingle'];
        $common_array['goods_name_py'] = $name_py;
        ...
        $common_array['tax_output'] = intval($goods_class['tax_output']);
        $common_array['tax_input'] = intval($goods_class['tax_input']);
        ...
        //$common_array['certification'] = UPLOAD_SITE_URL.'/'.ATTACH_GOODS.'/'.$store_id.'/'.$param['certification_path'];
        $common_array['certification'] = $param['certification_path'];
        ...
        if (is_array($param['spec'])) {
            foreach ($param['spec'] as $value) {
                ...
                if (!empty($goods_info)) {
                    ...
                    if ($this->monitorEdit($goods_info, $goods)) $goods['edit_sap'] = '0';//add by chenlu 20160818
     ...
     * @param $state 1成功 0失败
     */
    private function _recordLog($content = '', $seller_id, $seller_name, $store_id, $state = 1,$type=0)
    {
        $log = array();
    ...
    public function goodsShow($commonid_array, $store_id, $seller_id, $seller_name)
    {
        ...
        if ($return) {
            // 添加操作日志
            $this->_recordLog('商品上架，SPU:' . implode(',', $commonid_array), $seller_id, $seller_name, $store_id,1,self::LOG_TYPE_SHOW);
            return callback(true);
        } else {
    ...
    public function goodsUnShow($commonid_array, $store_id, $seller_id, $seller_name)
    {
        ...
        if ($return) {
            ...
            // 添加操作日志
            $this->_recordLog('商品下架，SPU:' . implode(',', $commonid_array), $seller_id, $seller_name, $store_id,1,self::LOG_TYPE_HIDE);
            return callback(true);
        } else {
    ...
    public function goodsDrop($commonid_array, $store_id, $seller_id, $seller_name)
    {
        ...
        if ($return) {
            // 添加操作日志
            $this->_recordLog('删除商品，SPU：' . implode(',', $commonid_array), $seller_id, $seller_name, $store_id,1,self::LOG_TYPE_DELETE);
            return callback(true);
        } else {
    ...
    //监控更新商品 是否需要推送到SAP  add by chenlu 20160818
    public function monitorEdit($old, $new)
    {
        if (!is_array($old) || !is_array($new)) return false;
        //监控的字段
        $field = array('goods_name', 'gc_id', 'goods_price', 'goods_marketprice', 'goods_state', 'transport_id');
        foreach ($field as $td) {
            if (isset($new[$td]) && $new[$td] != $old[$td]) return true;
        }
    ...
    //微信小程序处理商品列表
    public function deal_goods_list($goods_list) {
        //获取商品列表编号数组
        $goodsid_array = array();
        ...
        $p_pintuan_where = array(
            'goods_id' => array('in', $goodsid_array),
            'state' => 1,
            'end_time' =>  array('gt', TIMESTAMP),
            'start_time' => array('lt', TIMESTAMP),
            'limit_user' => array('gt', 0)
        );
        $tuan_array = $p_pintuan_goods_model->getPintuanGoodsList($p_pintuan_where);
        $tuan_array = array_under_reset($tuan_array, 'goods_id');

        foreach ($goods_list as $key => $value) {
            $goods_list[$key]['sole_flag'] = false;
            $goods_list[$key]['group_flag'] = false;
            $goods_list[$key]['xianshi_flag'] = false;
            $goods_list[$key]['tuan_flag'] = false;
            ...
            } elseif (!empty($tuan_array[$value['goods_id']])) {
                $goods_list[$key]['tuan_flag'] = true;
            } else {
