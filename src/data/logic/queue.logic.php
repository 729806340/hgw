...
class queueLogic {
    ...
    public function createB2bOrderUpdateStorage($goods_buy_quantity) {
    ...
    public function cancelOrderUpdateXianShiStorage($goods_buy_quantity) {
        /** @var p_xianshi_goodsModel $model_xianshi_goods */
        $model_xianshi_goods = model('p_xianshi_goods');
        $result = false;
        foreach ($goods_buy_quantity as $goods_id => $param) {
            $update = array();
            $update['xianshi_sold'] = array('exp','xianshi_sold-'.$param['goods_num']);
            $condition = array(
                'goods_id' => $goods_id,
                'xianshi_id' => $param['promotions_id']
            );
        ...
        if (!$result) {
            return callback(false,'变更限时商品销量失败');
        } else {
            return callback(true);
        }
    }
    ...
    public function build_pwdRedpacket($t_id){
        ...
        while($t_total > 0){
            ...
            for($t = 0; $t < $step; $t++){
                ...
                $tmp['rpacket_skus'] = $t_info['rpacket_t_skus'];
                $tmp['rpacket_range'] = $t_info['rpacket_t_range'];
                $tmp['rpacket_goods_type'] = $t_info['rpacket_t_goods_type'];
