...
class new_goods_classModel extends Model
{
    /**
     * 缓存数据
     */
    ...
    /**
     * 缓存数据 原H('new_goods_class')形式
     */
    protected $gcForCacheModel;

    public function __construct() {
        parent::__construct('new_goods_class');
    }
    ...
    public function dropCache() {
        ...
        dkcache('new_gc_class');
        dkcache('new_all_categories');
    }
    ...
    public function getGoodsClassList($condition, $field = '*') {
        $result = $this->table('new_goods_class')->field($field)->where($condition)->order('gc_parent_id asc,gc_sort asc,gc_id asc')->limit(false)->select();
        return $result;
    }
    ...
    public function get_all_category($update_all = 0) {
        ...
        // 不存在时更新或者强制更新时执行
        if ($update_all == 1 || !($gc_list = rkcache('new_all_categories'))) {
            ...
            wkcache('new_all_categories', $gc_list);
        }
    ...
    public function getKeyWords($gc_id = null){
        ...
        if ($gc_id > 0){
            ...
            $goods_class = Model('new_goods_class')->getGoodsClassIndexedListAll();
