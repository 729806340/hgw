<?php
/**
 * 代金券模型 * @汉购网 (c) 2015-2018 ShopWWI Inc. (http://www.hangowa.com)
...
class voucherModel extends Model
{
    ...
    public function exchangeVoucher($template_info, $member_id, $member_name = '')
    {
        ...
        if ($result) {
            ...
            $voucher_count = rcache($member_id, 'm_voucher');
            $voucher_count = intval($voucher_count['voucher_count']) + 1;
    ...
    //获取会员可领取的免费代金卷列表
    public function getFreesByMemberId($member_id, $num)
    {
        if (empty($member_id) || empty($num)) return array();

        $model_member = Model('member');
        $member_info = $model_member->getMemberInfoByID($member_id);
        //会员级别
        $member_currgrade = $model_member->getOneMemberGrade(intval($member_info['member_exppoints']));
        $level = $member_currgrade ? $member_currgrade['level'] : 0;

        $where['voucher_t_state'] = $this->templatestate_arr['usable'][0];
        $where['voucher_t_gettype'] = $this->voucher_gettype_array['free']['sign'];
        $where['voucher_t_end_date'] = array('gt', time());
        $where['voucher_t_mgradelimit'] = array('elt', $level);
     ...
     * @param int $member_id
     * @param int $number
     * @param int $goods_total
     * @return array
     */
    public function getStoreVoucherList($store_id, $member_id=0, $number=10, $goods_total=0) {
        ...
        if ($member_id > 0) {
            ...
            $member_currgrade = $model_member->getOneMemberGrade(intval($member_info['member_exppoints']));
            $level = $member_currgrade ? $member_currgrade['level'] : 0;
            $param['voucher_t_mgradelimit'] = array('elt', $level);
            //更新过期代金券状态
            $this->_checkVoucherExpire($member_id);
            $where = array(
            ...
            //过滤用户已经领取的
            $exit_voucher_list = $this->table('voucher')->field('voucher_t_id')->where($where)->group('voucher_t_id')->select();
            if (!empty($exit_voucher_list)) {
                $param['voucher_t_id'] = array('not in', array_column($exit_voucher_list, 'voucher_t_id'));
            }
        }

        if ($goods_total > 0) {
            $param['voucher_t_limit'] = array('lt', $goods_total);
        }
        ...
        $param['voucher_t_gettype'] = $this->voucher_gettype_array['free']['sign'];

        $voucher_t_list =  $voucher_model->getVoucherTemplateList($param, '*', 0, $number, 'voucher_t_recommend desc,voucher_t_id desc');
        foreach ($voucher_t_list as $k=>$voucher_t) {
            $voucher_t['voucher_t_start_date'] = date('Y-m-d', $voucher_t['voucher_t_start_date']);
            $voucher_t['voucher_t_end_date'] = date('Y-m-d', $voucher_t['voucher_t_end_date']);
            $voucher_t_list[$k] = $voucher_t;
        }
        return $voucher_t_list;
    }
