...
class redpacketModel extends Model {
    ...
    //返回所有钻石红包
    public function getAllRptTemplate(){
        $where = array();
        $where['rpacket_t_diamond'] = 1;
        $where['rpacket_t_gettype'] = 3;
        return $this->table('redpacket_template')->where($where)->field('rpacket_t_id')->select();
    }

    //判断该红包是否是钻石红包,钻石红包只能在充值时由系统自动发放
    public function isDiamondRpt($rptTplId){
        $where = array();
        $where['rpacket_t_id'] = $rptTplId;
        $where['rpacket_t_diamond'] = 1;
        return $this->table('redpacket_template')->where($where)->find();
    }
    ...
    public function getCurrentAvailableRpt($condition = array(), $goods_total = 0) {
        ...
        $condition['rpacket_state'] = 1;
        $rpt_list = $this->table('redpacket')->field('rpacket_id,rpacket_end_date,rpacket_goods_type,rpacket_price,rpacket_limit,rpacket_t_id,rpacket_code,rpacket_owner_id,rpacket_range,rpacket_skus,rpacket_classes')->where($condition)->key('rpacket_t_id')->select();
    ...
    //领取钻石红包资格判断
    //c.会员是钻石会员 and 红包是钻石红包  这两种情况，钻石会员可以领取钻石红包
    public function canGetDiamondRpt($tid,$member_id){
        if($this->isDiamondRpt($tid) and !Model('member')->isDiamondMember($member_id) ){
            return false;
        }
        return true;
    }
    ...
    public function getCanChangeTemplateInfo($tid,$member_id){
        ...
        //钻石红包只能由钻石会员领取
        //钻石红包非会员领取，返回错误
        if(!$this->canGetDiamondRpt($tid,$member_id)){
            return array('state'=>false,'msg'=>'非钻石会员无法领取钻石红包');
        }
     ...
     * 解冻红包
     */
    public function releaseRedpacket($pay_sn, $buyer_id) {
        $_where = array('rpacket_order_id' => $pay_sn);
        $_data = array(
            'rpacket_state' => '1',
            'rpacket_order_id' => '',
        );
        return $this->editRedpacket($_where, $_data, $buyer_id);
    }
    ...
    public function exchangeRedpacket($template_info, $member_id, $member_name = ''){
        ...
        $insert_arr['rpacket_range'] = $template_info['rpacket_t_range'];
        if($insert_arr['rpacket_range']==3){
            $insert_arr['rpacket_classes'] = $template_info['rpacket_t_classes'];
        }else{
            $insert_arr['rpacket_skus'] = $template_info['rpacket_t_skus'];
        }
        $insert_arr['rpacket_goods_type'] = $template_info['rpacket_t_goods_type'];
        ...
        if ($result){
            ...
            $m_redpacket = rcache($member_id ,'m_redpacket');
            $m_redpacket = intval($m_redpacket['m_redpacket'])+1;
            wcache($member_id, array('redpacket_count' => $m_redpacket), 'm_redpacket');
