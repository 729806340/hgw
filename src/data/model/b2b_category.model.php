...
class b2b_categoryModel extends Model
{
    /**
     * 缓存数据
     */
    ...
    public function __construct() {
        parent::__construct('b2b_category');
    }
    ...
    public function getBcNameByBcid($ids){
        $condition = array();
        $condition['bc_id'] = array('in',$ids);
        return $this->where($condition)->field('bc_name')->select();
    }
    ...
    public function getGoodsClassList($condition, $field = '*') {
        $result = $this->table('b2b_category')->field($field)->where($condition)->order('bc_pid asc,bc_sort asc,bc_id asc')->limit(false)->select();
        return $result;
    }


    //获取自己是第几级分类
    public function getDeep($bc_id){
        $bc_id = $this->table('b2b_category')->where('bc_id = '.$bc_id)->find();
        $deep = 1;
        if($bc_id['bc_pid']){
            $deep = 1 + $deep;
            $bc_id = $this->table('b2b_category')->where('bc_id = '.$bc_id['bc_pid'])->find();
            if($bc_id['bc_pid']){
                $deep = 1 + $deep;
                $bc_id = $this->table('b2b_category')->where('bc_id = '.$bc_id['bc_pid'])->find();
                if($bc_id['bc_pid']) return $deep;
            } else {
                return $deep;
            }
        } else {
    ...
    public function delGoodsClassByGcIdString($gcids) {
        ...
        // 删除商品分类
        $this->delGoodsClass(array('bc_id' => array('in', $gcid_array)));
        return true;
    }
    ...
    public function getChildClass($parent_id){
        ...
        if (is_array($all_class)){
            ...
            foreach ($all_class as $k => $v){
                $gc_id  = $v['bc_id'];//返回的结果包括父类
                $gc_parent_id   = $v['bc_pid'];
    ...
    public function getGoodsClassNav($id = 0, $sign = 1) {
        if (intval ( $id ) > 0) {
            ...
            if ($sign == 1) {
                $nav_link [] = array(
                        'title' => $data[$id]['bc_name']
                );
            } else {
                $nav_link [] = array(
                        'title' => $data[$id]['bc_name'],
                        'link' => urlB2B('search', 'index', array('bc_id' => $data[$id]['bc_id']))
                );
            }
        ...
        // 首页导航
        //$nav_link[] = array('title' => Language::get('homepage'), 'link' => B2B_SITE_URL);
        krsort ( $nav_link );
        return $nav_link;
    }
    ...
    public function getGoodsClassLineForTag($id = 0) {
        if (intval($id)> 0) {
            ...
            $gc_line['tax_input'] = $class['tax_input'];
            $gc_line['tax_output'] = $class['tax_output'];
            /**
             * 是否是子类
             */
    ...
    public function getGoodsclassCache($choose_gcid, $show_depth=3){
        ...
        if ($choose_gcid > 0){
            ...
            for ($i=$gc_depth-1; $i>=0; $i--){
                ...
                $parentid = $gc_list[$parentid]['bc_pid'];
            }
        }
