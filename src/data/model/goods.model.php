...
class goodsModel extends Model
{
    ...
    public function showGoods($condition,$state){
        $data['is_show'] = $state;

        try {
//            $this -> beginTransaction();
//            $res = $this -> table ('goods_common') -> where ( $condition ) -> update ( $data ) ;
//            if( !$res ) {
//                throw new Exception( '更新状态失败' );
//            }
            $res = $this -> table ('goods') -> where ( $condition ) -> update ( $data ) ;
            if( !$res ) {
...
//            $this->rollback();
    ...
    public function getGoodsList($condition, $field = '*', $group = '', $order = '', $limit = 0, $page = 0, $count = 0)
    {
        ...
        //SKU管理过滤掉is_show=1条件
        if(!$condition['is_show_manage']) {
            $condition['is_show'] = 1;
        }
        unset($condition['is_show_manage']);

        $this->getClassMoreGoods($condition);

        $res =  $this->table('goods')->field($field)->where($condition)->group($group)->order($order)->limit($limit)->page($page, $count)->select();
        return  $res;
    }

    public function getFenxiaoGoodsList($condition, $page = 10, $limit = '')
    {
        return $this->table('goods')->field('goods_id,goods_commonid,goods_name,store_id,store_name,goods_price,goods_state,goods_storage,goods_image')
            ->where($condition)
            ->order('goods_id desc')
            ->limit($limit)
            ->page($page)
    ...
    public function getGoodsOnlineList($condition, $field = '*', $page = 0, $order = 'goods_id desc', $limit = 0, $group = '', $lock = false, $count = 0)
    {
        ...
        $stores = $storeModel->getStoreList(array('is_shequ_tuan'=>1),'','store_id','store_id');
        $storeIds = array_column($stores,'store_id');
        if (!isset($condition['store_id']))
            $condition['store_id'] = array('not in',$storeIds);*/
        $goods_list = $this->getGoodsList($condition, $field, $group, $order, $limit, $page, $count);



        if(is_array($goods_list)){
            foreach ($goods_list as $key => $value){
                if($value['goods_promotion_price']>0) continue;
                $goods_list[$key]['goods_promotion_price'] = $goods_list[$key]['goods_price'];
            }
        }
        return $goods_list;
    }
    ...
    public function calculateStorage($goods_list)
    {
        ...
        if (!empty($goods_list)) {
            ...
            $goods_storage = $this->getGoodsList(array('goods_commonid' => array('in', $goodscommonid_array)), 'goods_storage,goods_commonid,goods_id,goods_storage_alarm,goods_cost', '', '', false);
            $storage_array = array();
            foreach ($goods_storage as $val) {
                ...
                $storage_array[$val['goods_commonid']]['goods_cost'] = $storage_array[$val['goods_commonid']]['goods_cost'] . '商品sku_id：'. $val['goods_id'] . '成本价是'. $val['goods_cost'];
    ...
    public function editGoodsById($update, $goodsid_array, $updateXS = false)
    {
        ...
        //add by ljq 添加商品信息修改审核
        if($update['goods_price'])
        {
           $goodsinfo = $this->table('goods')->field('goods_id,goods_commonid,goods_name,goods_price,gc_id,gc_id_1,gc_id_2,gc_id_3,tax_input,tax_output,goods_cost')->where($condition)->select();
           $goodsinfo = array_under_reset($goodsinfo, 'goods_id');
           $new_arr = array('goods_price'=>ncPriceFormat($update['goods_price']) , 'gc_id'=>$update['gc_id'],'gc_id_1'=>$update['gc_id_1'] ,'gc_id_2'=>$update['gc_id_2'],'gc_id_3'=>$update['gc_id_3']);
           foreach($goodsinfo as $k=>$v){
             $old_arr=array('goods_price'=>$v['goods_price'],'gc_id'=>$v['gc_id'],'gc_id_1'=>$v['gc_id_1'] ,'gc_id_2'=>$v['gc_id_2'],'gc_id_3'=>$v['gc_id_3']);
             $compare_arr = array_diff($new_arr,$old_arr);
             if(count($compare_arr)>0){
                 $storeinfo = Model('store')->getStoreInfoByID($update['store_id']);
                 $new_value = array();
                 $old_value = array();
                 foreach($compare_arr as $item=>$val){
                     $new_value[$item] = $new_arr[$item];
                     $old_value[$item] = $old_arr[$item];
                     unset($update[$item]);
                 }                
                 if($storeinfo['manage_type']=='platform'){
                     $type = 2;  //平台商品信息修改审核
                     $title = "平台商品({$v['goods_name']})信息修改审批";
                     $message = '平台商品信息修改审批';
                 }else{
                     //判断是否修改进项税
                     if($compare_arr['gc_id']){
                        $old_value['tax_input'] =  $v['tax_input'];
                        $old_value['tax_output'] = $v['tax_output'];
                        $tax = Model('goods_class')->getGoodsClassInfoById($compare_arr['gc_id']);
                        $new_value['tax_input'] = $tax['tax_input'];
                        $new_value['tax_output'] = $tax['tax_output'];
                     }
                     $type = 1; //共建自营商品修改审核
                     $title = "非平台商品({$v['goods_name']})信息修改审批";
                 }                 
                 /** @var WorkflowService $service */
                 $service = Service('Workflow');
                 $service->init(null,$_SESSION['member_name'],'商家');
                 try{
                     $service->launch($type,$k,$new_value,$old_value,1,$title);
                 }catch (Exception $e){
    ...
    public function editGoodsCommonById($update, $commonid_array)
    {
        ...
        //add by ljq 过滤需要审核修改的值
        $no_update = array('goods_price','gc_id','gc_id_1','gc_id_2','gc_id_3');
        foreach ($no_update as $k=>$v){
            if($update[$v]) unset($update[$v]);
        }
    ...
    {
        //$update_param['edit_sap'] = '0';//违规下架商品时 更新推送标识 add by chenlu 20160818
    ...
    public function getGoodsInfoAndPromotionById($goods_id)
    {
        ...
        //社区限时折扣
        if (C('promotion_allow') && empty($goods_info['sole_info']) && empty($goods_info['groupbuy_info'])) {
            $goods_info['shequ_xianshi_info'] = Model('shequ_xianshi_goods')->getShequXianshiGoodsInfoByGoodsID($goods_info['goods_id']);
        }

        $goods_info['pintuan_info'] = Model('p_pintuan_goods')->getPintuanGoodsInfoByGoodsID($goods_info['goods_id']);
    ...
    public function getGoodsCommonInfoByID($goods_commonid, $fields = '*')
    {
        ...
        if (empty($common_info) || $common_info['tax_input'] == 200 || $common_info['tax_output'] == 200) {
     ...
     * 商品删除及相关信息
     * 
     * @editlog 不从数据库中删除，更新goods表is_del字段为1 zengxj 2016.08.23
     * 
    ...
    public function delGoodsAll($condition)
    {
    	...
    	$commonid_array = array_unique( array_column($common_list, 'goods_commonid') );
    	
    	$data = array(
        		'is_del' => 1
        ) ;
        $where = array(
        		'goods_commonid' => array('in', $commonid_array)
        ) ;
        
        try {
        	
        	$this -> beginTransaction();
        	$res = $this -> table ('goods_common') -> where ( $where ) -> update ( $data ) ;
        	if( !$res ) {
        		throw new Exception( '更新状态失败2' );
        	}
        	$data['goods_edittime'] = time() ;
        	$res = $this -> table ('goods') -> where ( $where ) -> update ( $data ) ;
        	if( !$res ) {
        		throw new Exception( '更新状态失败' );
        	}
    ...
    public function getGoodsInfoByID($goods_id, $fields = '*')
    {
        ...
        if (empty($goods_info) || $goods_info['tax_input'] == 200 || $goods_info['tax_output'] == 200) {
    ...
    public function getGoodsDetail($goods_id)
    {
        ...
        if ($goods_info['is_book'] != '1') {
            ...
            if (!empty($goods_info['pintuan_info'])) {
                $goods_info['promotion_type'] = 'pintuan';
                $goods_info['is_pintuan'] = 1;
                $goods_info['title'] = $goods_info['pintuan_info']['pintuan_title'];
                $goods_info['remark'] = $goods_info['pintuan_info']['pintuan_title'];
                $goods_info['promotion_price'] = $goods_info['pintuan_info']['pintuan_price'];
                $goods_info['down_price'] = ncPriceFormat($goods_info['goods_price'] - $goods_info['pintuan_info']['pintuan_price']);
                $goods_info['pintuan_limit_floor'] = $goods_info['pintuan_info']['limit_floor'];
                $goods_info['pintuan_limit_user'] = $goods_info['pintuan_info']['limit_user'];
            }
            //团购
            ...
            //满即送
            $mansong_info = ($goods_info['is_virtual'] == 1) ? array() : Model('p_mansong')->getMansongInfoByStoreID($goods_info['store_id'],$goods_id);
        }
        ...
        if ($goods_info['is_pintuan'] == 1) {
            $goods_info['buynow_text'] = '立即开团';
    ...
    function getClassMoreGoods(&$condition)
    {
        //更多分类
        foreach ($condition as $k => $v) {
            if (strpos($k, "gc_id_") !== false) {
                $cat_id = $v;
                $where['gc_id'] = array('in', array($v));
                $grel_model = Model("goods_class_rel");
                $rels = $grel_model->where($where)->select();
                if ($rels) {
                    $gids = array_column($rels, 'goods_commonid');
                    $whereGoods = $condition;
                    unset($whereGoods[$k]);
                    $whereGoods['goods_commonid'] 	= array('in', $gids);
                    //过滤下架的
                    $list = $this->table('goods')->where($whereGoods)->select();
                    if ($list) {
                        $gids = array_column($list, 'goods_commonid');
                        $condition['_string'] = ' 1=1  OR  goods_commonid IN (' . implode(",", $gids) . ')';//v($condition);
                    }
                }
            }
        }
     ...
     * @return array
     */
    public function getGoodsChainList($condition, $field = '*', $page=null)
    {
        $condition['goods_state'] = self::STATE1;
        $condition['goods_verify'] = self::VERIFY1;
        $condition['is_del'] = '0';
        ...
        $condition = $this->_getRecursiveClass($condition);
        return $this->table('goods,chain_stock')->field($field)->join('inner')->on('chain_stock.goods_id = goods.goods_id')->where($condition)->page($page)->select();
    }
}
