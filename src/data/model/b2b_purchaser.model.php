...
class b2b_purchaserModel extends Model {

    public function __construct() {
        parent::__construct('b2b_purchaser');
    }


    public function getPurchaserList($condition = array(), $field = '*'){
        return $this->table('b2b_purchaser')->field($field)->where($condition)->select();
    }
    ...
    public function purchaserFlag(){
        //验证自己是否是采购商
        $res = $this->isPurchaser();
        if(!empty($res)){
            Tpl::output('purchaser_flag', 1);
        } else {
            Tpl::output('purchaser_flag', 0);
        }
    }
    /**
     * 新增供应商
     ...
     */
    public function addPurchaser($param){
        try{
            $model = Model('b2b_purchaser');
            $model->beginTransaction();

            $res = $this->insert($param);
            if(!$res){
                throw new Exception('插入采购商失败');
            }
            $res_m = Model('member')->where('member_id = '.$param['member_id'])->update(array('b2b_purchaser' => 1));
            if(!$res_m){
                throw new Exception('改变会员采购商状态失败');
            }
            $model->commit();
            return true;
        } catch(Exception $e){
