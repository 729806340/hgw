...
class cartModel extends Model {
    ...
    private function _addCartDb($goods_info = array(),$quantity) {
        ...
        if (isset($goods_info['config_tuan_id'])) {
            $condition['config_tuan_id'] = $goods_info['config_tuan_id'];
        } else {
            $condition['config_tuan_id'] = 0;
        }
        ...
        $array['config_tuan_id'] = isset($goods_info['config_tuan_id']) ? $goods_info['config_tuan_id'] : 0;
    ...
    //合并小程序未登录的购物车
    public function mergeWeiCart($member_id, $cart_new_list)
    {

        //取出当前DB购物车已有信息
        $cart_cur_list = $this->listCart('db',array('buyer_id'=>$member_id));
        //数据库购物车已经有的商品，不再添加
        if (!empty($cart_cur_list) && is_array($cart_cur_list) && is_array($cart_new_list)) {
            foreach ($cart_new_list as $k=>$v){
                if (!is_numeric($k) || in_array($k,array_column($cart_cur_list, 'goods_id'))){
                    unset($cart_new_list[$k]);
                }
            }
        }
        ...
        if (!empty($goods_list)){
            foreach ($goods_list as $goods_info){
                ...
                $this->addCart($goods_info,'db',$cart_new_list[$goods_info['goods_id']]);
            }
        }
    }
}
