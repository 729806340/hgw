...
class p_xianshiModel extends Model{
    ...
    public function getXianshiList($condition, $page=null, $order='', $field='*') {
        $xianshi_list = $this->table('p_xianshi')->field($field)->where($condition)->page($page)->order($order)->select();
    ...
    public function getXianshiConfigList($condition, $page=null, $order='', $field='*') {
        $xianshi_list = $this->table('p_xianshi_config')->field($field)->where($condition)->page($page)->order($order)->select();
        if(!empty($xianshi_list)) {
            for($i =0, $j = count($xianshi_list); $i < $j; $i++) {
                $xianshi_list[$i] = $this->getXianshiExtendInfo($xianshi_list[$i]);
            }
    ...
    public function getXianshiInfo($condition) {
        $xianshi_info = $this->table('p_xianshi')->where($condition)->find();
        $xianshi_info = $this->getXianshiExtendInfo($xianshi_info);
        return $xianshi_info;
    }

    public function getXianshiConfigInfo($condition) {
        $xianshi_info = $this->table('p_xianshi_config')->where($condition)->find();
        return $xianshi_info;
    }
    ...
    public function addXianshi($param){
        ...
        return $this->table('p_xianshi')->insert($param);
    }

    public function addXianshiConfig($param){
        return $this->table('p_xianshi_config')->insert($param);
    }
    ...
    public function editXianshi($update, $condition){
        return $this->table('p_xianshi')->where($condition)->update($update);
    }
    ...
    public function delXianshi($condition){
        ...
        return $this->table('p_xianshi')->where($condition)->delete();
    }

    public function delXianshiConfig($config_xianshi_id) {
        // 查看是否有店铺使用该活动
        $w = array('config_xianshi_id' => $config_xianshi_id);
        $xianshi = $this->getXianshiList($w);
        if (!empty($xianshi)) return false;

        return $this->table('p_xianshi_config')->where($w)->delete();
    }
