...
class messageModel extends Model {
    ...
    public function getMessageList($sql)
    {
    ...
    //获取消息列表
    public function getWeiMessageList($condition, $limit) {
        $message_list = $this->table('message')->where($condition)->order('message_id DESC')->limit($limit)->select();
        return $message_list;
    }
    ...
    public function saveMessage($param) {
        ...
        // 消息模板
        $msg_tpl_list = $this->get_msg_tpl();
        $content = trim($param['msg_content']);
        $mmt_code = $this->get_cur_msg_tpl($content);
        $cur_msg_tpl = isset($msg_tpl_list[$mmt_code]) ? $msg_tpl_list[$mmt_code] : array();
        $array  = array();
        ...
        if (empty($mmt_code) || empty($cur_msg_tpl)) {
            $data['title'] = '用户消息';
        ...
        $array['message_json']      = json_encode($data, JSON_UNESCAPED_UNICODE);
     ...
     * @return array
     */
    public function get_msg_tpl()
    {
        $msg_tpl = Model('member_msg_tpl')->getMemberMsgTplList(array());
        return array_under_reset($msg_tpl, 'mmt_code');
    }
     ...
     * @return string
     */
    public function get_cur_msg_tpl($message_body)
    {
        $mmt_code = '';
        $str1 = '您关注的商品';
        $str2 = '已经到货';
        if (strpos($message_body, $str1) !== false && strpos($message_body, $str2) !== false) {
