...
class web_configModel extends Model {
    ...
    public function updateWebHtml($web_id = 1, $web_array = array()) {
        $web_html = '';
        $code_list = $this->getCodeList(array(
            'web_id' => $web_id
        ));
        if (! empty($code_list) && is_array($code_list)) {
            ...
            $output = array();
            if (empty($web_array)) {
                $web_list = $this->getWebList(array(
                    'web_id' => $web_id
                ));
            ...
            $output['web_show'] = $web_array['web_show'];
            ...
            switch ($web_page) {
                ...
                case 'channel_fl':
                    ...
                    break;
                //自定义焦点图片 add by guiyj on 20160729    
                case 'custom':
                    $style_file = BASE_DATA_PATH . DS . 'resource' . DS . 'web_config' . DS . 'custom/' . $output['style_name'] . '.php';
                    break;
                case 'self':
                    $style_file = BASE_DATA_PATH . DS . 'resource' . DS . 'web_config' . DS . 'custom/' . $output['style_name'] . '.php';
                    break;
     ...
     * 自定义模块html信息
     */
    public function getCustomWebHtml($style = '', $update_all = 0) {
        $web_array = array();
		$filter = array(
            'web_show' => 1,
            'web_page' => array(
		...
		!empty($style) && $filter['style_name'] = $style;
        $web_list = $this->getWebList($filter);
        if (! empty($web_list) && is_array($web_list)) {
            foreach ($web_list as $k => $v) {
                $key = $v['web_page'] . '_' . $v['style_name'];
                if ($update_all == 1 || empty($v['web_html'])) { // 强制更新或内容为空时查询数据库
                    $web_array[$key] .= $this->updateWebHtml($v['web_id'], $v);
                } else {
                    $web_array[$key] .= $v['web_html'];
     ...
     * 自营模块html信息
     */
    public function getSelfWebHtml($style = '', $update_all = 0) {
        $web_array = array();
        $filter = array(
            'web_show' => 1,
            'web_page' => array(
    ...
    public function getAdvList($type = 'screen') {
        ...
        $result = $this->table('adv_position')
    ...
    public function getGoodsList($condition = array(), $order = 'goods_id desc', $page = '') {
        ...
        if (! empty($goods_list) && is_array($goods_list)) {
            ...
            foreach ($goods_list as $key => $value) {
                ...
                $value['goods_type'] = 1;
                //$value['goods_price'] = ($value['goods_promotion_price'] > 0) ? $value['goods_promotion_price'] : $value['goods_price'];
                $list[$goods_id] = $value;
            }
    ...
    public function updateWebGoods($condition = array('web_show' => '1')) {
        ...
        if (! empty($web_list) && is_array($web_list)) {
            ...
            $condition = array();
            $condition['web_id'] = array(
                'in',
                array_keys($web_style_array)
            );
            $condition['var_name'] = array(
                'in',
                array(
                    'recommend_list',
                    'sale_list'
