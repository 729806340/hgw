...
class shequ_areaModel extends Model {

    public function __construct() {
        parent::__construct('shequ_area');
    }
     ...
     * @return mixed
     */
    public function getAreaListSort($condition = array(), $fields = '*', $order = 'area_sort desc, area_id asc', $page = null) {
        return $this->where($condition)->field($fields)->page($page)->order($order)->limit(false)->select();
    }
    ...
    public function getAreaObject($src = 'cache') {
        if ($src == 'cache') {
            $data = $this->getCache();
        } else {
            $data = $this->_getAllArea();
        ...
        foreach ($data['children'] as $k => $v) {
            $tmp = array();
            foreach ($v as $vv) {
                $tmp[] = array('id'=>$vv, 'name'=>$data['name'][$vv]);
            }
            $arr[$k]=$tmp;
        }
        return $arr;
    ...
    private function _getAllArea() {
        $data = array();
        $area_all_array = $this->limit(false)->order('area_sort desc')->select();
