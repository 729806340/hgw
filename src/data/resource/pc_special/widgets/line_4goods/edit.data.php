...
?>/images/cms_edit_bg_line.png) repeat-y scroll 0 0;
    ...
    .pc-item-edit-content .item:hover .btn_move_up {
        display: block;
        position: absolute;
        top: 4px;
        right: 132px;
    }
    .pc-item-edit-content .item:hover .btn_move_down {
        display: block;
        position: absolute;
        top: 4px;
        right: 67px;
...
<div class="index_block adv_list">
    ...
    <div nctype="item_content" class="content" id="item_content">
        ...
        <select name="item_template">
            <option value="default" <?php if ('default' == $item_info['item_template']) {
                echo "selected";
            } ?> >默认样式
            </option>
            <option value="style1" <?php if ('style1' == $item_info['item_template']) {
                echo "selected";
            } ?> >风格样式一
            </option>
            <option value="style02" <?php if ('style02' == $item_info['item_template']) {
                echo "selected";
            } ?> >风格样式二
            </option>
        </select>
        ...
        <?php if (!empty($item_data['item']['goods']) && is_array($item_data['item']['goods'])) { ?>
            <?php foreach ($item_data['item']['goods'] as $item_value) { ?>
                <div nctype="item_image" class="item" good-item-id="<?php echo $item_value['goods_id']; ?>">
                    ...
                    <input nctype="goods_id" name="item_data[item][goods][]" type="hidden"
                           value="<?php echo $item_value['goods_id']; ?>">

                    <a nctype="btn_move_up" href="javascript:;" type="button" class="btn_move_up">
                        <i class="fa fa-arrow-up "></i>上移
                    </a>
                    <a nctype="btn_move_down" href="javascript:;" type="button" class="btn_move_down">
...
<div class="search-goods">
    ...
    <a id="btn_pc_special_goods_search" class="ncap-btn" href="javascript:;"
       style="vertical-align: top; margin-left: 5px;">搜索</a>
