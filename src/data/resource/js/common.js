...
//现联跳转js
function redirect_xianlian(){
    $.get('index.php?act=index&op=login', function(result){
        if(result=='0'){
            showDialog('请先注册成为汉购网会员，才能享受该项福利哦。如果您已注册，请登录！', 'notice');
            login_dialog();
        }else{
            var url = 'index.php?act=member&op=xianlian';
...
function drop_topcart_item(cart_id){
    $.getJSON(SITEURL+'/index.php?act=cart&op=del&cart_id='+cart_id+'&callback=?', function(result){
        ...
        if(result.state){
            ...
            $('#top-cart-count').html(cart_goods_num).show();
