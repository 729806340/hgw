...
/* B2B商品分类选择函数 */
function gcategoryInitB2B(divId)
{
    $("#" + divId + " > select").get(0).onchange = gcategoryChangeB2B; // select的onchange事件
...
function gcategoryChangeB2B()
{
    ...
    {
        var _self = this;
        var url = SITEURL + '/index.php?act=goods&op=josn_class&callback=?';
        $.getJSON(url, {'gc_id':this.value}, function(data){
            if (data)
            {
                if (data.length > 0)
                {
                    $("<select class='class-select'><option value=''>-请选择-</option></select>").change(gcategoryChangeB2B).insertAfter(_self);
                    var data  = data;
	...
	if(gc_json){
		for(var i in gc_json){
			if(gc_json[i].depth == 1){
				html += ('<option value="'+gc_json[i].bc_id+'">'+gc_json[i].bc_name+'</option>');
			}
		}
	}
	html += '</select>';
		...
		if(childid){
			...
			if(childid_arr){
				for(var i in childid_arr){
					html += ('<option value="'+gc_json[childid_arr[i]].bc_id+'">'+gc_json[childid_arr[i]].bc_name+'</option>');
				}
			}
			html += '</select>';
