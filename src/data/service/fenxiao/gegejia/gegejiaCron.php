...
class gegejiaCron
{
    private  $partner='GGJ_WHYHJZ';
    private  $key='f3820a3718844096b3ff8339515b576c';
    private  $gateway='http://openapi.gegejia.com:8902/api';

    //发货兼容问题
    private  $partner1='GGJ_WHYX01';
    private  $key1='0b543ac237734e5883a1f94403e1b6f6';

    public static $source = "gegejia";
    public static $onlineDate = "2017-06-12 12:00:00";//上线的时间
    ...
    private function _sendRequest1($params,$url,$authorize = false)
    {
        $endTime=date('Y-m-d H:i:s');
        $json = '{
			"partner":"'.$this->partner1.'",
			"timestamp":"'.$endTime.'",
			"params":{
				'.$params.',
		    }
        ...
        $md5key=strtoupper(md5($this->key1.$json.$this->key1));
        $curl = new Curl();
...
//            v($res);
                ...
                foreach($res['orders'] as $order){
                    ...
                    if($item=$this->_prepareOrder($order)) {
                        ...
                        foreach($item['item'] as $v){
                            //if($v['fxpid']=='2018321405aa') $goods_num=$v['num'];
                        }
                        if($goods_num!==0){
                            $item['item']=array();
    ...
    private function _prepareOrder($source)
    {
        ...
        $detail['shipping_fee']=$source['freight'];
        return $detail;
    }
    ...
    public function push_ship($params = array())
    {
        ...
        if ($res['success']) {
            ...
            $res1=$this->_sendRequest1($params,$url="/order/sendOrder");
            if ($res1['success']) {
                $res1 = json_encode(array(
                    'succ' => '1',
                    'msg' =>  $res['errMsg'].':'.$res['detail'].'发货成功'
                ));
            }else {
                $res1 = json_encode(array(
                    'succ' => '0',
                    'msg' =>  '宜昌'.$res['errMsg'].':'.$res['detail'].',恩施:'.$res1['errMsg'].':'.$res1['detail']
                ));
            }
            return $res1;
        }
