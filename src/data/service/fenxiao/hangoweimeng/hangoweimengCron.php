 ...
 * 汉购微盟
 */
class hangoweimengCron
{
    private $gateway="http://hangouweixin.hansap.com/web/api.php?uniacid=262";
    public  static $onlineDate="2017-07-26 17:30:00";
    public static $source="hangoweimeng";
    ...
    public function  _sendRequest($params){
        ...
        $curl->post($url);
        if($curl->error){
            Log::record('汉购微盟 HTTP 请求失败! method:' . $url . ';Error:' . $curl->errorCode . ': ' . $curl->errorMessage);
            throw new Exception('汉购微盟 HTTP 请求失败! method:' . $url . ';Error:' . $curl->errorCode . ': ' . $curl->errorMessage);
        }
        $res=$curl->response;
        if(!$res['status']){
            Log::record('汉购微盟返回错误! method:' . $url . '; Info:' . $res['message']);
        }
        return $res;
    }
    ...
    function _prepareOrder($source){
        ...
        foreach($goodsList as $goods){
            $get_goodsid=$goods['optionid']=="0" ? $goods['goodsid']:$goods['optionid'];//判读如果商品没有规格就使用goods_id为分销对应的id
