...
class chuchujieCron
{
    ...
    static $onlineDate = "2017-12-15 9:00:00"; // 上线日期
    public static $source = "chuchujie";
                ...
                foreach($res['info'] as $order){
                    ...
                    if($item=$this->_prepareOrder($order)) {
                        ...
                        foreach($item['item'] as $v){
                            //if(intval($v['fxpid'])==76720530) $goods_num=$v['num'];
                        }
                        if($goods_num!==0){
                            $item['item']=array();
    ...
    private function _sendRequest($url_path,$param){
        ...
        $curl->get($url);
        if ($curl->error) {
            Log::record('楚楚街 HTTP 请求失败! Path:' . $url . ';Error:' . $curl->errorCode . ': ' . $curl->errorMessage);
            throw new Exception('楚楚街 HTTP 请求失败! Path:' . $url . ';Error:' . $curl->errorCode . ': ' . $curl->errorMessage);
        }
