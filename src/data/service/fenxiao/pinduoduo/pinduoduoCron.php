...
class pinduoduoCron
{
    ...
    public static $source = "pinduoduo";
    ...
    public function __construct($getRel = 1)
    {
        ...
        if (empty($config)) {
            Log::record("数据库中拼多多暂无token相关配置，请手动获取token值");
            exit();
        }
    ...
    private function _sendRequest($param,$flag=0)
    {
        ...
        if ($res->error_response) {
            Log::record('拼多多（汉购网） HTTP 请求失败! Error:' . $res->error_response->error_code . ': ' . $res->error_response->error_message);
            if($res->error_response->error_code==20000){
                ...
                if($return){
                    ...
                    $messege->send('15527487239', "拼多多（汉购网）token更新失败提醒：", <<<"HTML"
<p>手动更新地址：www.hangowa.com/shop/api/pddtoken/gettoken.php</p>
<p>账号：汉购网  密码：Hangowa123</p>
<p>错误日志：$log_time</p>
HTML
                    );
                }*/
        ...
        if($flag||in_array(date('H'),array('11','17'))||time()-$this->update_time) {
            ...
            for ($i = 0; $i <= 19; $i++) {
                ...
                if ($curl->httpStatusCode == 200) {
                    if ($res->error_response) {
                        Log::record('拼多多（汉购网） HTTP 请求失败! Error:' . $res->error_response->error_code . ': ' . $res->error_response->error_message . ' 相关参数为:' . json_encode($param) . " 请求执行了" . ($i + 1) . '次');
                        try {
                            throw new Exception('拼多多（汉购网） HTTP 请求失败! Error:' . $res->error_response->error_code . ': ' . $res->error_response->error_message);
                        } catch (Exception $e) {
                            echo $e->getMessage();
                        }
                        continue;
...
HTML
                    ...
                    Log::record('拼多多（汉购网） HTTP 中curl请求失败! Error:' . $curl->httpStatusCode);
                }
            }
        }
    }
