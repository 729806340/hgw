...
class pddapi {
            ...
            while ( $flag ) {
                ...
                foreach ($response['result']['pageItems'] as $order ) {
                    ...
                    if( !$model -> insert ( $data ) ) {
                        log::selflog("保存失败：" . $order['order_sn'], 'pddhistory') ;
                    }
                }
    ...
    function getInsertData( $row ){
        $data = array() ;
        if( $row['order_sn'] ) {
            $goods_amount = $row['goods_amount'] / 100 ;
            $goods_price = $row['goods_price'] / 100 ;
            $data = array(
                'source' => 'pinduoduo',
                ...
                'discount_amount' => $row['discount_amount'] / 100,
                'total_amount' => $row['goods_amount'] / 100,
                'product_name' => $row['goods_aame'],
                'ship_area' => $row['province_name'] . "/" . $row['city_name'] . "/" . $row['district_name'],
                'ship_name' => $row['receive_name'],
                'ship_addr' => $row['ship_address'],
                'ship_mobile' => $row['mobile'],
                'order_time' => $row['confirm_time'],
                'refund_id' => intval($row['after_sale_id']),
                'refund_status' => intval($row['after_sale_status']),
                'refund_amount' => $row['refund_amount'] ? $row['refund_amount']/100 : 0,
            );//var_dump($data);
        }
        return $data ;
    }
