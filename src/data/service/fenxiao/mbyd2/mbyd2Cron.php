...
class mbyd2Cron
{
    private  $partner='MD_WUHANYIXINGJIUZHOU1';
    private  $key='94958dadc22148779b41f54fc2a6b703';
    private  $gateway='https://openapi.mbyundian.com/api/';


    public static $source = "mbyd2";
    ...
    private function _sendRequest($params,$url,$authorize = false)
    {
        ...
        if ($curl->error) {
            Log::record('脉宝云店2 HTTP 请求失败! method:' . $url . ';Error:' . $curl->errorCode . ': ' . $curl->errorMessage);
            try{
                throw new Exception('脉宝云店2 HTTP 请求失败! method:' . $url . ';Error:' . $curl->errorCode . ': ' . $curl->errorMessage);
            }catch(Exception $e){
                echo $e->getMessage();
            }
        }
...
//        $start_time="2017-12-21 15:00:00";
//        $end_time="2017-12-21 15:27:59";
        $p = 1;
        do{
...
//            v($res);
    ...
    public function push_ship($params = array())
    {
        ...
        if ($res['success']) {
            ...
            $res = json_encode(array(
                'succ' => '0',
                'msg' =>'脉宝云店2-'.$res['errMsg'].':'.$res['detail']
            ));
        }
        return $res;
    }
