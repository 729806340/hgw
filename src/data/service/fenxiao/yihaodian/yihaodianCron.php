...
class yihaodianCron
{
    ...
    private function _prepareOrder($source)
    {
        ...
        foreach ($goodsList as $goods) {
            if (isset($this->rel[$goods['productId']]) && $this->rel[$goods['productId']]) {
                ...
                $this->_error($source['orderCode'], "分销商品 {$goods['productCName']} ({$goods['productId']}) 没有映射");
                $hasError = true;
                continue;
            }
            $items[] = array(
                ...
                'oid' => isset($goods['productId']) ? $goods['productId'] : $source['orderCode'],//必须添加否则fenxiao_sub无法查询
            );
        }
        ...
        $detail['mobile'] = $source['orderInfo']['orderDetail']['goodReceiverMoblie']; //手机号码
    ...
    public function getSkuList($params = array()){
        ...
        $res = $this->_sendRequest($param);
        $data_out = array();
        foreach($res['productList']['product'] as $k => $v){
    ...
    public function checkUnshipOrder()
    {
        ...
        foreach ($orders as $order) {
            ...
            $orderDetail = $this->getOneOrderDetail($fx_order_id);
            if ($orderDetail['errorCount']!="0") continue;
