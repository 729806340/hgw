...
class suningyigouCron
{
    ...
    private $_service;
    public static $source = "suningyigou";
    public static $onlineDate = "2017-08-03 17:40:00"; //上线日期，不保存上线日之前的订单
    ...
    private function _prepareOrder($source)
    {
        ...
        foreach ($goodsList as $goods) {
            ...
            //实际收款金额
            $finalPrice+=$goods['unitPrice']*$goods['saleNum']+$goods['transportFee']-$goods['vouchertotalMoney']-$goods['coupontotalMoney'];
            //优惠金额
            $discount+=$goods['vouchertotalMoney']+$goods['coupontotalMoney'];
        }
    ...
    public function orderlist($params = array())
    {
        ...
        do {
            ...
            if (!empty($res)) {
                $sns = $service->getSavedidByApiorderno(array_column($res,'orderCode'));
                $items = array();
                foreach ($res  as $orders =>$order) {
    ...
    public function getRefundOrder($service)
    {
        ...
        do{
            ...
            if (!empty($res)) {
                $items = $this->_prepareRefund($this->_filterRefunds($res));
    ...
    public function _getReturnOrder($modified_time=null,$page=1){
        ...
        if (is_array($res['sn_body']['batchQueryRejected'])) {
            if (isset($res['sn_head']['totalSize']) && $res['sn_head']['totalSize'] > $page*30) $res['next'] = $page + 1;
            return $res['sn_body']['batchQueryRejected'];
        }
        return array('next' => false, 'count' => 0);
    }
    ...
    private function _prepareRefund($items)
    {
        ...
        foreach ($result as $suborder) {
            ...
            $data['buyer_message'] = $items[$orderno][$goods_id]['returnReason'];  //申请原因
            $data['ordersn'] = $ordersn;  //汉购网订单编号
            $data['goods_id'] = $suborder['productCode']; //商品编号
    ...
    public function traceRefund($service)
    {
        ...
        do {
            ...
            if (!empty($res)) {
                $items = $this->_filterTraceRefunds($res);
