...
class sgsxCron
{
    ...
    public  $mall_id=1049887;
    public  $data_type='JSON';
    public  static $onlineDate="2017-08-10 11:40:00";
    public  static $source = "sgsx";
    ...
    public function __construct($getRel = 1)
    {
        ...
        if (empty($config)) {
            Log::record("数据库中水果生鲜店暂无token相关配置，请手动获取token值");
            exit();
        }
    ...
    {
        //模拟登陆订单补抓，需要传补抓时间，以及istest标记：http://www.hangowa.com/shop/index.php?act=autotask&op=orderlist&source=sgsx&istest=1&begin="2018-01-01 00:00:00"&end="2018-01-07 23:59:59"
        ...
        do {
            if($istest){
                ...
                $end=isset($_GET['end'])?$_GET['end']:"2018-01-07 23:59:59";
                echo "开始抓取第".$page.'页';
                $where['begin']=$begin;
                $where['end']=$end;
    ...
    /*增量获取订单*/
   /* private function getorderlist2($create_time=null,$page = 1,$status=null)
    {
    ...
    private function _sendRequest($param,$flag=0)
    {
        ...
        if ($res->error_response) {
            Log::record('水果生鲜店 HTTP 请求失败! Error:' . $res->error_response->error_code . ': ' . $res->error_response->error_message);
            if($res->error_response->error_code==20000){
                ...
                if($return){
                    ...
                    $messege->send('15527487239', "水果生鲜店token更新失败提醒：", <<<"HTML"
<p>手动更新地址：www.hangowa.com/shop/api/pddtoken/gettoken.php</p>
<p>账号：13125071658  密码：Hangowa321</p>
<p>错误日志：$log_time</p>
HTML
                    );
                }*/
        ...
        if($flag||in_array(date('H'),array('11','17'))||time()-$this->update_time) {
            ...
            for ($i = 0; $i <= 19; $i++) {
                ...
                if ($curl->httpStatusCode == 200) {
                    if ($res->error_response) {
                        Log::record('水果生鲜店 HTTP 请求失败! Error:' . $res->error_response->error_code . ': ' . $res->error_response->error_message . ' 相关参数为:' . json_encode($param) . " 请求执行了" . ($i + 1) . '次');
                        try {
                            throw new Exception('水果生鲜店 HTTP 请求失败! Error:' . $res->error_response->error_code . ': ' . $res->error_response->error_message);
                        } catch (Exception $e) {
                            echo $e->getMessage();
                        }
                        continue;
...
HTML
                    ...
                    Log::record('水果生鲜店 HTTP 中curl请求失败! Error:' . $curl->httpStatusCode);
                }
            }
        }
    }
    ...
    public static function login_pdd($captcha='') {
        ...
        $post_fields['username'] = '13125071658';
