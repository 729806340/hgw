...
class huiguoCron
{
    ...
    private $_config = array(
        ...
        'key' => '31ddd6d58e00936a8addba3b6f8aa202',
        'secret' => '1bee38d34dc74915b184fd2e0a848cbd',
       // 'cus' => '26CE61DAD1C32E6D229A',
        'cus'=> 'hg6FD791531DBB87B450',
    );
...
//    );
    ...
    /** @var  FenxiaoService */
    private $_service;
    public static $source = "huiguo";
    public static $onlineDate = "2018-1-5 14:00:00"; //上线日期，不保存上线日之前的订单
    //在拼多多平台添加了映射的物流名称
...
//        if (C('ON_DEV')) return $this->_devConfig;
    ...
    {
        $data = json_decode(rkcache('huiguo.token'), true);
        if (isset($data['token']) && !empty($data['token']) && isset($data['expire']) && $data['expire'] - 600 > TIMESTAMP) return $data['token'];
        $param = array(
            'secret' => $this->_getKey(),
            'scope' => $this->_scope,
            'type' => 'json'
        ...
        if ($this->_errorCode == self::NO_ERROR) {
            wkcache('huiguo.token', json_encode($res));
            return $res['token'];
        }
...
//            var_dump($goods['goods_sku_id'],$this->rel);exit();
...
//        $createTime='1514998799|'.TIMESTAMP;
        $p = 1;
        $goods_num=0;
        do {
...
//                            if(intval($v['fxpid'])==58416) $goods_num=$v['num'];
//                        }
//                        if($goods_num!==0){
//                            $item['item']=array();
//                            $item['item'][0]['goods_id'] = '103086';
//                            $item['item'][0]['price'] = '1';
//                            $item['item'][0]['num'] = $goods_num;
//                            $item['item'][0]['name'] = '虾稻桂子米 5KG真空装 法泗桂子米 包邮';
//                            $item['item'][0]['fxpid'] = '1';
//                            $item['item'][0]['oid'] = '1';
//                            $item['order_sn'] .= '-1';
//                            $items[] = $item;
