...
class mbyd1Cron
{
    private  $partner='MD_WUHANYIXINGJIUZHOU';
    private  $key='0a65e60452f74a64906d1525bedac2ca';
    private  $gateway='https://openapi.mbyundian.com/api/';

//    //发货兼容问题
//    private  $partner1='GGJ_WHYX01';
//    private  $key1='0b543ac237734e5883a1f94403e1b6f6';

    public static $source = "mbyd1";
    public static $onlineDate = "2017-12-21 12:00:00";//上线的时间
    ...
    private function _sendRequest($params,$url,$authorize = false)
    {
        ...
        if ($curl->error) {
            Log::record('脉宝云店1 HTTP 请求失败! method:' . $url . ';Error:' . $curl->errorCode . ': ' . $curl->errorMessage);
            try{
                throw new Exception('脉宝云店1 HTTP 请求失败! method:' . $url . ';Error:' . $curl->errorCode . ': ' . $curl->errorMessage);
            }catch(Exception $e){
                echo $e->getMessage();
            }
        }
...
//            v($res);
                ...
                foreach($res['orders'] as $order){
                    ...
                    if($item=$this->_prepareOrder($order)) {
                        ...
                        foreach($item['item'] as $v){
                            if($v['fxpid']=='20183221142aa') $goods_num=$v['num'];
                        }
                        if($goods_num!==0){
                            $item['item']=array();
    ...
    public function push_ship($params = array())
    {
        ...
        if ($res['success']) {
            ...
            $res = json_encode(array(
                'succ' => '0',
                'msg' =>'脉宝云店1-'.$res['errMsg'].':'.$res['detail']
            ));
        }
        return $res;
    }
