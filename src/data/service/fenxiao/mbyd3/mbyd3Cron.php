...
class mbyd3Cron
{
    private  $partner='MD_WUHANYIXINGJIUZHOU2';
    private  $key='5df17d33d3e24c8fa27d29a250f5543c';
    private  $gateway='https://openapi.mbyundian.com/api/';
    public static $source = "mbyd3";
    ...
    private function _sendRequest($params,$url,$authorize = false)
    {
        ...
        if ($curl->error) {
            Log::record('脉宝云店3 HTTP 请求失败! method:' . $url . ';Error:' . $curl->errorCode . ': ' . $curl->errorMessage);
            try{
                throw new Exception('脉宝云店3 HTTP 请求失败! method:' . $url . ';Error:' . $curl->errorCode . ': ' . $curl->errorMessage);
            }catch(Exception $e){
                echo $e->getMessage();
            }
        }
...
//            v($res);
    ...
    public function push_ship($params = array())
    {
        ...
        if ($res['success']) {
            ...
            $res = json_encode(array(
                'succ' => '0',
                'msg' =>'脉宝云店3-'.$res['errMsg'].':'.$res['detail']
            ));
        }
        return $res;
    }
