...
class juanpiCron
{
    ...
    private $_scope = 'order_list,order_info,send_goods,aftersale_list,sgoods_list,sgoods_info';
    private $_config = array(
        'host' => 'http://open.juanpi.com',
        'key' => 'b3fbdd35403ac92cd6ff28e74d6eb3d0',
        'secret' => '293c13563f816184553efe6eab59c9ae',
       // 'cus' => '26CE61DAD1C32E6D229A',
        'cus'=> '9D1FE944533567B3E664',
    );
    ...
    /** @var  FenxiaoService */
    private $_service;
    public static $source = "juanpi";
    public static $onlineDate = "2016-11-30 9:00:00"; //上线日期，不保存上线日之前的订单
    //在拼多多平台添加了映射的物流名称
    ...
    private function _sendRequest($param, $authorize = false)
    {
        ...
        if ($curl->error) {
            Log::record('卷皮 HTTP 请求失败! Path:' . $path . ';Error:' . $curl->errorCode . ': ' . $curl->errorMessage);
            throw new Exception('卷皮 HTTP 请求失败! Path:' . $path . ';Error:' . $curl->errorCode . ': ' . $curl->errorMessage);
        }
        $res = $curl->response;
        if ($res['status'] == 0) {
            $this->_errorCode = $res['info'];
            if ($res['info'] == 10007) return array();
            Log::record('卷皮返回错误! Path:' . $path . '; Info:' . $res['info']);
    ...
    {
        $data = json_decode(rkcache('juanpi.token'), true);
        ...
        if ($this->_errorCode == self::NO_ERROR) {
            wkcache('juanpi.token', json_encode($res));
            return $res['token'];
        }
    ...
    private function _prepareOrder($source)
    {
        ...
        if(!isset($newArea[0])||empty($newArea[0])){
            ...
            if(!isset($newArea[1])||empty($newArea[1])){
                ...
                $this->_error($source['orderno'],
                    "分销订单 ({$source['orderno']}) 的收货地址县/市/区信息获取失败，地区数据：{$source['new_area']}");
            }
        }
        $detail = array();
    ...
    public function push_ship($params = array())
    {
        ...
        $param = array(
            ...
            'jDeliverEname' => $logi['companyname'],
            'jDeliverCname' => $logi['code'],
        ...
        if ($this->_errorCode == self::NO_ERROR) {
            ...
            $emailObj = new Email ();
            //var_dump($message);
            //$emailObj->send_sys_email('zenxiangjie@hansap.com', "卷皮发货错误提醒", $message);
            //$emailObj->send_sys_email('shenlei@hansap.com', "卷皮发货错误提醒", $message);
        }
        return $res;
    }
