...
class order extends commons
{
    ...
    public function add($oids=array())
    {
        ...
        $list = $orderModel->getOrderList($where, '', '*', 'order_id', $this->getLimit($this->code), array('order_common', 'order_goods'));

        $where = array();
        $where['send_sap'] = '0';//未推送
        $address_list = $orderModel->getOrderAddressList($where);

        $order_list_key = array_under_reset($list,'order_id');

        $out_list = array();
        foreach($address_list as $address_info){
            $order_info = $order_list_key[$address_info['order_id']];
            if(empty($order_info)){
                continue;
            }
            $order_info['order_sn'] = $order_info['order_sn'].'_'.$address_info['id'];
            $order_info['address_id'] = $address_info['id'];
            $order_info['extend_order_common']['reciver_info']['mob_phone'] = $address_info['buyer_phone'];
            $order_info['extend_order_common']['reciver_info']['address'] = $address_info['address'];
            $order_info['goods_price'] = $address_info['rec_price'];
            $order_info['goods_num'] = $address_info['rec_num'];
			$order_info['buyer_name'] = $address_info['buyer_name'];
            $purchaser_info = Model('b2b_purchaser')->where(array('member_id' => $order_info['buyer_id']))->find();
            $order_info['purchaser_id'] =  $purchaser_info['purchaser_id'];
            $out_list[] = $order_info;
        }
        return $this->conversion($this->clean_add($out_list));
    }
    ...
    private function updateSendState($ids, $state)
    {
        ...
        Model('b2b_order_address')->where($where)->update(array('send_sap' => $state));
        return true;
    }
    ...
    private function conversion($list)
    {
        ...
        foreach ((array)$list as $v) {
            $it['tid'] = $time . '_' . $v['address_id'];//操作唯一标识符

            $it['cardCode'] = $v['purchaser_id'];//店铺ID
//            $it['cardName'] = $v['supplier_name'];//店铺名称
...
//            $order_from = orderFrom($v['order_from']);
            ...
            $it['userFields'] = array(
                ...
                'U_OINV_CUSTOMER' => $v['buyer_name'], //买家
...
//                }
                ...
                if ($item['manage_type']=='platform') {
                    // 调整佣金计算方式
                    // 确认收货时佣金计算以新的方式计算
                    $comprice = ncPriceFormat(($v['goods_price']*$v['goods_num']+$item['rpt_bill']) * $item['commis_rate'] / 100);

                    //订单商品行成本：如果是平台的加上红包，且goods_cost字段与结算单成本计算一致。
                    //不使用原本数据库的goods_cost字段
                    // $item['goods_cost'] =  $item['goods_cost'] + $item['rpt_bill']; //错误写法
                    $item['goods_cost'] =  ncPriceFormat($v['goods_price']*$v['goods_num'] + $item_shipping_fee - $comprice );
                    $item['tax_output'] = $item['tax_input'] = 0;   //平台的税率为0
                }

                $real_pay_price = ncPriceFormat($v['goods_price']*$v['goods_num']);
                $it['docLines'][] = array(
                    'itemCode' => $item['goods_id'],//B1物料编号
                    'priceAfVat' => ncPriceFormat($v['goods_price']),//物料单价
                    'quantity' => $v['goods_num'],//物料数量
