...
class connect_smsControl extends BaseLoginControl{
    ...
    public function find_passwordOp(){
        if (true||checkSeccode($_POST['nchash'],$_POST['captcha'])){
            if(C('sms_password') != 1) {
            ...
            if(empty($sms_log) || ($sms_log['add_time'] < TIMESTAMP - 1800) || $count>= 10) {//半小时内进行验证为有效
                showDialog('动态码输入错误超过10次或已过期，请重新发送','','error');
            }
            if($sms_log['log_captcha'] != $captcha){
                wkcache($cacheKey,$count>0?$count+1:1);
                showDialog('动态码错误，重新输入','','error');
