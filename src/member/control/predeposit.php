...
class predepositControl extends BaseMemberControl {
    ...
    public function rechargecard_addOp()
    {
        ...
        if (!$sn || strlen($pwd) > 50) {
            showMessage('平台充值卡密码不能为空且长度不能大于50', '', 'html', 'error');
            exit;
        }

        try {
            /** @var predepositModel $model */
            $model = Model('predeposit');
            $model->addRechargeCard($sn, $pwd, $_SESSION);

            $res = Model('rechargecard')->chargeCardLog($sn);
            if(!$res)  throw new Exception("充值卡记录失败");
