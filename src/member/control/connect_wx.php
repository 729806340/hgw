...
class connect_wxControl extends BaseLoginControl{
    ...
    public function edit_infoOp(){
        if (chksubmit()) {
            ...
            $member = array();
            $memberInfo = $model_member->getMemberInfo(array('member_id'=> $_SESSION['member_id']));
            $member['member_passwd'] = passwordHash($_POST['password'],$memberInfo['password_salt'],
                $memberInfo['password_account'],
                $memberInfo['member_time']
            );
            //md5($_POST["password"]);
    ...
    public function get_infoOp(){
        ...
        if(!empty($code)) {
            ...
            if(!empty($user_info['unionid'])) {
                ...
                if(empty($member)) $member = $model_member->getMemberInfo(array('weixin_unionid'=> $user_info['openid']));
                if(empty($member)&&C('OLD_STATUS')) {
                    $member = $model_member->getEcOpenid($unionid,'weixin');
                    if(empty($member)) $member = $model_member->getEcOpenid($user_info['openid'],'weixin');
                }
                //v($member);
    ...
    public function get_user_info($code){
        ...
        if(!empty($code_info['access_token'])) {
            ...
            $weixin_info['appid'] = $weixin_appid;
