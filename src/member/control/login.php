    ...
    public function indexOp(){
        ...
        if ($result !== false){
            ...
            if(C('OLD_STATUS')==true){
                $ec_member_info = $model_member->getOldMemberInfo(array('login_account'=>$user_name));
                if($ec_member_info){ // 若旧平台存在对应用户
                    $passwordHash = passwordHash($password,$ec_member_info['password_salt'],$ec_member_info['password_account'],$ec_member_info['member_time']);
                    ...
                    $msg = empty($member_info)?'用户名不存在':'账户同步错误；请联系管理员！';
                    showDialog($msg,'','error',$script);
                }
            }
            /*
		    $condition = array();
...
//获取站点主机后缀(.com, .tmc or .local etc)
    ...
    public function geetestOp() {
        ...
        echo $GtSdk->get_response_str();exit;
    }
    ...
    public function check_memberOp() {
		if(C('ucenter_status')) {
            ...
            if(empty($check_member_name)&&C('OLD_STATUS')==true)
                $check_member_name = $model_member->getOldMemberInfo(array('login_account'=>$_GET['user_name']));
    ...
    public function find_passwordOp(){
        ...
        $passwordHash   = passwordHash($new_password,$member['password_salt'],
            $member['password_account'],
            $member['member_time']
        );

        if(!($member_model->editMember(array('member_id'=>$member['member_id']),array('member_passwd'=>$passwordHash)))){
...
//         while($flag){
//            $order_ids = array();
//            $sql = !empty($lastId) ? "SELECT order_id,buyer_phone 
//                 FROM `shopwwi_orders` 
//                 WHERE order_id >{$lastId} AND order_id IN (
//                 SELECT order_id FROM `shopwwi_order_goods` 
//                 WHERE `order_id` > {$lastId} AND goods_id IN (101046,102112)
//                 )
//                 AND order_state IN(20,30,40) ORDER BY order_id ASC LIMIT 5000":"SELECT order_id , buyer_phone
//                 FROM `shopwwi_orders`
//                 WHERE order_id IN (SELECT order_id FROM `shopwwi_order_goods` WHERE goods_id IN (101046,102112))
//                 AND order_state IN(20,30,40) ORDER BY order_id ASC LIMIT 5000";
//            $result = Model('order')->query($sql);
//            $end = end($result);
//            $lastId = intval($end['order_id']);
//            foreach($result as $k=>$v){
//                $order_ids[] = $v['order_id'];         
//            }
           
//            $commonResult = Model('order_common')
//                      ->field('order_id,reciver_name,reciver_info')
//                      ->where(array('order_id'=>array('in',$order_ids)))
//                      ->select();
//            $commonResult = array_under_reset($commonResult, 'order_id');
    
//            foreach($result as $k=>$v){
// 			   $tongji[$v['buyer_phone']]['buycount'] ++;
//                if(empty($tongji[$v['buyer_phone']]['receiver'])){
//                    $buyer_info = unserialize($commonResult[$v['order_id']]['reciver_info']);
// 				   $tongji[$v['buyer_phone']]['receiver'] = $commonResult[$v['order_id']]['reciver_name'];
//                    $tongji[$v['buyer_phone']]['address'] = $buyer_info['address'];
//                }
//            }
//            count($result) < 5000 and $flag=false;
//         }
//         echo time()."\n";
...
//                'buycount'=>$v['buycount']
//            );
//            Model('mi_users')->insert($export_data);
//         }
        
// //         $mUserCount = count($export_data);
// //         $pagesize = 1000;  //每次写入1000条数据
// //         $pagecount = ceil($mUserCount/$pagesize);
// //         for($i = 0 ; $i<$pagecount ;$i++){
// //             $start = $i*$pagesize;
// //             $data = array_slice($export_data ,$start,$pagesize);
// //             Model('mi_users')->insertAll($data);
// //         }
//         echo time()."\n";
//     }
}
