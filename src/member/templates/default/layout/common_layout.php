...
<![endif]-->
	...
	<script type="text/javascript">
		var PRICE_FORMAT = '<?php echo $lang['
		currency '];?>%s';
		$(function() {
			//首页左侧分类菜单
...
<body>
	...
	<style>
		...
		.search-form .select_option li {
			text-indent: 1.5em;
			width: 70px;
			cursor: pointer;
		}
		
		.search-form .select_option li.selected {
			background-color: #F3F3F3;
			color: #999;
		}
		
		.search-form .select_option li.hover {
			background: #BEBEBE;
			color: #fff;
		}
		
		.search-form input.inp_srh,
	...
	<div class="header-wrap">
		<header class="public-head-layout wrapper">
			...
			<div class="logo-test">
				<?php echo $output['setting_config']['shopwwi_stitle']; ?>
			</div>
	...
	<nav class="public-nav-layout <?php if($output['channel']) {echo 'channel-'.$output['channel']['channel_style'].' channel-'.$output['channel']['channel_id'];} ?>">
		<div class="wrapper">
			...
			<ul class="site-menu">
				...
				<?php } ?>
				<?php if (C('cms_isuse')){ ?>
				<!--<li><a href="<?php echo urlShop('special', 'special_list');?>" <?php if($output[ 'index_sign']=='special' && $output[ 'index_sign'] !='0' ) {echo 'class="current"';} ?>> <span>专题9999</span></a></li>-->
				<?php } ?>
				<?php if(!empty($output['nav_list']) && is_array($output['nav_list'])){?>
				<?php foreach($output['nav_list'] as $nav){?>
				<?php if($nav['nav_location'] == '1'){?>
				<li><a <?php if($nav[ 'nav_new_open']) { echo ' target="_blank"'; } switch($nav[ 'nav_type']) { case '0': echo ' href="' . $nav[ 'nav_url'] . '"'; break; case '1': echo ' href="' . urlShop( 'search', 'index',array( 'cate_id'=>$nav['item_id'])) . '"';
        ...
        ?>><?php echo $nav['nav_title'];?></a></li>
			...
			<div class="con-cart">
				<a target="_self" href="<?php echo SHOP_SITE_URL;?>/index.php?act=cart" class="cart-btn">
					<i class="cart-icon"></i>
					<em class="cart-num"><?php echo $output['cart_goods_num'];?></em>
					<span>购物车</span>
				</a>
			</div>
		</div>
