<template>
  <div class="detailBox">
    <div class="header">
      ...
      <div class="h_item_new" v-if="type == 2">
        <div style="width: 100px;">订单状态：</div>
        <el-select v-model="order_state" placeholder="请选择" class="searchipt">
          <el-option label="全部" value=""></el-option>
          <el-option label="待配货" value="20"></el-option>
          <el-option label="配货中" value="21"></el-option>
          <el-option label="已发货 " value="30"></el-option>
          <el-option label="已完成 " value="40"></el-option>
          <el-option label="已退款 " value="41"></el-option>
        </el-select>
    ...
    <div style="max-height: 370px;overflow-y: auto;">
      <div class="tableBox" v-for="(item,index) in tableData">
        <div class="tabTop">
          ...
          <div v-if="type == 1">物流编号：{{item.shipping_code}}</div>
        </div>
...
<script>
  export default {
    ...
    props: ['tableItem','type'],//父组件传的数据  1物流  2自提
    data () {
      return {
        srcList:[],
        tableData: [],
