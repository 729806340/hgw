...
class goodsControl extends mobileHomeControl{
    ...
    public function goods_detailOp() {
        ...
        if (empty($goods_detail) || $goods_detail['goods_info']['is_del'] == '1' || $goods_detail['goods_info']['store_id'] == 223) {
            output_error('商品不存在');
        }
        ...
        foreach($hot_sales as $value) {
            ...
            $goods_commend['goods_promotion_price'] = $value['goods_promotion_price']>0?$value['goods_promotion_price']:$value['goods_price'];
            if (!empty($sole_array[$value['goods_id']])) {
    ...
    public function ntalkerOp() {
		...
		$items = array(
		    'id'=>$goods_detail['goods_info']['goods_id'],
		    'name'=>$goods_detail['goods_info']['goods_name'],
		    'siteprice'=>$goods_detail['goods_info']['goods_price'],
		    'marketprice'=>$goods_detail['goods_info']['goods_marketprice'],
		    'brand'=>'',
		    'category'=>'',
		    'imageurl'=>cthumb($goods_detail['goods_info']['goods_image'], 360),
		    //'url'=>'http://www.hangowa.com/item-'.$goods_detail['goods_info']['goods_id'].'.html',
            'url'=>url('goods','index',array('goods_id'=>$goods_detail['goods_info']['goods_id']),true,SHOP_SITE_URL),
        );
        $res = array('item' => $items);
