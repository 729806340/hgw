...
class member_buyControl extends mobileMemberControl {
    ...
    public function buy_step1Op() {
        ...
        foreach ($result['store_cart_list'] as $key => $value) {
			...
			if(!empty($store_cart_list[$key]['store_voucher_list'])){
                $voucher_info_data = $store_cart_list[$key]['store_voucher_list'];
                $voucher_t_id = key($voucher_info_data);
                $store_cart_list[$key]['store_voucher_info'] = array(
                    'voucher_price'=>$voucher_info_data[$voucher_t_id]['voucher_price'],
        ...
        //修改无法切换收货地址 update by ljq
        $address_info = array();
        if(intval($_POST['address_id']) > 0){
            $where_address = array();
            $where_address['address_id'] = intval($_POST['address_id']);
            $where_address['member_id'] = $this->member_info['member_id'];
            $address_info = Model('address')->getAddressInfo($where_address);
        }

        $buy_list = array();
        $buy_list['store_cart_list'] = $store_cart_list;
        $buy_list['freight_hash'] = $result['freight_list'];
        $buy_list['address_info'] = $address_info['address_id']>0?$address_info:$result['address_info'];
		...
		$buy_list['rpt_info'] = $result['rpt_list'][0];
		$buy_list['address_api'] = $logic_buy->changeAddr($result['freight_list'], isset($buy_list['address_info']['city_id'])?$buy_list['address_info']['city_id']:'1', '1', $this->member_info['member_id']);
    ...
    public function payOp() {
        ...
        //暂时屏蔽
        $pay_info['member_available_pd'] = 0;
        $pay_info['member_available_rcb'] = 0;
