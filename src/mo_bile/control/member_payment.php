...
class member_paymentControl extends mobileMemberControl {
    ...
    public function pay_newOp() {
        ...
        if (in_array($_GET['payment_code'],array('alipay','wxpay_jsapi','bestpay'))) {
            $model_mb_payment = Model('mb_payment');
            $condition = array();
    ...
    private function _api_pay($order_pay_info) {
        ...
        if ($this->payment_code == 'wxpay_jsapi') {
            ...
            $param['orderFee'] = (int) (bcmul(100, $order_pay_info['api_pay_amount']));
        ...
        if($this->payment_code == 'bestpay'){
            $param['order_sn'] = $order_pay_info['order_no'];
            ...
            $param['order_date'] = $order_pay_info['order_list'][0]['add_time'];
        }else{
        ...
        $html_text = $payment_api->submit($param);

        // 判断是否是微信浏览器，如果是微信浏览器，则用iframe嵌入支付宝页面
        // 因为微信不允许直接302跳转到支付宝页面
        if (false === strpos(strtolower($_SERVER['HTTP_USER_AGENT']), 'micromessenger') ) {
            if($this->payment_code == 'bestpay'){
            ...
            session_start();
            $_SESSION['alipay_form'] = $html_text;
            $key = !empty($_POST['key']) ? $_POST['key'] : '';
            if(empty($key) && !empty($_GET['key'])) {
				$key = $_GET['key'];
			}
            echo '<iframe src="/mo_bile/index.php?act=member_payment&op=redirect_3th_frame' . '&key=' . $key . '" frameborder="0" width="100%" height="1000px;"></iframe>';
            exit; 
        }
    ...
    /**
     * 从session获取 待跳转到支付宝页面付款的html代码，单独用一个页面访问，以便嵌套iframe
     */
    public function redirect_3th_frameOp() {
        session_start();
        $s = $_SESSION['alipay_form'];
        exit($s);
    }
