...
class autoControl extends mobileMemberControl{
	...
	public function checkAuthOp(){
		...
		if (isset($_GET['code'])){				
            ...
            $weixin_info = array();
            $weixin_info['unionid'] = $res5['unionid'];
            $weixin_info['nickname'] = $res5['nickname'];
            $weixin_info['openid'] = $res5['openid'];
            $weixin_info['appid'] = $this->appId;
            $res5['weixin_info'] = serialize($weixin_info);
            ...
            if(empty($member_info)&&C('OLD_STATUS')) {
                $member_info = $model_member->getEcOpenid($res5['unionid'],'weixin');
                if(empty($member_info)) $member_info = $model_member->getEcOpenid($res5['openid'],'weixin');
            }
	...
	private function register($user_info){
        ...
        if(!empty($unionid)) {
            ...
            if(empty($member)&&C('OLD_STATUS')==true)
                $member = $model_member->getOldMemberInfo(array('login_account'=>trim($member_name)));
            if(!empty($member)) {
				if(!empty($member_info)) {
	...
	private function getAccessToken() {
		$data = rkcache('weixin_access_token');//json_decode(file_get_contents("../access_token.json"));
        $check_res = false;
        if($data){
        ...
        if (!$data||!$check_res||$data->expire_time < time() || $check_res->errcode>0) {
			...
			if ($access_token) {
				...
				wkcache('weixin_access_token',$data);
