...
class storeControl extends mobileHomeControl
{
    ...
    public function indexOp()
    {
        ...
        $store_info['store_banner'] = getStoreLogo($store_online_info['mb_title_img']);
        ...
        //畅销列表
        $goods_sales = $model_goods->getGoodsListByColorDistinct($where, $goods_fields, 'goods_salenum desc', 0, 4);
        //新品列表
        $goods_news = $model_goods->getGoodsListByColorDistinct($where, $goods_fields, 'goods_id desc', 0, 4);
        output_data(array(
            'store_info'  => $store_info,
            'goods_sales' => $this->_goods_list_extend($goods_sales),
            'goods_news'  => $this->_goods_list_extend($goods_news),
        ));
    }
