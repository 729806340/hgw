  ...
  var goods_max = 20;//商品数
  var brand_max = 10;//品牌限制
  var recommend_show = 1;//当前选择的商品推荐
  var slide_pic_max = 20;//切换广告图片限制
  var sale_max = 5;//促销区组数
  var sale_goods_max = 20;//商品数
...
function add_slide_adv() {//增加图片
	for (var i = 1; i <= slide_pic_max; i++) {//防止数组下标重复
		if ($("#upload_adv_form ul li[slide_adv_id='"+i+"']").size()==0) {//编号不存在时添加
			...
			text_input += '<textarea name="adv['+i+'][extra_fields]"></textarea> ';
...
function select_slide_adv(pic_id) {//选中图片
	...
	var extra_fields = obj.find("textarea[name='adv["+pic_id+"][extra_fields]']").val();
    var pic_img = obj.find("img").attr("src");
	...
	$("textarea[name='slide_pic[extra_fields]']").html(extra_fields);
	$("textarea[name='slide_pic[extra_fields]']").val(extra_fields);
...
function slide_adv(pic_id,pic_img) {//更新图片
	...
	if (pic_img!='') {
        ...
        var extra_fields = $("textarea[name='slide_pic[extra_fields]']").val();
	    obj.find("img").attr("title",pic_name);
	    obj.find("img").attr("src",UPLOAD_SITE_URL+'/'+pic_img);
		...
		obj.find("textarea[name='adv["+pic_id+"][extra_fields]']").html(extra_fields);
		obj.find("textarea[name='adv["+pic_id+"][extra_fields]']").val(extra_fields);
