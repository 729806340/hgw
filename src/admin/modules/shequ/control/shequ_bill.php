...
WHERE a.order_id = {$orderGoods['order_id']};");
    ...
    public function get_bill_info_xmlOp()
    {
        ...
        elseif($_GET['query_type'] == 'refund'){
            $refund_condition['shequ_tuan_id'] = array('gt','0'); //使用索引
           // $refund_condition['refund_amount'] = array('gt',0);
            $refund_condition['shequ_tz_id'] = $bill_info['ob_store_id'];
            $refund_condition['shequ_tz_bill_id'] = $bill_info['ob_id'];
            //退单列表
            /** @var refund_returnModel $model_refund_return */
            $model_refund_return = Model('refund_return');

            if ($_POST['query'] != '' && in_array($_POST['qtype'], array('order_sn','refund_sn', 'buyer_name'))) {
            ...
            if ($_GET['order_sn'] != '') {
                $refund_condition['order_sn'] = array('like', "%{$_GET['order_sn']}%");
            }
            if ($_GET['refund_sn'] != '') {
                $refund_condition['refund_sn'] = array('like', "%{$_GET['refund_sn']}%");
            }
            if ($_GET['purchase_sap'] != '') {
                $refund_condition['purchase_sap'] = array('eq', "{$_GET['purchase_sap']}");
            }
            if ($_GET['buyer_name'] != '') {
                if ($_GET['jq_query']) {
                    $refund_condition['buyer_name'] = $_GET['buyer_name'];
                } else {
                    $refund_condition['buyer_name'] = array('like', "%{$_GET['buyer_name']}%");
                }
            }
            $sort_fields = array('refund_amount', 'add_time', 'shequ_bill_time', 'buyer_id', 'store_id', 'store_id','rpt_bill');
            if (in_array($_POST['sortorder'], array('asc', 'desc')) && in_array($_POST['sortname'], $sort_fields)) {
                $order = $_POST['sortname'] . ' ' . $_POST['sortorder'];
            }
            $refund_return_list = $model_refund_return->getRefundReturnList($refund_condition, $_POST['rp'], '*', '',$order);
            $data = array();
            $data['now_page'] = $model_refund_return->shownowpage();
            $data['total_num'] = $model_refund_return->gettotalnum();
            foreach ($refund_return_list as $refund_return_info) {
                $list = array();
                $list['operation'] = "<a target=\"_blank\" class=\"btn green\" href=\"index.php?act=refund&op=view&refund_id={$refund_return_info['refund_id']}\"><i class=\"fa fa-list-alt\"></i>查看</a>";
                $list['order_sn'] = $refund_return_info['order_sn'];
                $list['refund_sn'] = $refund_return_info['refund_sn'];
                $list['refund_amount'] = $refund_return_info['refund_amount'];
                $list['shequ_return_amount'] = $refund_return_info['shequ_return_amount'];
                $list['buyer_name'] = $refund_return_info['buyer_name'];
                $list['buyer_id'] = $refund_return_info['buyer_id'];
                $list['store_name'] = $refund_return_info['store_name'];
                $list['store_id'] = $refund_return_info['store_id'];
                $data['list'][$refund_return_info['refund_id']] = $list;
            }
            exit(Tpl::flexigridXML($data));
      }
        else {
...
//        $condition['ob_state'] = BILL_STATE_SYSTEM_CHECK;
     ...
     *
     */
    public function export_refund_billOp()
    {
        ...
        if($_GET['refund_id']){
            $refund_id_arr = explode(',',$_GET['refund_id']);
            if(!empty($refund_id_arr)){
                $refund_condition = array('order_id'=>array('in',$refund_id_arr));
            }else{
        ...
        if ($_GET['query'] != '' && in_array($_GET['qtype'], array('order_sn','refund_sn', 'buyer_name'))) {
            $refund_condition[$_GET['qtype']] = array('like', "%{$_GET['query']}%");
        }
        if ($_GET['query'] != '' && in_array($_GET['qtype'], array('check_status','purchase_sap', 'send_sap'))) {
            $refund_condition[$_GET['qtype']] = array('eq', "{$_GET['query']}");
        }
        if($_GET['search_type'] != ''){
            if($_GET['search_type'] == '已结算'){
                $post_query = array('in','1,2,3');
      ...
      //  $refund_return_list = $model_refund_return->getRefundReturnList($refund_condition, $_POST['rp'], '*', '',$order);
        ...
        if (!is_numeric($_GET['curpage'])) {
            $count = $model_refund_return->getRefundReturn($refund_condition);
            $array = array();
            if ($count > self::EXPORT_SIZE) {
                //显示下载链接
                $page = ceil($count / self::EXPORT_SIZE);
        ...
        $data = $model_refund_return->getRefundReturnList($refund_condition, $_POST['rp'], '*', '',$order);
        $export_data = array();
        $export_data[0] = array('订单编号','退款编号','退款金额','扣除佣金','买家','买家ID','商家','商家ID');
        $ob_shequ_commis_total = 0;
        $ob_shequ_commis_refund = 0;
        $ob_shequ_commis_result = 0;
        foreach($data as $k=>$v){
            ...
            $export_data[$k+1][] = $v['shequ_return_amount'];
    ...
    public function export_orderOp()
    {
        ...
        if($_GET['order_id']){
            $order_id_arr = explode(',',$_GET['order_id']);
            if(!empty($order_id_arr)){
                $condition = array('order_id'=>array('in',$order_id_arr));
            }else{
                $condition = array();
            }
        }else{
       ...
       // ini_set('memory_limit','4G');
        ...
        $condition['shequ_tz_bill_id'] = $bill_info['ob_id'];
        $if_start_date = preg_match('/^20\d{2}-\d{2}-\d{2}$/', $_GET['query_start_date']);
        $if_end_date = preg_match('/^20\d{2}-\d{2}-\d{2}$/', $_GET['query_end_date']);
        $start_unixtime = $if_start_date ? strtotime($_GET['query_start_date']) : null;
        $end_unixtime = $if_end_date ? strtotime($_GET['query_end_date']) : null;
...
//        }
       ...
       if($_GET['search_type'] != ''){
        ...
        $condition['shequ_tuan_id'] = array('gt','0'); //使用索引
