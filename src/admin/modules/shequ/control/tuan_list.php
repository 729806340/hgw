    ...
    public function tuanlist_xmlOp()
    {
        ...
        foreach ($tuan_list as $tuan_info) {
            ...
            if ($tuan_info['state'] == '0') {
                $list['operation'] = "<a class=\"btn orange\" href=\"index.php?act=tuan_list&op=change_state&id={$tuan_info['id']}\"><i class=\"fa fa-gavel\"></i>待审核</a>";
            } else {
                $list['operation'] = "<a class=\"btn orange\" href=\"index.php?act=tuan_list&op=change_state&id={$tuan_info['id']}\"><i class=\"fa fa-list-alt\"></i>已审核</a>";
            }
//            if ($tuan_info['state'] == 0) {
...
//            $list['zhandui'] = $tuan_info['zhandui'];
            $list['area'] = $tuan_address_list['area'];
            $list['street'] = $tuan_address_list['street'];
            $list['community'] = $tuan_address_list['community'];
            $list['address'] = $tuan_address_list['address'];
...
//            $list['bank_sn'] = $tuan_info['bank_sn'];
    ...
    public function change_stateOp()
    {
        ...
        /** @var shequ_tuanzhangModel $model_tuan_zhang */
        ...
        if (chksubmit()) {
            $res = $model_tuan_zhang->edit(array('id' => $id), array('state' =>$_POST['state'], 'update_time' => time(),'register_time'=>time()));
            if ($res&&intval($_POST['state'])=='1') {
                //更新用户团长id
                /** @var memberModel $member_model */
                $member_model = Model('member');
                $member_model->editMember(array('member_id' => $info['member_id']),array('tuanzhang_id' => $id));
                showMessage("审核成功", 'index.php?act=tuan_list&op=index', 'succ');
            } else {
                $model_tuan_zhang->where(array('id'=>$id,'name'=>$info['name']))->delete();
                showMessage("审核不通过", 'index.php?act=tuan_list&op=index', 'error');
            }
        } else {
