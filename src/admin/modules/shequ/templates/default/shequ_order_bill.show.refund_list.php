            ...
            <div class="layout-box">
                ...
                <dl>
                    <dl>
                        <dt>退款编号</dt>
                        <dd>
                            <label><input type="text" value="" name="refund_sn" id="order_sn" class="s-input-txt"></label>
                        </dd>
                    </dl>
                    <dl>
                    <dt>买家</dt>
...
<script type="text/javascript">
    $(function(){
        ...
        $('#ncsubmit').click(function(){
            $("#flexigrid").flexOptions({url: 'index.php?act=shequ_bill&op=get_bill_info_xml&query_type=<?php echo $_GET['query_type'];?>&ob_id=<?php echo $_GET['ob_id'];?>&'+$("#formSearch").serialize(),query:'',qtype:''}).flexReload();
        });

        // 高级搜索重置
        $('#ncreset').click(function(){
            $("#flexigrid").flexOptions({url: 'index.php?act=shequ_bill&op=get_bill_info_xml&query_type=<?php echo $_GET['query_type'];?>&ob_id=<?php echo $_GET['ob_id'];?>'}).flexReload();
            $("#formSearch")[0].reset();
        });
        $("#flexigrid").flexigrid({
            url: 'index.php?act=shequ_bill&op=get_bill_info_xml&query_type=<?php echo $_GET['query_type'];?>&ob_id=<?php echo $_GET['ob_id'];?>',
            colModel : [
                ...
                {display: '退款编号', name : 'refund_sn', width : 130, sortable : true, align: 'left'},
                {display: '退款金额', name : 'refund_amount', width : 70, sortable : true, align: 'left'},
                {display: '扣除佣金', name : 'shequ_return_amount', width : 70, sortable : true, align: 'left'},
            ...
            searchitems : [
                {display: '退款编号', name : 'refund_sn', isdefault: true},
...
//    window.location.href = $("#flexigrid").flexSimpleSearchQueryString() +'&ob_id=<?php //echo $_GET['ob_id'];?>//&op=export_order&order_id='+id;
        window.location.href = $("#flexigrid").flexSimpleSearchQueryString() +'&ob_id=<?php echo $_GET['ob_id'];?>&op=export_refund_bill&refund_id='+id+'&'+$("#formSearch").serialize();
    }
</script>
