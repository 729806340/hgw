...
<div class="page">
    ...
    <div class="explanation" id="explanation">
        ...
        <ul>
            <li>商家已拒绝的退款订单列表，能进行恢复和同意操作退款订单。</li>
        </ul>
    </div>
...
<script type="text/javascript">
    $(function(){
        ...
        $('#ncsubmit').click(function(){
            $("#flexigrid").flexOptions({url: 'index.php?act=refund&op=get_reject_xml&'+$("#formSearch").serialize(),query:'',qtype:''}).flexReload();
        });
        // 高级搜索重置
        $('#ncreset').click(function(){
            $("#flexigrid").flexOptions({url: 'index.php?act=refund&op=get_reject_xml'}).flexReload();
            $("#formSearch")[0].reset();
        });
        $("#flexigrid").flexigrid({
            url: 'index.php?act=refund&op=get_reject_xml',
            colModel : [
                {display: '操作', name : 'operation', width :360, sortable : false, align: 'center', className:"thOver"},
            ...
            buttons : [
                ...
                {display: '批量恢复', name : 'renew', bclass : 'csv', title : '将选定行数据批量恢复', onpress : fg_operate },
                {display: '批量同意', name : 'agree', bclass : 'csv', title : '将选定行数据批量同意', onpress : fg_operate },
                {display: '批量隐藏', name : 'hide', bclass : 'csv', title : '将选定行数据批量隐藏', onpress : fg_operate },
            ],
    ...
    function fg_operate(name, grid) {
        ...
        if (name == 'renew') {
            console.log('已点击按钮renew,准备执行方法refuse_restores');
            refuse_restores(itemlist);
        }
        if (name == 'agree') {
            console.log('已点击按钮agree,准备执行方法refuse_agrees');
            refuse_agrees(itemlist);
        }
        if (name == 'hide') {
            console.log('已点击按钮hide,准备执行方法change_dispaly_states');
            change_dispaly_states(itemlist);
        }
    }
    function fg_csv(ids) {
        id = ids.join(',');
    ...
    function refuse_restore(refund_id){
        if(confirm("你确定要恢复退款订单状态吗？")){
            $.ajax({
                type: "GET",
                dataType: "json",
                url: "index.php?act=refund&op=refuse_restore",
    ...
    //批量恢复
    function refuse_restores(refund_ids){
        console.log('已执行方法refuse_restores');
        if(confirm("你确定要恢复这些退款订单状态吗？")){
            console.log('已点击按钮是');
            $.ajax({
                type: "GET",
                dataType: "json",
                url: "/admin/modules/shop/index.php?act=refund&op=refuse_restores",
                data: {"refund_ids":refund_ids},
                success: function(data){
                    console.log('return success');
                    if (data.state){
                        console.log('data:ture');
                        alert(data.msg);
                        console.log('准备刷新flexigrid');
                        $("#flexigrid").flexReload();
                    } else {
    ...
    function refuse_agree(refund_id){
        if(confirm("你确定要同意退款订单状态吗？")){
            $.ajax({
                type: "GET",
                dataType: "json",
                url: "index.php?act=refund&op=refuse_agree",
                data: {"refund_id":refund_id},
                success: function(data){
                    if (data.state){
                        alert(data.msg);
    ...
    //批量同意
    function refuse_agrees(refund_ids){
        console.log('已开始执行refuse_agrees');
        if(confirm("你确定要同意这些退款订单状态吗？")){
            console.log('已点击按钮是');
            $.ajax({
                type: "GET",
                dataType: "json",
                url: "/admin/modules/shop/index.php?act=refund&op=refuse_agrees",
                data: {"refund_ids":refund_ids},
                success: function(data){
                    console.log('return success');
                    if (data.state){
    ...
    function change_dispaly_state(refund_id,display_state){
        $.ajax({
            type: "POST",
            dataType: "json",
            url: "index.php?act=refund&op=changeDisplay",
    ...
    //批量隐藏
    function change_dispaly_states(refund_ids,display_state){
        console.log('已开始执行change_dispaly_states');
        $.ajax({
            type: "POST",
            dataType: "json",
            url: "/admin/modules/shop/index.php?act=refund&op=changeDisplays",
            data: {"refund_ids":refund_ids,"display_state":display_state},
            success: function(data){
                console.log('return success');
                if (data.state){
                    console.log('data:ture');
