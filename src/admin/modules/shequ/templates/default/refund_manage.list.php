  ...
  <div class="explanation" id="explanation">
    ...
    <ul>
      <li>买家提交申请，商家同意并经平台确认后，退款金额以预存款的形式返还给买家（充值卡部分只能退回到充值卡余额）。</li>
        <?php if ($_GET['op'] == 'kefu') { ?>
            <li>客服人员进行拒绝操作，会改变退款状态为商家已拒绝。</li>
        <?php } ?>
    </ul>
  </div>
...
$(function(){
    ...
    $('#ncsubmit').click(function(){
        $("#flexigrid").flexOptions({url: 'index.php?act=refund&op=get_manage_xml&kefu_state=<?php echo $output['kefu_state'];?>&'+$("#formSearch").serialize(),query:'',qtype:''}).flexReload();
    });
    // 高级搜索重置
    $('#ncreset').click(function(){
        $("#flexigrid").flexOptions({url: 'index.php?act=refund&op=get_manage_xml&kefu_state=<?php echo $output['kefu_state'];?>'}).flexReload();
        $("#formSearch")[0].reset();
    });
    $("#flexigrid").flexigrid({
    	<?php if( $output['fxsellerdo'] == 1 ) { ?>
        url: 'index.php?act=refund&op=get_manage_xml&fxsellerdo=1',
        <?php }else {?>
        url: 'index.php?act=refund&op=get_manage_xml&kefu_state=<?php echo $output['kefu_state'];?>',
        <?php }?>
        colModel : [
            {display: '操作', name : 'operation', width : 160, sortable : false, align: 'center', className: ''},
            ...
            {display: '操作人', name : 'admin_name', width : 40, sortable : true, align: 'center'}
            ],
        searchitems : [
        ...
        sortname: <?php if( $output['fxsellerdo'] == 1 ) { ?>'seller_time'<?php }else{?>'refund_id'<?php }?>,
        sortorder: "desc",
...
function kefu_reject(id) {
    if(confirm('确定要拒绝吗？')){
        $.ajax({
            type: "POST",
            dataType: "json",
            url:'index.php?act=refund&op=kefu_reject&refund_id=' + id
        })
            .done(function (res) {
                console.log(res);
                if (res.state){
        ...
        //window.location.href = 'index.php?act=refund&op=kefu_reject&refund_id=' + id;
    }

}
function fg_csv(ids) {
    id = ids.join(',');
    window.location.href = $("#flexigrid").flexSimpleSearchQueryString()+'&op=export_step1&kefu_state=<?php echo $output['kefu_state'];?>&refund_state=2&refund_id=' + id;
}
</script> 
