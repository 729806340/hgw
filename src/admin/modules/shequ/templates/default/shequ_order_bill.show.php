  ...
  <div class="ncap-form-default">
    ...
    <div class="bot">
      ...
      <?php }elseif ($output['bill_info']['ob_state'] == BILL_STATE_HANGO&&in_array($output['gname'],array('汉购网商务','超级管理员'))){?>
            <a class="ncap-btn-big ncap-btn-green mr10" style="display: none;" onclick="ajax_get_confirm('确定要重建账单吗?重建账单后若有数据变更，将返回商家重新审核。','modules/shequ/index.php?act=shequ_bill&op=rebuild_bill&ob_id=<?php echo $_GET['ob_id'];?>')">重建账单</a>
        <a class="ncap-btn-big ncap-btn-green mr10" onclick="ajax_get_confirm('提交后将无法撤销，进入下一步财务支付，确认提交吗?','modules/shequ/index.php?act=shequ_bill&op=approve_hango&ob_id=<?php echo $_GET['ob_id'];?>')">发送到财务支付</a>

      <?php }elseif ($output['bill_info']['ob_state'] == BILL_STATE_FIRE_PHONIX&&$output['gname']=='公司商务'){?>
        <a class="ncap-btn-big ncap-btn-green mr10" onclick="ajax_get_confirm('确定要重建账单吗?重建账单后若有数据变更，将返回商家重新审核。?','modules/shequ/index.php?act=shequ_bill&op=rebuild_bill&ob_id=<?php echo $_GET['ob_id'];?>')">重建账单</a>
        ...
        <a class="ncap-btn-big ncap-btn-green mr10" onclick="ajax_get_confirm('打回前请上传需要补齐凭证的订单数据附件（建议使用Excel），确认打回吗?','modules/shequ/index.php?act=shequ_bill&op=reject_com&ob_id=<?php echo $_GET['ob_id'];?>')">打回补齐凭证</a>
        <a class="ncap-btn-big ncap-btn-green mr10" onclick="ajax_get_confirm('审核后将无法撤销，进入下一步付款环节，确认审核吗?','modules/shequ/index.php?act=shequ_bill&op=approve_com&ob_id=<?php echo $_GET['ob_id'];?>')">审核完成</a>
      ...
      <?php }elseif ($output['bill_info']['ob_state'] == BILL_STATE_CEO&&$output['gname']=='总经理'){?>
        <a class="ncap-btn-big ncap-btn-green mr10" onclick="ajax_get_confirm('同意后将无法撤销，进入下一步付款环节，确认审核吗?','modules/shequ/index.php?act=shequ_bill&op=approve_pay&ob_id=<?php echo $_GET['ob_id'];?>')">同意支付</a>
...
<!--      <a target="_blank" class="ncap-btn-big ncap-btn-blue mr10" href="index.php?act=shequ_bill&op=bill_part_pay&ob_id=--><?php //echo $_GET['ob_id'];?><!--">--><?php //echo $lang['nc_exdport'];?><!--部分付款</a>-->
  ...
  <div class="homepage-focus" nctype="sellerTplContent">
    ...
    <?php include template($output['tpl_name'], 'shequ');?>
  </div>
</div>
