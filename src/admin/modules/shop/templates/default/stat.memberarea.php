...
<div class="page">
  <div class="fixed-bar">
    <div class="item-title">
      <div class="subject">
        <h3><?php if($output['source'] == 'fenxiao'){echo '（分销）';}?>区域分析</h3>
        <h5><?php if($output['source'] == 'fenxiao'){if($_REQUEST['channel']){echo $output['channel']['member_cn_code'];}else{echo '全部渠道';}} ;?></h5>
      </div>
...
$(function () {
    ...
    //加载统计列表
    $("#statlist").flexigrid({
        url: 'index.php?act=stat_member&op=get_arealist_xml&source=<?php echo $output['source'];?>&t=<?php echo $output['searchtime'];?>&channel=<?php echo $_REQUEST['channel'];?>',
        colModel : [
...
function fg_operation(name, bDiv){
    var stat_url = 'index.php?act=stat_member&op=area_list&source=<?php echo $output['source'];?>&exporttype=excel&t=<?php echo $output['searchtime'];?>&channel=<?php echo $_REQUEST['channel'];?>';
    get_excel(stat_url,bDiv);
}
//加载统计地图
function getStatMap(type){
	$('#'+type+'_div').load('index.php?act=stat_member&op=area_map&source=<?php echo $output['source'];?>&type='+type+'&t=<?php echo $output['searchtime'];?>&channel=<?php echo $_REQUEST['channel'];?>');
}
</script>