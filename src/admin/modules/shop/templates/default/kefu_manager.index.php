  ...
  <div class="fixed-bar">
    ...
    <div class="ncap-search-bar">
        ...
        <form method="get" action="index.php" name="formSearch" id="formSearch">
            <input type="hidden" name="act" value="kefu_manager" />
                        ...
                        <dd>
                            <label>
                                <select name="search_type" id="search_type" class="class-select">
                                    <option value="day" <?php echo $output['search_type']=='day'?'selected':''; ?>>按照天统计</option>
                                    <option value="week" <?php echo $output['search_type']=='week'?'selected':''; ?>>按照周统计</option>
                                    <option value="month" <?php echo $output['search_type']=='month'?'selected':''; ?>>按照月统计</option>
                                </select>
                            </label>
                        </dd>

                        <dd id="searchtype_day">
                            <label>
...
function update_flex() {
    $("#flexigrid").flexigrid({
        url: 'index.php?act=kefu_manager&op=get_xml&'+$("#formSearch").serialize(),
        colModel : [
            {display: '操作', name : 'operation', width : 150, sortable : false, align: 'center', className: 'handle'},
            {display: '客服名称', name : 'admin_name', width : 250, sortable : false, align: 'center'},
            {display: '销售订单数', name : 'order_count', width : 150, sortable : false, align : 'left'},
            {display: '销售订单金额', name : 'order_amount', width : 150, sortable : false, align : 'left'},
            {display: '退款单数', name : 'refund_count', width : 150, sortable : false, align : 'left'},
            {display: '退款单金额', name : 'refund_amount', width : 150, sortable : false, align : 'left'},
        ],
        title: '平台客服列表'
    });
...
$(function () {
    ...
    $('#ncsubmit').click(function(){
        console.log('aaa');
        $("#flexigrid").flexOptions({url: 'index.php?act=kefu_manager&op=get_xml&'+$("#formSearch").serialize(),query:'',qtype:''}).flexReload();
