                ...
                <div id="searchCon" class="content">
                    <div class="layout-box">
                        ...
                        <dl>
                            ...
                            <dd>
                                <label>
                                    <input type="text" value="" name="goods_id" id="goods_id" class="s-input-txt" placeholder="输入商品SKU">
                                </label>
                            </dd>
                        </dl>
                        <dl>
...
<!--                        </dl>-->
                        ...
                        <dl>
                            ...
                            <dd>
                                <label>
                                    <select name="is_pyramid_goods" class="s-select">
                                        <option value=""><?php echo $lang['nc_please_choose'];?></option>
                                        <option value="1">是分销商品</option>
                                        <option value="2">非分销商品</option>
                                    </select>
                                </label>
                            </dd>
                        </dl>
...
<script type="text/javascript">
    $(function(){
        $("#flexigrid").flexigrid({
            url: 'index.php?act=sku&op=get_xml',
            colModel : [
                ...
                {display: '进项税率', name : 'tax_input', width : 60, sortable : false, align: 'center'},
                {display: '销项税率', name : 'tax_output', width : 60, sortable : false, align: 'center'},
                {display: '成本价格(元)', name : 'goods_cost_new', width : 100, sortable : true, align: 'center'},
            ...
            buttons : [
                ...
                {display: '<i class="fa fa-file"></i>显示', name : 'show', bclass : 'csv', title : '将选定行商品显示', onpress : fg_operation },
                {display: '<i class="fa fa-ban"></i>隐藏', name : 'hide', bclass : 'csv', title : '将选定行商品隐藏', onpress : fg_operation },
            ],
            searchitems : [
        ...
        $('#ncsubmit').click(function(){
            $("#flexigrid").flexOptions({url: 'index.php?act=sku&op=get_xml&'+$("#formSearch").serialize(),query:'',qtype:''}).flexReload();
        });

        // 高级搜索重置
        $('#ncreset').click(function(){
            $("#flexigrid").flexOptions({url: 'index.php?act=sku&op=get_xml'}).flexReload();
            $("#formSearch")[0].reset();
        });
    });
    ...
    function fg_operation(name, bDiv) {
        ...
        if (name == 'show' || name == 'hide') {
            var state = (name == 'show')?1:0;
            if($('.trSelected',bDiv).length>0){
                var itemlist = new Array();
                $('.trSelected',bDiv).each(function(){
                    itemlist.push($(this).attr('data-id'));
                });
                fg_show(itemlist,state);
            } else {
                return false;
            }
        }
    ...
    function fg_show(id,state) {
        ...
        $.ajax({
            type: "GET",
            dataType: "json",
            url: "index.php?act=sku&op=goods_show",
