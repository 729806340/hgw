...
<div class="page">
    <div class="fixed-bar">
        <div class="item-title">
            <div class="subject">
                <h3><?php echo $lang['pend_refund_return'];?></h3>
    ...
    <div class="explanation" id="explanation">
        ...
        <ul>
            <li>分销后台地址
                <a target="_blank" href="http://mms.yangkeduo.com/Pdd.html#/aftersales/after_sale_list/index" class="ncap-btn ncap-btn-green" >拼多多</a>
                <a target="_blank" href="https://mch.wxrrd.com/shop/order/list/feedback" class="ncap-btn ncap-btn-green" >人人店</a>
                <a target="_blank" href="https://www.youzan.com/v2/trade/order#list&p=1&type=feedback&state=all&orderby=book_time&order_es_tag=&tuanId=&order=desc&page_size=20&disable_express_type=" class="ncap-btn ncap-btn-green" >有赞</a>
                <a target="_blank" href="https://user.juanpi.com/refund/backList" class="ncap-btn ncap-btn-green" >卷皮</a>
                <a target="_blank" href="http://portal.shzyfl.cn/order/exchangeOrderManagerView.do" class="ncap-btn ncap-btn-green" >返利</a>
                <a target="_blank" href="https://zhaoshang.zhe800.com/" class="ncap-btn ncap-btn-green" >折800</a>
                <a target="_blank" href="http://seller.gegejia.com/yggSellerManager/seller/toLogin" class="ncap-btn ncap-btn-green" >格格家</a>
                <a target="_blank" href="http://zs.mengdian.com/main" class="ncap-btn ncap-btn-green" >萌店</a>
                <a target="_blank" href="http://seller.chuchujie.com" class="ncap-btn ncap-btn-green" >楚楚街</a>
                <a target="_blank" href="http://open.beibei.com/console/application.html" class="ncap-btn ncap-btn-green" >贝贝网</a>
            </li>
        </ul>
    </div>
...
<script type="text/javascript">
    $(function(){
        ...
        $('#ncsubmit').click(function(){
            $("#flexigrid").flexOptions({url: 'index.php?act=pendtreat&op=get_all_xml&'+$("#formSearch").serialize(),query:'',qtype:''}).flexReload();
        });
        // 高级搜索重置
        $('#ncreset').click(function(){
            $("#flexigrid").flexOptions({url: 'index.php?act=pendtreat&op=get_all_xml'}).flexReload();
            $("#formSearch")[0].reset();
        });
        $("#flexigrid").flexigrid({
            url: 'index.php?act=pendtreat&op=get_all_xml',
            colModel : [
                {display: '操作', name : 'operation', width : 150, sortable : false, align: 'center', className: 'handle'},
                {display: '分销订单号', name : 'fx_order_id', width : 200, sortable : true, align: 'center'},
                {display: '分销来源', name : 'source', width : 200, sortable : true, align: 'center'},
   ...
   function fg_operate(name, grid) {
    ...
    //点击退款退货按钮
    function  refund_change(id,type){
        var word="你确定要将此单改为退货状态？";
        if(type=="1"){
            word="你确定要将此单改为退款状态？";
        }
        if(confirm(word)){
            $.ajax({
                type: "GET",
                dataType: "json",
                url: "index.php?act=pendtreat&op=change_state",
                data: {"refund_id":id,"state_type":type},
