...
<style>
    .pc-special-layout .module-list div {
        background: none;
    }
</style>
<div class="page">
    <!-- 页面导航 -->
    <div class="fixed-bar">
        <div class="item-title"><?php if ($output['special_id'] > 0) { ?><a class="back"
                                                                            href="index.php?act=pc_special&op=special_list"
                                                                            title="返回列表"><i
                        class="fa fa-arrow-circle-o-left"></i></a><?php } ?>
            <div class="subject">
                <h3>模板设置 - 编辑PC专题 - <?php echo $output['special_info']['special_title']; ?></h3>
    ...
    <div class="explanation" id="explanation">
        ...
        <ul>
            ...
            <li>新增加的版块内容默认为<strong>“禁用”</strong>状态，编辑内容并<strong>“启用”</strong>该块后将在PC端即时显示。</li>
        </ul>
    </div>

    <!-- 列表 -->
    <div class="pc-special-layout">
        <div class="pc-item-box">
                    ...
                    <?php foreach ($output['list'] as $key => $value) { ?>
                        <div nctype="special_item" class="special-item <?php echo $value['item_type']; ?> <?php echo $value['usable_class']; ?>" data-item-id="<?php echo $value['item_id']; ?>">
                            ...
                            <div id="item_edit_content">
                                <?php require(BASE_RESOURCE_PATH . '/pc_special/widgets/' . $value['item_type'] . '/' . $value['item_template'] . '.tpl.php'); ?>
                            </div>
                            <div class="handle">
                                <a nctype="btn_move_up" href="javascript:;">
                                    <i class="fa fa-arrow-up "></i>上移
                                </a>
                                <a nctype="btn_move_down" href="javascript:;">
                                    <i class="fa fa-arrow-down "></i>下移
                                </a>
                                <a nctype="btn_usable" data-item-id="<?php echo $value['item_id']; ?>" href="javascript:;">
                                    <i class="fa fa-toggle-on "></i><?php echo $value['usable_text']; ?>
                                </a>
                                <a nctype="btn_edit_item" data-item-id="<?php echo $value['item_id']; ?>" href="javascript:;">
                                    <i class="fa fa-pencil-square-o"></i>编辑
                                </a>
                                <a nctype="btn_del_item" data-item-id="<?php echo $value['item_id']; ?>" href="javascript:;">
                                    <i class="fa fa-trash-o "></i>删除
                                </a>
                                <a nctype="btn_edit_navi" data-item-id="<?php echo $value['item_id']; ?>" href="javascript:;">
                                    <i class="fa fa-pencil-square-o"></i>设置导航
                                </a>
                            </div>
                        </div>
                    <?php } ?>
        ...
        <div class="module-list">
            <?php if (!empty($output['module_list']) && is_array($output['module_list'])) { ?>
                <?php foreach ($output['module_list'] as $key => $value) { ?>
                    <div class="module_common" style=" background: url(<?php echo $value['preview']; ?>) no-repeat">
                        <span><?php echo $value['desc']; ?></span> <a nctype="btn_add_item" class="add"
                                                                      href="javascript:;"
                                                                      data-module-type="<?php echo $value['name']; ?>">添加</a>
                    </div>
                <?php } ?>
            <?php } ?>
        </div>
...
<script type="text/javascript">
    ...
    var url_item_add = "<?php echo urlAdminShop('pc_special', 'special_item_add');?>";
    var url_item_del = "<?php echo urlAdminShop('pc_special', 'special_item_del');?>";
    var url_item_edit = "<?php echo urlAdminShop('pc_special', 'special_item_edit');?>";
    var url_navi_edit = "<?php echo urlAdminShop('pc_special', 'special_navi_edit');?>";
    $(document).ready(function () {
        ...
        //编辑导航
        $('#item_list').on('click', '[nctype="btn_edit_navi"]', function () {
            var item_id = $(this).attr('data-item-id');
            go(url_navi_edit + '&item_id=' + item_id);
        });
        ...
        var update_item_sort = function () {
            ...
            $.post("index.php?act=pc_special&op=update_item_sort", {
                special_id: special_id,
        ...
        $('#item_list').on('click', '[nctype="btn_usable"]', function () {
            ...
            $.post("index.php?act=pc_special&op=update_item_usable", {
                item_id: item_id,
                usable: usable,
                special_id: special_id
            }, function (data) {
