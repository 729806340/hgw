...
<div class="page">
  <div class="fixed-bar">
    <div class="item-title">
      ...
      <ul class="tab-base nc-row">
        <li><a class="<?php echo $output['current']==''?'current':'';?>" href="index.php?act=shequ_bill">社区团长结算管理</a></li>
        <li><a class="<?php echo $output['current']=='hgsw'?'current':'';?>" href="index.php?act=shequ_bill&gname=hgsw">汉购网商务审核</a></li>
      </ul>
    </div>
  </div>
  <!-- 操作说明 -->
  <div class="explanation" id="explanation">
    ...
    <ul>
      <li>账单计算公式：支付金额 = 佣金金额 - 退单扣除佣金金额</li>
      <li>账单处理流程为：系统出账 > 团长确认 > 汉购网商务审核 > 财务支付(完成结算) 4个环节，其中平台审核和财务支付需要平台介入，请予以关注</li>
    </ul>
  </div>
...
$(function(){
    ...
    $('#ncsubmit').click(function(){
        $("#flexigrid").flexOptions({url: 'index.php?act=shequ_bill&op=get_bill_xml&'+$("#formSearch").serialize(),query:'',qtype:''}).flexReload();
    });

    // 高级搜索重置
    $('#ncreset').click(function(){
        $("#flexigrid").flexOptions({url: 'index.php?act=shequ_bill&op=get_bill_xml'}).flexReload();
        $("#formSearch")[0].reset();
    });
    $("#flexigrid").flexigrid({
        url: 'index.php?act=shequ_bill&op=get_bill_xml&gname=<?php echo $output['current']?>',
        colModel : [
            ...
            {display: '团长名称', name : 'ob_store_name', width : 130, sortable : false, align: 'left'},
            ...
            {display: '团长ID', name : 'ob_store_id', width : 90, sortable : true, align: 'center'},
        ...
        searchitems : [
           ...
           {display: '团长名称', name : 'ob_store_name'}
        ],
        sortname: "ob_id",
        sortorder: "desc",
    ...
    setTimeout(function(){
        $('span[nc_type="inline_edit"]').inline_edit({act: 'shequ_bill', op: 'ajax'});
    },500);
