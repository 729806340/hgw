...
<script type="text/javascript">
    $(function(){
        ...
        $('#ncsubmit').click(function(){
            $("#flexigrid").flexOptions({url: 'index.php?act=stat_aftersale&op=get_commodity_xml&reason=1&'+$("#formSearch").serialize()}).flexReload();
        });
        // 高级搜索重置
        $('#ncreset').click(function(){
            $("#flexigrid").flexOptions({url: 'index.php?act=stat_aftersale&op=get_commodity_xml&reason=1'}).flexReload();
            $("#formSearch")[0].reset();
        });
        $("#flexigrid").flexigrid({
            url: "index.php?act=stat_aftersale&op=get_reason_xml&reason=1&goods_id=<?php echo $output['goods_id']; ?>&query_start_date=<?php echo $output['start_date']; ?>&query_end_date=<?php echo $output['end_date']; ?>",
            colModel : [
                {display: '商家名称', name : 'store_name', width : 200, sortable : true, align: 'left'},
                {display: '商品名称', name : 'goods_name', width : 200, sortable : true, align: 'left'},
                {display: '退款原因', name : 'reason_info', width : 150, sortable : true, align: 'center'},
                {display: '退款金额总计', name : 'refund_amount_total', width : 150, sortable : true, align: 'center'},
                {display: '售前', name : 'before_sale', width : 100, sortable : true, align: 'center'},
                {display: '售后', name : 'after_sale', width : 100, sortable : true, align: 'center'},
                {display: '总计', name : 'order_num', width : 100, sortable : true, align: 'center'},
                {display: '售后占比', name : 'refund_rate', width : 150, sortable : true, align: 'center'},
            ],
            sortname: "refund_rate",
            sortorder: "desc",
            title: "从<?php echo $output['start_date']; ?>到<?php echo $output['end_date']; ?>的商品退款原因列表",
            rp: 20,
            rpOptions: [20],
        });
    });
