...
<div class="page">
  ...
  <div class="explanation" id="explanation">
    ...
    <ul>
      ...
      <li>统计某品类子分类在不同时间段的下单金额、下单商品数、下单量，为分析品类销量提供依据</li>
    </ul>
  </div>
  <li><a href="javascript:export_data()" class="ncap-btn">导出数据</a></li>
            ...
            <dd>
              <label>
                <select name="search_type" id="search_type" class="class-select">
                  ...
                  <option value="slice" <?php echo $output['search_arr']['search_type']=='slice'?'selected':''; ?>>按时间段统计</option>
                </select>
              </label>

            </dd>
            <dd id="searchtype_slice" style="display:none;">
              <label>
                <input readonly id="query_start_date" placeholder="请选择起始时间" name=query_start_date value="<?php echo @date('Y-m-d',$output['search_arr']['query_start_date']);?>" type="text" class="s-input-txt" />
              </label>
              <label>
                <input readonly id="query_end_date" placeholder="请选择结束时间" name="query_end_date" value="<?php echo @date('Y-m-d',$output['search_arr']['query_end_date']);?>" type="text" class="s-input-txt" />
              </label>
            </dd>
...
function export_data(){
  ...
  window.location.href = 'index.php?act=stat_industry&op=scale_list&source=<?php echo $output['source'];?>&stattype=export&t=<?php echo $output['searchtime'];?>&choose_gcid='+choose_gcid;
}

//加载统计数据
function getStatdata(type){
	var choose_gcid = $("#choose_gcid").val();
	$('#'+type+'_div').load('index.php?act=stat_industry&op=scale_list&source=<?php echo $output['source'];?>&stattype='+type+'&t=<?php echo $output['searchtime'];?>&choose_gcid='+choose_gcid);
}
</script>