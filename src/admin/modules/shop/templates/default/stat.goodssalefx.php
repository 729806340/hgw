        ...
        <div class="layout-box">
          ...
          <dl>
            ...
            <dl>
                ...
                <dd>
                    <label>
                        <select name="channel_name" id="channel" class="class-select">
                            <option value="" selected>请选择渠道（可选）</option>
                            <?php foreach($output['member_fenxiao'] as $k=>$v){?>
                            <option value="<?php echo $v['member_en_code'];?>" <?php echo $v['member_en_code']; ?>><?php echo $v['member_cn_code'];?></option>
                            <?php }?>
                        </select>
                    </label>
                </dd>
...
function update_flex(){
    $("#flexigrid").flexigrid({
        url: 'index.php?act=stat_goods&op=get_goods_xml&source=fenxiao&'+$("#formSearch").serialize(),
        colModel : [
            //{display: '操作', name : 'operation', width : 60, sortable : false, align: 'center', className: 'handle-s'},
            {display: '商品名称', name : 'goods_name', width : 180, sortable : false, align: 'center'},
            {display: '店铺名称', name : 'store_name',  width : 120, sortable : false, align: 'center'},
            {display: 'SPU', name : 'goods_commonid', width : 100, sortable : true, align: 'center'},
            {display: 'SKU', name : 'goods_id', width : 100, sortable : true, align: 'center'},
            {display: '下单商品件数', name : 'goodsnum',  width : 100, sortable : true, align: 'center'},
            {display: '下单单量', name : 'ordernum',  width : 100, sortable : true, align: 'center'},
            {display: '下单金额', name : 'goodsamount',  width : 100, sortable : true, align: 'center'},
            {display: '退单商品件数', name : 'refund_num',  width : 100, sortable : true, align: 'center'},
            {display: '退单单量', name : 'refund_count',  width : 100, sortable : true, align: 'center'},
            {display: '退单金额', name : 'refund_amount',  width : 100, sortable : true, align: 'center'},
            {display: '均单价', name : 'average_price',  width : 100, sortable : true, align: 'center'},
            {display: '均单量', name : 'average_order_num',  width : 100, sortable : true, align: 'center'},
            {display: '金额占比', name : 'money_point',  width : 100, sortable : true, align: 'center'},
            {display: '区域分析', name : 'district',  width : 120, sortable : true, align: 'center'},
        ],
        buttons : [
...
function fg_operation(name, bDiv){
    var stat_url = 'index.php?act=stat_goods&op=goods_sale_fenxiao';
