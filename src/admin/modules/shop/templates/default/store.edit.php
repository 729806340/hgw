...
<div class="page">
  <div class="fixed-bar">
    <div class="item-title"><a class="back" href="index.php?act=store&op=store" title="返回<?php echo $lang['manage'];?>列表"><i class="fa fa-arrow-circle-o-left"></i></a>
      <div class="subject">
        <h3><?php echo $lang['nc_store_manage'];?> - 编辑会员“<?php echo $output['store_array']['member_name'];?>”的店铺信息</h3>
  ...
  <h3>编辑店铺信息</h3>
    ...
    <form id="store_form" method="post">
      ...
      <div class="ncap-form-default">
        ...
        <dl class="row">
          ...
          <dl class="row">
              <dt class="tit">
                  <label>社区团购店铺类型</label>
              </dt>
              <dd class="opt">
                  <select  name="is_shequ_tuan">
                      <option <?php if($output['store_array']['is_shequ_tuan'] == 0){ ?>selected="selected"<?php } ?> value="0">物流</option>
                      <option <?php if($output['store_array']['is_shequ_tuan'] == 1){ ?>selected="selected"<?php } ?> value="1">自提</option>
                  </select>
                  <span class="err"></span>
                  <p class="notic"></p>
              </dd>
        ...
        <dl class="row" style="display: none;">
          ...
          <dd class="opt">
            ...
            <p class="notic"><?php if($output['store_array']['manage_type_new']&&$output['store_array']['manage_type_validate']>time()){
              ...
              echo "店铺类型将于{$manageTypeValidate}变更为：{$manageTypeNew}";
            }else{?>店铺类型修改将在下一结算周期生效，新店铺立即生效！<?php } ?></p>
          </dd>
        </dl>
        <dl class="row">
          <dt class="tit">
    ...
    <form id="joinin_form" enctype="multipart/form-data" method="post" action="index.php?act=store&op=edit_save_joinin" style="display:none;">
      ...
      <input type="hidden" name="member_name" value="<?php echo $output['member_name'];?>" />
      <input type="hidden" name="store_id" value="<?php echo $output['store_array']['store_id'];?>" />
