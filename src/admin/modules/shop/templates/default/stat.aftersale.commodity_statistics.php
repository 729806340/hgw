...
<script type="text/javascript">
    $(function(){
        ...
        $('#ncsubmit').click(function(){
            $("#flexigrid").flexOptions({url: 'index.php?act=stat_aftersale&op=get_commodity_xml&'+$("#formSearch").serialize()}).flexReload();
        });
        // 高级搜索重置
        $('#ncreset').click(function(){
            $("#flexigrid").flexOptions({url: 'index.php?act=stat_aftersale&op=get_commodity_xml'}).flexReload();
            $("#formSearch")[0].reset();
        });
        $("#flexigrid").flexigrid({
            url: 'index.php?act=stat_aftersale&op=get_commodity_xml',
            colModel : [
                ...
                {display: '售前退单总数', name : 'before_sale_num', width : 150, sortable : true, align: 'center'},
                {display: '售后退单总数', name : 'after_sale_num', width : 150, sortable : true, align: 'center'},
                {display: '订单总数', name : 'order_total', width : 150, sortable : true, align: 'center'},
                {display: '售前退单率', name : 'before_sale_rate', width : 150, sortable : true, align: 'center'},
                {display: '售后退单率', name : 'after_sale_rate', width : 150, sortable : true, align: 'center'},
                // {display: '退单率', name : 'refund_rate', width : 150, sortable : false, align: 'center'},
            ],
            sortname: "before_sale_rate",
            sortorder: "desc",
            title: '售后率最高的商品列表',
            rp: 20,
            rpOptions: [20],
