  ...
  <form id="form_store_verify" action="index.php?act=store&op=store_joinin_verify" method="post">
    ...
    <table border="0" cellpadding="0" cellspacing="0" class="store-joinin">
      ...
      <tbody>
          ...
          <th class="w150">商家类型：</th>
          	<td>
				<select id="manage_type" name="manage_type">
					<?php foreach ($output['manage_type'] as $k => $v) {?>
					<option value="<?php echo $k;?>" <?php echo $output['joinin_detail']['manage_type']==$k?"selected=''":""; ?> ><?php echo $v;?></option>
					<?php }?>
				</select>
			</td>
        </tr>
		...
		<input type="hidden" name="joinin_message" value="&nbsp;">
      </tbody>
    </table>
    <?php if(in_array(intval($output['joinin_detail']['joinin_state']), array(STORE_JOIN_STATE_NEW, STORE_JOIN_STATE_PAY)) && $output['show_button']==1) { ?>
    <div id="validation_message" style="color:red;display:none;"></div>
    <div class="bottom"><a id="btn_pass" class="ncap-btn-big ncap-btn-green mr10" href="JavaScript:void(0);">保存</a> </div>
    <?php } ?>
  </form>
</div>
