...
<div class="page">
  <div class="fixed-bar">
    <div class="item-title"><a class="back" href="index.php?act=new_goods_class&op=goods_class&gc_id=<?php echo $output['class_array']['gc_parent_id']?>" title="返回商品分类列表"><i class="fa fa-arrow-circle-o-left"></i></a>
      <div class="subject">
  ...
  <form id="goods_class_form" name="goodsClassForm" enctype="multipart/form-data" method="post">
    ...
    <div class="ncap-form-default">
      ...
      <dl class="row">
        <dt class="tit">
          <label><em>*</em>溢价比例</label>
        </dt>
        <dd class="opt">
          <input id="rise_rate" class="w60" type="text" value="<?php echo $output['class_array']['rise_rate'];?>" name="rise_rate">
          <i>%</i> <span class="err"></span>
          <p class="notic mb10">溢价比例必须为0-100的整数。</p>
          <label for="t_rise_rate">
            <input id="t_rise_rate" class="checkbox" type="checkbox" value="1" name="t_rise_rate">
            关联到子分类</label>
          <p class="notic">勾选关联到子分类后，该分类下的子分类溢价比利也将按此继承设定。</p>
        </dd>
      </dl>
      <dl class="row">
        <dt class="tit">
      ...
      <dl class="row">
        ...
        <dl class="row">
            <dt class="tit"><label>APP端显示</label></dt>
            <dd class="opt">

            <?php if(empty($output['class_array']['app_show'])):?>
        ...
        <dl class="row">
            <dt class="tit">APP端显示图</dt>
            <dd class="opt">
                <div class="input-file-show"><span class="show"><a class="nyroModal" rel="gal" href="<?php echo UPLOAD_SITE_URL.'/'.(ATTACH_COMMON.DS.$output['class_array']['app_img']);?>"> <i class="fa fa-picture-o" onMouseOver="toolTip('<img src=<?php echo UPLOAD_SITE_URL.'/'.(ATTACH_COMMON.DS.$output['class_array']['app_img']);?>>')" onMouseOut="toolTip()"/></i> </a></span><span class="type-file-box">
	...
	$('#goods_class_form').validate({
        ...
        rules : {
            gc_name : {
                required : true,
                remote   : {
                url :'index.php?act=new_goods_class&op=ajax&branch=check_class_name',
                type:'get',
                data:{
                    gc_name : function(){
                        return $('#gc_name').val();
