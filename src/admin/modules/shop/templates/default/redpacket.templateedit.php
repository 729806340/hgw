    ...
    <div class="ncap-form-default">
      ...
      <dl class="row">
        ...
        <dl class="row">
            ...
            <dd class="opt">
                <select name="rpacket_t_goods_type" id="rpacket_t_goods_type" readonly="readonly">
                ...
                <select name="rpt_range" id="rpt-range" readonly="readonly">
                    ...
                    <option value="3" <?php echo  $output['t_info']['rpacket_t_range']==3?'selected':'' ?>>选定商品分类适用</option>
                </select>
                <span class="err"></span>
        ...
        <dl class="row" id="range-select-row"  <?php echo  $output['t_info']['rpacket_t_range']==0?'style="display: none;"':'' ?>>
            ...
            <dd class="opt">
                <!-- 编辑红包时,发现POST数组里没有rpt_range字段,所以加上用来做其他逻辑判断 -->
                <input type="hidden" name="rpt_range"value="<?php echo $output['t_info']['rpacket_t_range']; ?>"/>
                <?php
                    $range = $output['t_info']['rpacket_t_range'];
                    $goodsList = $output['t_info']['goodsList'];
                    $goodsClassList = $output['t_info']['goodsClassList'];
                    // dump($output);
                    // dump($goodsClassList);
                    if($output['t_info']['ableedit']==false){
                ?>
                        <p><?php if ($range<3) { echo "已经选定商品"; }else{ echo "已经选定分类"; } ?>：</p>
                        <ul class="sku-list">
                                <?php
                                    ...
                                    if(is_array($goodsClassList))
                                        foreach ($goodsClassList as $gc){
                                            echo '<li><span>'.$gc['menu'].'</span></li>';
                                        }
                                ?>
                        </ul>
              ...
              <?php }else{ ?>
                        ...
                        <div class="rpt-range">
                            <div>
                                <input type="hidden" name="rpt_skus" id="rpt-skus" value="<?php echo $output['t_info']['rpacket_t_skus']?$output['t_info']['rpacket_t_skus']:$output['t_info']['rpacket_t_classes']; ?>"/>
                            </div>
                            <div id="goods-selected-list">
                                <div id="list-head">
                                    <span><?php if ($range<3) { echo "已选商品"; }else{ echo "已选分类"; } ?></span>
                                    <a href="JavaScript:cleanSku();" class="ncap-btn ncap-btn-red" id="select-goods-reset"><?php if ($range<3) { echo '清空已选商品'; }else{ echo '清空已选分类'; } ?></a>
                                </div>
                                <ul id="list-body">
                                    <?php
                                        if(is_array($goodsList))
                                        ...
                                        if(is_array($goodsClassList))
                                            foreach ($goodsClassList as $gc){
                                            echo '<li id="selected-sku-'.$gc['gc_id'].'" data-key="'.$gc['gc_id'].'"><span>'.$gc['menu'].'</span></li>';
                                            }
                                    ?>
                                </ul>
                            </div>
                            <div id="goods-select-box">
                                <div style="margin: 10px 0 0;">
                                    <input type="text" placeholder="<?php if ($range<3) { echo '搜索商品名称'; }else{ echo '搜索分类名称'; } ?>" value="" name="goods_name" id="goods_name" maxlength="20" class="input-txt">
                                    <a id="goods-search" href="JavaScript:void(0);" class="ncap-btn mr5">搜索</a>
                                </div>
                                <div id="goods-search-result"><?php if ($range<3) { echo '请搜索商品...'; }else{ echo '请搜索分类...'; } ?></div>
                            </div>
                        </div>
              <?php } ?>
