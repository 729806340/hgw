...
function update_flex(){
	//加载统计列表
    $("#glist").flexigrid({
        url: 'index.php?act=stat_channel&op=get_channel_xml&'+$("#formSearch").serialize(),
        colModel : [
            {display: '渠道id', name : 'channel_id', width : 60, sortable : false, align: 'center', className: 'handle-s'},
            {display: '渠道名称', name : 'channel', width : 120, sortable : false, align: 'center'},
...
$(function () {
	...
	$('#ncsubmit').click(function(){
	    $("#glist").flexOptions({url: 'index.php?act=stat_channel&op=get_channel_xml&'+$("#formSearch").serialize(),query:'',qtype:''}).flexReload();
...
function fg_operation(name, bDiv){
    //window.location.href = 'index.php?act=stat_channel&op=exportCvs&'+$("#formSearch").serialize();
    var stat_url = 'index.php?act=stat_channel&op=exportCvs';
