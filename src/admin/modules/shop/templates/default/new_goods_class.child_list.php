...
<div class="page">
  <div class="fixed-bar">
    <div class="item-title"><a class="back" href="index.php?act=new_goods_class&op=goods_class&gc_id=<?php echo $output['parent_id']?>" title="返回上级分类列表"><i class="fa fa-arrow-circle-o-left"></i></a>
      <div class="subject">
    ...
    <table class="flex-table">
      <thead>
        <tr>
          ...
          <th width="80" align="center">进项税</th>
          <th width="80" align="center">销项税</th>
          <th></th>
        </tr>
      </thead>
      <tbody>
        ...
        <tr data-id="<?php echo $v['gc_id'];?>">
          ...
          <td class="handle">
            ...
            <ul>
              <li><a href="index.php?act=new_goods_class&op=goods_class_edit&gc_id=<?php echo $v['gc_id'];?>">编辑分类信息</a></li>
              <?php if ($output['deep'] != 3) {?>
              <li><a href="index.php?act=new_goods_class&op=goods_class_add&gc_parent_id=<?php echo $v['gc_id'];?>">新增下级分类</a></li>
              <?php }?>
              <?php if ($v['have_child'] == 1) {?>
              <li><a href="index.php?act=new_goods_class&op=goods_class&gc_id=<?php echo $v['gc_id'];?>">查看下级分类</a></li>
              <?php }?>
              <?php if ($v['have_child'] != 1) { ?>
              <li><a href="index.php?act=new_goods_class&op=tax_edit&gc_id=<?php echo $v['gc_id']; ?>">类目税率修改</a></li>
              <?php } ?>
            </ul>
            </span></td>
          ...
          <td><?php echo $v['tax_input']=='200.000'?'未设置':sprintf("%.2f", $v['tax_input'])."%";?></td>
          <td><?php echo $v['tax_output']=='200.000'?'未设置':sprintf("%.2f", $v['tax_output'])."%";?></td>
          <td></td>
        </tr>
        <?php } ?>
        <?php }else { ?>
...
function fg_operation(name, bDiv) {
    if (name == 'add') {
        window.location.href = 'index.php?act=new_goods_class&op=goods_class_add&gc_parent_id=<?php echo $output['gc_id']?>';
    }
    if (name == 'del') {
...
function fg_del(ids) {
    ...
    if(confirm('删除后将不能恢复，确认删除这项吗？')){
        $.getJSON('index.php?act=new_goods_class&op=goods_class_del', {id:id}, function(data){
            if (data.state) {
                location.reload();
            } else {
                showError(data.msg)
