    ...
    <div class="ncap-form-default">
      <dl class="row">
        ...
        <dd class="opt tabswitch-target"> 总数：
          <input type="text" class="txt" name="total" style="width:60px;" />
           <span class="err"></span>
      ...
      <dl class="row">
        <dt class="tit">
          <label><em>*</em>卡号密码</label>
        </dt>
        <dd class="opt">
          <input class="input-txt" type="text" name="pwd" />
          <span class="err"></span>
...
$("#form_add").validate({
    ...
    messages : {
        ...
        batchflag : {
            required:'<i class="fa fa-exclamation-circle"></i>批次标识不能为空',
            maxlength: '<i class="fa fa-exclamation-circle"></i>请输入20字之内的批次标识'
        },
        pwd:{
            required: '<i class="fa fa-exclamation-circle"></i>请输入充值卡密码',
            minlength : '<i class="fa fa-exclamation-circle"></i>密码长度不可以小于4个字符',
        },
    }
});
});
