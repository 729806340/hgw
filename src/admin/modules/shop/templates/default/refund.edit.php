...
<div class="page">
  ...
  <form id="post_form" method="post" action="index.php?act=refund&op=edit&refund_id=<?php echo $output['refund']['refund_id']; ?>">
    ...
    <div class="ncap-form-default">
      ...
      <?php } ?>
      <?php if ($output['order']['pay_refund_amount'] > 0) { ?>
      <dl class="row">
        <dt class="tit">在线支付金额</dt>
        <dd class="opt"><?php echo ncPriceFormat($output['order']['pay_refund_amount']);?></dd>
      </dl>
      <?php } ?>
      <div class="title">
        <h3>平台退款审核</h3>
	  ...
	  <dl class="row">
        ...
        <dd class="opt">
			<?php echo $output['refund']['refund_way'] == 'offline' ? '线下支付' : orderPaymentName($output['refund']['refund_way']); ?>
        </dd>
      </dl>
	  <dl class="row">
        <dt class="tit">
        ...
        <dd class="opt">
			<?php echo $output['refund']['refund_name']; ?>
        </dd>
      </dl>
	  <dl class="row">
        <dt class="tit">
        ...
        <dd class="opt">
			<?php echo $output['refund']['refund_account']; ?>
        </dd>
      </dl>
        <dl class="row">
            <dt class="tit">
            ...
            <dd class="opt">
                <input type="checkbox" id="send_payment" name="send_payment" value="1">是
            </dd>
        </dl>
      <dl class="row" id="refund_message" style="display: none">
        <dt class="tit">
          <label><em>*</em><?php echo $lang['refund_message'];?></label>
        </dt>
        <dd class="opt">
          <textarea id="admin_message" name="admin_message" class="tarea"><?php echo $output['refund']['admin_message']; ?></textarea>
          <span class="err"></span> 
...
$(function(){
	...
	$('#send_payment').click(function (data) {
        var send_payment = $('#send_payment').is(':checked');
        if (send_payment) {
            $('#refund_message').show();
        } else {
            $('#refund_message').hide();
