...
class new_goods_classControl extends SystemControl{
    private $links = array(
        array('url'=>'act=new_goods_class&op=goods_class','lang'=>'nc_manage'),
        array('url'=>'act=new_goods_class&op=goods_class_import','lang'=>'goods_class_index_import')
    );
    ...
    public function goods_classOp(){
        ...
        $model_class = Model('new_goods_class');
        ...
        if ($gc_id > 0){
            ...
            Tpl::showpage('new_goods_class.child_list');
        } else {
            Tpl::output('class_list',$class_list);
            Tpl::output('top_link',$this->sublink($this->links,'goods_class'));
            Tpl::setDirquna('shop');//网 店 运 维shop wwi.com
            Tpl::showpage('new_goods_class.index');
        }
    }
    ...
    public function goods_class_addOp(){
        ...
        if (chksubmit()){
            ...
            if ($error != ''){
                ...
                if ($result){
                    $url = array(
                        array(
                            'url'=>'index.php?act=new_goods_class&op=goods_class_add&gc_parent_id='.$_POST['gc_parent_id'],
                            'msg'=>$lang['goods_class_add_again'],
                        ),
                        array(
                            'url'=>'index.php?act=new_goods_class&op=goods_class',
		...
		Tpl::setDirquna('shop');//网 店 运 维shop wwi.com
        Tpl::showpage('new_goods_class.add');
    }
    ...
    public function goods_class_editOp(){
        ...
        if (chksubmit()){
            ...
            $obj_validate->validateparam = array(
                ...
                array("input"=>$_POST["rise_rate"], "require"=>"true", 'validator'=>'range','max'=>100,'min'=>0, "message"=>'请填写正确的溢价比例'),
                array("input"=>$_POST["gc_sort"], "require"=>"true", 'validator'=>'Number', "message"=>$lang['goods_class_add_sort_int']),
            );
            $error = $obj_validate->validate();
            if ($error != ''){
            ...
            $update_array['rise_rate']    = intval($_POST['rise_rate']);
            ...
            $update_array['app_show']      = intval($_POST['app_show']);
            ...
            //add by ljq  添加佣金变更审核
//             $goods_class = $this->__updateClassCommis($_POST['gc_id'], $_POST['commis_rate']);
            ...
            if ($_POST['t_commis_rate'] == '1' || $_POST['t_rise_rate'] == '1' || $_POST['t_associated'] == '1' || $_POST['t_gc_virtual'] == '1' || $_POST['t_show_type'] == '1') {
...
//                    if(($gc_id>0) && $gc_id != $_POST['gc_id']){
//                         $this->__updateClassCommis($gc_id, $_POST['commis_rate']);
//                    }
//                }
                // 更新该分类下子分类的所有分佣比例
                if ($_POST['t_rise_rate'] == '1') {
                    $update['rise_rate']  = $update_array['rise_rate'];
                }
                // 更新该分类下子分类的所有类型
            ...
            $url = array(
                array(
                    'url'=>'index.php?act=new_goods_class&op=goods_class_edit&gc_id='.intval($_POST['gc_id']),
        ...
        Tpl::output('parent_list',$parent_list);
        // 一级分类列表
        $gc_list = Model('new_goods_class')->getGoodsClassListByParentId(0);
        Tpl::output('gc_list', $gc_list);

        Tpl::output('type_list',$t_list);
        Tpl::output('class_array',$class_array);
        $this->links[] = array('url'=>'act=new_goods_class&op=goods_class_edit','lang'=>'nc_edit');
        Tpl::output('top_link',$this->sublink($this->links,'goods_class_edit'));
						//网 店 运 维shop wwi.com
		Tpl::setDirquna('shop');
        Tpl::showpage('new_goods_class.edit');
    }
    
    private function __updateClassCommis( $id , $commis_rate){
        $goods_class = Model('new_goods_class')->getGoodsClassInfoById($id);
        $condition = array();
        $condition['type'] = 4;
    ...
    public function goods_class_importOp(){
        ...
        if (chksubmit()){
            ...
            if (!empty($_FILES['csv']) && !empty($_FILES['csv']['name']) && $file_type == 'csv'){
                ...
                showMessage($lang['nc_common_op_succ'],'index.php?act=new_goods_class&op=goods_class');
            }else {
        ...
        Tpl::output('top_link',$this->sublink($this->links,'goods_class_import'));
						//网 店 运 维shop wwi.com
		Tpl::setDirquna('shop');
        Tpl::showpage('new_goods_class.import');
    }
    ...
    public function goods_class_delOp(){
        if ($_GET['id'] != ''){
            //删除分类
            Model('new_goods_class')->delGoodsClassByGcIdString($_GET['id']);
    ...
    public function tag_resetOp(){
        ...
        if(!$return){
            showMessage($lang['goods_class_reset_tag_fail'], 'index.php?act=new_goods_class&op=tag');
        }
        ...
        if(is_array($goods_class) and !empty($goods_class)) {
            ...
            if($return){
                ...
                showMessage($lang['nc_common_op_succ'], 'index.php?act=new_goods_class&op=tag');
            }else{
                $this->log(L('nc_reset').'tag',0);
                showMessage($lang['nc_common_op_fail'], 'index.php?act=new_goods_class&op=tag');
            }
        }else{
            $this->log(L('nc_reset').'tag',0);
            showMessage($lang['goods_class_reset_tag_fail_no_class'], 'index.php?act=new_goods_class&op=tag');
        }
    }
    ...
    public function tag_updateOp(){
        ...
        if(is_array($tag_list) && !empty($tag_list)){
            ...
            $this->log(L('nc_update').'tag',0);
            showMessage($lang['goods_class_update_tag_fail_no_class'], 'index.php?act=new_goods_class&op=tag');
        }
    ...
    public function nav_editOp() {
        ...
        $model_goods = Model('new_goods_class');
    ...
    function tax_editOp()
    {
    	...
    	if(chksubmit()){
    		...
    		$tax_input = trim($_POST['tax_input']);
    		$tax_output = trim($_POST['tax_output']);
    		$type = 51;//流程type
    		
    		if($tax_input == $class_array['tax_input'] && $tax_output == $class_array['tax_output']){
    			showMessage('该类目进项税，销项税都没有变更');
    		}
    		
    		$whereWorkflow = array();
    		$whereWorkflow['type'] = $type;
    		$whereWorkflow['model']='goods_class';
    		$whereWorkflow['model_id'] = $gc_id;
    		$whereWorkflow['status'] = array('neq' ,'1');
    		$workflowInfo = Model('workflow')->getWorkflowInfo($whereWorkflow , 'id');
    		
    		if($workflowInfo['id']>0){
    			showMessage('此商品已提交了修改类目税率审核,不能重复提交');
    		}

    		$new_value = array('tax_input'=>$tax_input,'tax_output'=>$tax_output);
    		$old_value = array('tax_input'=>$class_array['tax_input'],'tax_output'=>$class_array['tax_output']);
    		$data = array();          //审核表数组
    		$data['title'] = '编号：'.$gc_id."类目（{$class_array['gc_name']}）税率变更审核流程";
    		$data['type'] = $type;   //类目税率变更
    		$data['model'] = 'goods_class';
    		$data['model_id'] = $gc_id;
    	...
    	$tax_arr = array();
    	$tax_arr['200.000'] = '未设定';
    	$tax_arr['17.000'] = '17.000';
    	$tax_arr['16.000'] = '16.000';
    	$tax_arr['13.000'] = '13.000';
    	$tax_arr['10.000'] = '10.000';
    	$tax_arr['6.000'] = '6.000';
    	$tax_arr['0.000'] = '0.000';
    	Tpl::output('tax_arr',$tax_arr);
    	Tpl::output('class_array',$class_array);
    	Tpl::setDirquna('shop');
    	Tpl::showpage('goods_class.tax_edit');
    }
}
