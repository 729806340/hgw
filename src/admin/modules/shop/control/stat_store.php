...
class stat_storeControl extends SystemControl{
    ...
    public function get_storesales_xmlOp(){
        ...
        $countnum = intval($count_arr['countnum']);
        //列表字段
        $field = " store_id,min(store_name) as store_name,SUM(order_amount) as orderamount, COUNT(*) as ordernum, COUNT(DISTINCT buyer_phone) as membernum";
        //排序
        ...
        if (!empty($list) && is_array($list)){
            ...
            //昨天的下单人员
            $front_order=Model('stat_order')->where($condition)->field('store_id,buyer_phone')->limit(false)->select();
            //今天的下单数量
            $current_order=Model('stat_order')->where($where)->field('store_id,buyer_phone')->limit(false)->select();

            $last_buyer=implode(',',array_unique(array_column($front_order,'buyer_phone')));
            $where['buyer_phone']=array("in",$last_buyer);
            $last_order=Model('stat_order')->where($where)->field('store_id,buyer_phone')->limit(false)->select();
            $last_buyer_order=array_intersect($last_order,$current_order);
            $arr=array_count_values(array_column($last_buyer_order,'store_id'));

            $front_order=array_unique($front_order,SORT_REGULAR);
            $last_statistics=array_count_values(array_column($front_order,'store_id'));
            foreach ($list as $k => $v){
                ...
                $out_array['reporate']=count(array_unique($arr[$v['store_id']]['same_member']));
                $same_member=$arr[$v['store_id']];
                $total_member=$last_statistics[$v['store_id']];
                /*复购率=这周重复购买的上周会员/上周起下单的会员量*/
                $reporate=$same_member/$total_member;
                $out_array['reporate']=number_format($reporate*100,3)."%";
                /*增长率=这周购买人数-上周购买人数/上周起下单的会员量*/
                if($total_member==0){
                    $growthrate=0;
                }else{
                    $growthrate=(intval($v['membernum'])-$total_member)/$total_member;
                }
                $out_array['$growthrate']=number_format($growthrate*100,3)."%";
                $statlist[$v['store_id']] = $out_array;
            }
        }
        $data = array();
