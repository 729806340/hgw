...
//            echo $model_order->getLastSql();exit();
    ...
    public function service_feeOp()
    {
        ...
        Tpl::output('order_info',$order_info);
        Tpl::setDirquna('shop');/*网 店 运 维shop wwi.com*/
        Tpl::showpage('order.service_fee','null_layout');
    }

    public function add_service_feeOp()
    {
        $order_sn = $_POST['order_sn'];
        $technical_fee = $_POST['technical_fee'];
        $financial_fee = $_POST['financial_fee'];
        /** @var orderModel $orderModel */
        $orderModel = Model('order');
        $order = $orderModel->getOrderInfo(array('order_sn'=>$order_sn));
        if(empty($order)){
        ...
        $res = $orderModel->where(array('order_id'=>$order['order_id']))->update($data);
        if($res) exit(json_encode(array('state'=>true,'msg'=>'服务费更新成功')));
        exit(json_encode(array('state'=>false,'msg'=>'服务费更新失败')));
    }
    private function _importServiceFee($filePath)
    {
        if(!is_file($filePath)){
        ...
        /* 检查文件格式是否正确 */
        $title = $data[0];
        if($title[0]!='技术服务费'||$title[1]!='金融服务费'){
        ...
        foreach ($data as $k=>$v){
            ...
            $item['fx_order_id'] = $v[2];
            $item['technical_fee'] = $v[0];
            $item['financial_fee'] = $v[1];
            $item['order_sn'] = $v[3];
            $tasks[] = $item;
        }
        $orderList = $orderModel->getOrderList(array('fx_order_id'=>array('in',array_column($tasks,'fx_order_id'))));
        $orderList = array_under_reset($orderList,'fx_order_id',2);
        /** @var memberModel $memberModel */
        $memberModel = Model('member');
        foreach ($tasks as $k=>$task){
            if(true === ($result = $this->_addServiceFee($task,$orderList)))
                $res['success']+=1;
            else{
                $res['fail'][] = $task['row'];
     ...
     */
    private function _addServiceFee($task,$orderList)
    {
        /** @var orderModel $orderModel */
        $orderModel = Model('order');
        if(empty($task['order_sn'])){
            if(!isset($orderList[$task['fx_order_id']])) {
                return '分销订单不存在';
            }
            if(count($orderList[$task['fx_order_id']])>1){
                return '分销订单被拆分，请填写汉购网订单号';
            }
            $order_sn = $orderList[$task['fx_order_id']][0]['order_sn'];
        }else{
            $order = $orderModel->getOrderInfo(array('order_sn'=>$task['order_sn']));
            if(empty($order)){
                return '订单不存在';
            }
        ...
        $data = array(
            'technical_fee'=>$task['technical_fee'],
            'financial_fee'=>$task['financial_fee'],
            'send_sap'=>0,
        );
        $res = $orderModel->where(array('order_sn'=>$order_sn))->update($data);
        if($res) return true;
        return '服务费更新失败';
    }

    public function import_service_feeOp(){
        set_time_limit(0);
        ...
        if (move_uploaded_file($file['tmp_name'], $fileName)) {
            $result = $this->_importServiceFee($fileName);
    ...
    private function createExcel($data = array()){
        ...
        $store_list = $storeModel->getStoreList(array('store_id'=>array('in',$storeIds)));
        ...
        foreach ((array)$data as $k=>$order_info){
            $store = $store_list[$order_info['store_id']];
            $order_info['state_desc'] = orderState($order_info);
            $list = array();
            $orderGoods = array();
            $list['order_sn'] = $order_info['order_sn'].str_replace(array(1,2,3), array(null,' [预定]','[门店自提]'), $order_info['order_type']);
            ...
            $list['store_id'] = $order_info['store_id'];
            $list['store_name'] = $order_info['store_name'];
            $list['manage_type'] =$this->get_manage_type($order_info['manage_type'],$store);
            $list['buyer_id'] = $order_info['buyer_id'];
            $list['buyer_name'] = $order_info['buyer_name'];
    ...
    public function  get_manage_type($type,$store){
     $data=array(
         'co_construct'=>'共建',
         'platform'=>'平台',
         'hango'=>'自营',
     ...
     $res = !empty($data[$type]) ? $data[$type]:"";
     if ($res=='共建'&&$store&&isset($store['invoice'])){
         if ($store['invoice']==0){
             $res .= '(不需要发票)';
         }
     }
    ...
    private function _get_condition(& $condition) {
        ...
        if(in_array($_GET['order_from'],array('1','2','3','4','6','7'))){
            $condition['order_from'] = $_GET['order_from'];
        }else{
            $condition['order_from'] = array('not in', array(8,9));//过滤社区团购自提的
        }
