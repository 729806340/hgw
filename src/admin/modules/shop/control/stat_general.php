...
class stat_generalControl extends SystemControl{
    private $links = array(
    	...
    	array('url'=>'act=stat_general&op=fenxiao','lang'=>'stat_fenxiaoindex')
    );
    ...
    public function generalOp(){
        ...
        $stat_order = $model->getoneByStatorder($where, $field);//die($model->getLastSql());
    ...
    public function fenxiaoOp(){
    	...
    	$field = ' COUNT(*) as ordernum, SUM(order_amount) as orderamount, AVG(order_amount) as orderavg ';
    	$stat_order = $model->getoneByStatorder($where, $field);//die($model->getLastSql());
    	$statnew_arr['ordernum'] = ($t = $stat_order['ordernum'])?$t:0;
    	$statnew_arr['orderamount'] = ncPriceFormat(($t = $stat_order['orderamount'])?$t:(0));
    	//$statnew_arr['ordermembernum'] = ($t = $stat_order['ordermembernum'])?$t:0;
    	...
    	Tpl::output('statnew_arr',$statnew_arr);
    	Tpl::output('stat_time',$stat_time);
    	Tpl::output('top_link',$this->sublink($this->links, 'fenxiao'));
    	Tpl::setDirquna('shop');/*网 店 运 维shop wwi.com*/
    	Tpl::showpage('stat.fenxiao.index');
    }
}
