...
class stat_tradeControl extends SystemControl{
    private $links = array(
        ...
        array('url'=>'act=stat_trade&op=sale_new','text'=>'订单统计_店铺')
    );
    ...
    public function __construct(){
        ...
        //处理搜索时间
        if (in_array($_REQUEST['op'],array('sale_trend','get_sale_xml', 'get_sale_new_xml', 'get_plat_sale'))){
    ...
    public function saleOp(){
        ...
        if ($_GET['exporttype'] == 'excel'){
            ...
            $order_list = $model->statByStatorder($where, '', 0, 50000, 'order_id desc', '');
            //统计数据标题
            $statlist = array();
    ...
    public function sale_newOp(){
        ...
        if ($_GET['exporttype'] == 'excel'){
            ini_set('memory_limit','4G');
            $field = "sum(order_amount) as order_totals,store_id,store_name, count(order_id) as order_numbers, sum(refund_amount) as order_return_totals";
            $order = "order_totals desc";
            $list = $model->statByStatorder($where, $field, 0, 2000, $order, 'store_id');
            //统计数据标题
            $statlist = array();
            $statlist['headertitle'] = array('店铺id','店铺名称','订单金额','订单数量','退单金额');
            $statlist['data'] = $list;
            import('libraries.excel');
            $excel_obj = new Excel();
            $excel_data = array();
            ...
            foreach ((array)$statlist['data'] as $k => $v){
                ...
                $excel_data[$k+1][] = array('data'=>$v['store_name']);
                $excel_data[$k+1][] = array('data'=>$v['order_totals']);
                $excel_data[$k+1][] = array('data'=>$v['order_numbers']);
                $excel_data[$k+1][] = array('data'=>$v['order_return_totals']);
            }
            $excel_data = $excel_obj->charset($excel_data,CHARSET);
            $excel_obj->addArray($excel_data);
            $excel_obj->addWorksheet($excel_obj->charset('店铺订单统计',CHARSET));
            $excel_obj->generateXML($excel_obj->charset('店铺订单统计',CHARSET).date('Y-m-d-H',time()));
            exit();
        } else {
            Tpl::output('top_link',$this->sublink($this->links, 'sale_new'));
            Tpl::setDirquna('shop');
            Tpl::showpage('stat.sale_new');
        }
    }
     ...
     * 输出店铺订单统计XML数据
     */
    public function get_sale_new_xmlOp(){
        /** @var statModel $model */
        $model = Model('stat');
        //默认统计当前数据
        if(!$this->search_arr['search_type']){
        ...
        $field = "sum(order_amount) as order_totals,store_id,store_name, count(order_id) as order_numbers, sum(refund_amount) as order_return_totals";
        $order = "order_totals desc";
        $list = $model->statByStatorder($where, $field, $page, 0, $order, 'store_id');
        $statlist = array();
        if (!empty($list) && is_array($list)){
            foreach ($list as $k => $v){
                $out_array['store_name'] = $v['store_name'];
                $out_array['order_totals'] = $v['order_totals'];
                $out_array['order_numbers'] = $v['order_numbers'];
                $out_array['order_return_totals'] = $v['order_return_totals'];
                $statlist[$v['store_id']] = $out_array;
            }
        }
        $data = array();
        $data['now_page'] = $model->shownowpage();
        $total_num = $model->statByStatorder($where, "count(distinct store_id) as total_num");
        $data['total_num'] = $total_num[0]['total_num'];
        $data['list'] = $statlist;
