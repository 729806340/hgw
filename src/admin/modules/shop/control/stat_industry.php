...
class stat_industryControl extends SystemControl{
    private $links = array(
    	...
    	array('url'=>'act=stat_industry&op=scale_fenxiao','lang'=>'stat_industryscale_fx'),
    	array('url'=>'act=stat_industry&op=rank_fenxiao','lang'=>'stat_industryrank_fx'),
    	array('url'=>'act=stat_industry&op=price_fenxiao','lang'=>'stat_industryprice_fx'),
    	array('url'=>'act=stat_industry&op=general_fenxiao','lang'=>'stat_industrygeneral_fx'),
    );
    ...
    public function __construct(){
        ...
        //处理搜索时间
        $op = array('scale','rank','price','scale_fenxiao','rank_fenxiao','price_fenxiao');
     ...
     * 品类规模
     */
    public function scaleOp($source=""){
        if( isset($_GET['source']) && $_GET['source'] == 'fenxiao' ) {
            $source = "fenxiao" ;
        }
//        log::selflog("scale_end：".$source, 'stat') ;
        ...
        $active = $source == 'fenxiao' ? 'scale_fenxiao' : 'scale' ;
        ...
        if($gc_childarr){
            ...
            switch ($_GET['stattype']){
               ...
               case 'goodsnum':
                   ...
                   break;
                case 'export':
                    $field .= ',COUNT(DISTINCT order_id) as ordernum,SUM(goods_num) as goodsnum,SUM(goods_pay_price) as orderamount';
                    $orderby = 'goodsnum desc';
                    break;
               default:
            ...
            if($_GET['stattype'] == 'export'){
                $export_data = array();
                $export_data[0] = array('商品分类','下单金额','下单商品数','下单量');
                $k = 0;
                foreach ((array)$goods_list as $v){
                    $statgc_id = intval($v['statgc_id']);
                    if (in_array($statgc_id,array_keys($gc_childarr))){
                        $gc_name = strval($gc_childarr[$v['statgc_id']]['gc_name']);
                    } else {
                        $gc_name = '其他';
                    }
                    $export_data[$k + 1][] = $gc_name;
                    $export_data[$k + 1][] = $v['orderamount'];
                    $export_data[$k + 1][] = $v['goodsnum'];
                    $export_data[$k + 1][] = $v['ordernum'];
                    $k++;
                }
                $csv = new Csv();
                $export_data = $csv->charset($export_data, CHARSET, 'gbk');
                $csv->filename = 'stat_industry';
    ...
    public function rankOp($source = ""){
        ...
        $active = $source == 'fenxiao' ? 'rank_fenxiao' : 'rank' ;
        Tpl::output('top_link',$this->sublink($this->links, $active));
		Tpl::setDirquna('shop');/*网 店 运 维shop wwi.com*/
     ...
     * 价格分布
     */
    public function priceOp($source=""){
        if(!$this->search_arr['search_type']){
            $this->search_arr['search_type'] = 'day';
        }
        $model = Model('stat');
        ...
        if ($pricerange_arr){
            ...
            $goodsnum_stat_arr['title'] = '品类价格下单商品数';
            $goodsnum_stat_arr['yAxis'] = '';

            $orderamount_stat_arr['legend']['enabled'] = false;
            $orderamount_stat_arr['title'] = '品类价格下单金额';
        ...
        $active = $source == 'fenxiao' ? 'price_fenxiao' : 'price';
        Tpl::output('top_link',$this->sublink($this->links, $active));
		Tpl::setDirquna('shop');/*网 店 运 维shop wwi.com*/
     ...
     * 销售统计
     */
    public function generalOp($source=""){
    	$active = $source == 'fenxiao' ? 'general_fenxiao' : "general" ;
    	Tpl::output('source',$source);
        Tpl::output('top_link',$this->sublink($this->links, $active));
		Tpl::setDirquna('shop');/*网 店 运 维shop wwi.com*/
    ...
    public function general_listOp(){
        ...
        if (!empty($gc_childarr) && is_array($gc_childarr)){
            ...
            if ($this->search_arr['exporttype'] == 'excel'){
                ...
                $excel_obj->addWorksheet($excel_obj->charset('品类概况总览',CHARSET));
                $excel_obj->generateXML($excel_obj->charset('品类概况总览',CHARSET).date('Y-m-d-H',time()));
                exit();
            }
        }
    }

    public function export_dataOp(){
     ...
     * 分销品类规模
     */
    public function scale_fenxiaoOp(){
    	$this -> scaleOp('fenxiao') ;
    }
    /**
     * 分销品类排行
     */
    public function rank_fenxiaoOp(){
    	$this -> rankOp('fenxiao') ;
    }
    /**
     * 分销价格分布
     */
    public function price_fenxiaoOp(){
    	$this -> priceOp('fenxiao') ;
    }
    /**
     * 分销销售统计
     */
    public function general_fenxiaoOp(){
    	$this -> generalOp('fenxiao') ;
    }
}
