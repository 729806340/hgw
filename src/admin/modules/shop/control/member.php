...
class memberControl extends SystemControl{
     ...
     * 设置支付密码
     */
    public function paypwd_editOp(){
        $lang   = Language::getLangContent();
        $model_member = Model('member');
        if ($this->admin_info['gname'] != '客服组') {
            showMessage('只有客服组才可以操作！');
        }
        /**
         * 保存
         */
        if (chksubmit()){
            ...
            $pay_passwd = $_POST['pay_passwd'];
            $re_pay_passwd = $_POST['re_pay_passwd'];
            if (strlen($pay_passwd) > 20 || strlen($pay_passwd) < 6) {
                showMessage('支付密码长度有误');
            }
            if ($pay_passwd != $re_pay_passwd) {
                showMessage('两次密码不一致');
            }

            $update_array['member_paypwd'] = md5($pay_passwd);
            $update = $model_member->editMember(array('member_id' => $member_id), $update_array);
            if ($update){
                $this->log(L('nc_edit,member_index_name').'[ID:'.$_POST['member_id'].']',1);
                showMessage('支付密码修改成功', 'index.php?act=member&op=index');
            }else {
                showMessage($lang['member_edit_fail']);
            }
        }
        ...
        Tpl::output('member_array',$member_array);
		Tpl::setDirquna('shop');/*网 店 运 维shop wwi.com*/
        Tpl::showpage('member.paypwd_edit');
    }
    ...
    public function get_xmlOp() {
        ...
        foreach ($member_list as $value) {
            $param = array();
            $param['operation'] = "<a class='btn blue' href='index.php?act=member&op=member_edit&member_id=" . $value['member_id'] . "'><i class='fa fa-pencil-square-o'></i>编辑</a><a class='btn green' href='index.php?act=member&op=paypwd_edit&member_id=" . $value['member_id'] . "'><i class='fa fa-pencil-square-o'></i>设置支付密码</a>";
