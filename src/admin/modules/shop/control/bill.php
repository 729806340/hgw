...
class billControl extends SystemControl
{
    ...
    public function show_billOp()
    {
        ...
        } elseif ($_GET['query_type'] == 'log') {
            $sub_tpl_name = 'bill_order_bill.show.log_list';
        } elseif ($_GET['query_type'] == 'attachment') {
            $sub_tpl_name = 'bill_order_bill.show.attachment';
        } elseif ($_GET['query_type'] == 'pay_log') {
            $sub_tpl_name = 'bill_order_bill.show.pay_log';
        } elseif ($_GET['query_type'] == 'jdy') {
            //筋斗云采购单
            $sub_tpl_name = 'bill_order_bill.show.jdy_list';
            Tpl::output('entry_state', $jdy_entryModel::$states);
        } else{
            //订单列表
        ...
        $storeInfo = $store_model->getStoreInfo(array('store_id'=>$bill_info['ob_store_id']));
    ...
    public function edit_rptOp()
    {
        ...
        Tpl::showpage('bill_order_bill.edit.rpt', 'null_layout');
    }
    ...
    public function edit_orderOp()
    {
        ...
        Tpl::output('order_info', $order_info);
        Tpl::setDirquna('shop');/*网 店 运 维shop wwi.com*/
        Tpl::showpage('bill_order_bill.edit', 'null_layout');
    }
    ...
    public function edit_recOp()
    {
        ...
        $billInfo = $billModel->getOrderBillInfo($map);
        if (!empty($billInfo)) {
...
WHERE a.order_id = {$orderGoods['order_id']};");
    ...
    public function edit_refundOp()
    {
        ...
        Tpl::setDirquna('shop');/*网 店 运 维shop wwi.com*/
        Tpl::showpage('bill_order_bill.refund.edit', 'null_layout');
    }
    ...
    public function get_bill_info_xmlOp()
    {
        ...
        if ($_GET['query_type'] == 'refund') {
            ...
            foreach ($refund_list as $refund_info) {
                ...
                if($refund_info['purchase_sap']!==0){
                    $list['operation'].="<a target=\"_blank\" class=\"btn green\" href=\"index.php?act=bill&op=reset_sap&type=refund&id={$refund_info['refund_id']}&ob_id={$ob_id}\"><i class=\"fa fa-list-alt\"></i>重推</a>";
                }
                ...
                $list['check_result'] = $check_result_array[$refund_info['check_result']];
                ...
                $list['mj_bill'] = is_null($orderGoods['mj_bill']) ? '0.00' : $orderGoods['mj_bill'];
                $list['xs_bill'] = is_null($orderGoods['xs_bill']) ? '0.00' : $orderGoods['xs_bill'];
                ...
                $list['send_sap'] = $refund_info['send_sap'];
                $list['sap_return_credit'] = $refund_info['sap_return_credit'];
                $list['purchase_sap'] = $refund_info['purchase_sap'];

                $list['errInf'] = $refund_info['errInf'];
                $list['check_status'] = $refund_info['check_status'];
        ...
        elseif ($_GET['query_type'] == 'cost') {
            ...
            if ($_POST['query'] != '' && in_array($_POST['qtype'], array('check_status'))) {
                $cost_condition[$_POST['qtype']] = array('eq', "{$_POST['query']}");
            }
            ...
            foreach ($store_cost_list as $store_cost_info) {
                $list = array();
                if($store_cost_info['purchase_sap']!==0){
                    $list['operation'].="<a target=\"_blank\" class=\"btn green\" href=\"index.php?act=bill&op=reset_sap&type=cost&id={$store_cost_info['cost_id']}&ob_id={$ob_id}\"><i class=\"fa fa-list-alt\"></i>重推</a>";
                }
                ...
                $list['fx_order_id'] = $store_cost_info['fx_order_id'];
                $list['send_sap'] = $store_cost_info['send_sap'];
                $list['purchase_sap'] = $store_cost_info['purchase_sap'];
                $list['check_result'] = $check_result_array[$store_cost_info['check_result']];
                $list['errInf'] = $store_cost_info['errInf'];
                $list['check_status'] = $store_cost_info['check_status'];
        ...
        elseif ($_GET['query_type'] == 'log') {
            ...
            if (preg_match('/^\d{8,20}$/', $_POST['query'])) {
                $condition['order_sn'] = array('like', "%{$_POST['query']}%");
            }
            if ($_GET['order_sn'] != '' && preg_match('/^\d{8,20}$/', $_GET['order_sn'])) {
                $condition['order_sn'] = array('like', "%{$_GET['order_sn']}%");
            }
            if ($_GET['log_role'] == '1') {
                $condition['log_role'] = 0;
            }
            $log_list = $model_order->table('bill_log')->where($condition)->page(20)->order('log_id desc')->select();
            $data = array();
            $data['now_page'] = $model_order->shownowpage();
            $data['total_num'] = $model_order->gettotalnum();
            foreach ($log_list as $log_info) {
                $list = array();
                if ($bill_info['ob_state'] == BILL_STATE_HANGO && $this->admin_info['gname'] == '汉购网商务') {
                    $list['operation'] .= "<a class=\"btn green\" href=\"javascript:;\" onclick=\"ajax_form('edit-bill-data','调整结算信息','index.php?act=bill&op=edit_order&ob_id={$ob_id}&order_id={$log_info['order_id']}',1020,0);\"><i class=\"fa fa-gavel\"></i>调整</a>";
                } else if ($bill_info['ob_state'] == BILL_STATE_FIRE_PHONIX && $this->admin_info['gname'] == '公司商务') {
                    $list['operation'] .= "<a class=\"btn green\" href=\"javascript:;\" onclick=\"ajax_form('edit-bill-data','调整结算信息','index.php?act=bill&op=edit_order&ob_id={$ob_id}&order_id={$log_info['order_id']}',1020,0);\"><i class=\"fa fa-gavel\"></i>调整</a>";
                } else {
                    $list['operation'] = '无操作';
                }
                $list['log_time'] = date('Y-m-d H:i', $log_info['log_time']);
                $list['log_msg'] = $log_info['log_msg'];
                $list['log_role'] = $log_info['log_role'] == 1 ? '客服' : '商家';
                $list['log_user'] = $log_info['log_user'];
                $data['list'][$log_info['log_id']] = $list;
            }
            exit(Tpl::flexigridXML($data));
        }
        elseif ($_GET['query_type'] == 'attachment') {
            ...
            foreach ($log_list as $log_info) {
                $list = array();
                $list['operation'] .= '<a class="btn green" href="' . $log_info['new_value'] . '"><i class="fa fa-gavel"></i>下载</a>';
                $list['log_id'] = $log_info['log_id'];
                $list['log_time'] = date('Y-m-d H:i', $log_info['log_time']);
                $list['log_msg'] = $log_info['log_msg'];
                $list['log_role'] = $log_info['log_role'] == 1 ? '客服' : '商家';
                $list['log_user'] = $log_info['log_user'];
...
//            }
            ...
            $bill_log_list = $model_order_bill_log->table('order_bill_log')->where($condition)->page(20)->order('obl_id desc')->select();
            ...
            foreach ($bill_log_list as $log_info) {
                ...
                $list['supplier_name'] = $log_info['supplier_name'];
        ...
        elseif ($_GET['query_type'] == 'jdy'){
            //精斗云列表
            $model_order = model('order');
            $model_jdy_entry = model("jdy_entry");
            if ($_POST['query'] != '' && in_array($_POST['qtype'], array('goods_name', 'jdy_supplier_name','jdy_supplier_number'))) {
                $condition[$_POST['qtype']] = array('like', "%{$_POST['query']}%");
            }
            if ($_POST['query'] != '' && in_array($_POST['qtype'], array('goods_id'))) {
                $condition[$_POST['qtype']] = array('eq', "{$_POST['query']}");
            }
            if ($_GET['goods_name'] != '') {
                $condition['goods_name'] = array('like', "%{$_GET['goods_name']}%");
            }
            if ($_GET['jdy_supplier_name'] != '') {
                $condition['jdy_supplier_name'] = array('like', "%{$_GET['jdy_supplier_name']}%");
            }
            if ($_GET['jdy_supplier_number'] != '') {
                $condition['jdy_supplier_number'] = array('like', "%{$_GET['jdy_supplier_number']}%");
            }
            if ($_GET['goods_id'] != '') {
                $condition['goods_id'] = array('eq', "{$_GET['goods_id']}");
            }
            if ($_GET['state'] != '') {
                $condition['state'] = array('eq', "{$_GET['state']}");
            }
            $condition['ob_id'] = $bill_info['ob_id'];
            ...
            $entry_list = $model_jdy_entry->getList($condition, $_POST['rp'], $order,'*','');
            ...
            foreach ($entry_list as $entry) {
                $list = array();
                $list['operation'] = "";
                if ($entry['state'] == jdy_entryModel::STATE_PUSH_ERROR)
                {
                    $list['operation'] .= "<a class=\"btn green\" href=\"javascript:;\" onclick=\"ajax_form('edit-bill-data','调整结算信息','index.php?c=bill&a=edit_order&ob_id={$ob_id}&entry_id={$entry['id']}',1020,0);\"><i class=\"fa fa-gavel\"></i>重推</a>";
                }
                //映射失败,且数据未被重置才展示重映按钮[0,10]
                if ($entry['map_time'] != 0 && in_array($entry['state'], array(jdy_entryModel::STATE_MAPPING,jdy_entryModel::STATE_CREATED,))) {
                    $list['operation'] .= "<a class=\"btn green\" href=\"javascript:;\" onclick=\"remap({$entry['id']});\"><i class=\"fa fa-gavel\"></i>重新映射</a>";
                }
                if ($bill_info['jdy_push_time'] != 0 && $entry['state'] == jdy_entryModel::STATE_MAPPED) {
                    $list['operation'] .= "<a class=\"btn green\" href=\"javascript:;\" onclick=\"push({$bill_info['ob_id']});\"><i class=\"fa fa-gavel\"></i>推送订单</a>";
                }
                if ($bill_info['jdy_refund_time'] != 0 && $entry['refund_state'] == jdy_entryModel::REFUND_STATE_CREATED && $entry['refund'] >0 && $entry['jdy_supplier_number'] != '') {
                    $list['operation'] .= "<a class=\"btn green\" href=\"javascript:;\" onclick=\"push_refund({$bill_info['ob_id']});\"><i class=\"fa fa-gavel\"></i>推送退单</a>";
                }
                $list['jdy_supplier_number'] = $entry['jdy_supplier_number'];
                $list['jdy_supplier_name'] = $entry['jdy_supplier_name'];
                $list['goods_id'] = $entry['goods_id'];
                $list['goods_name'] = $entry['goods_name'];
                $list['amount'] = ncPriceFormat($entry['amount']);
                $list['cost'] = ncPriceFormat($entry['cost']);
                $list['price'] = ncPriceFormat($entry['price']);
                $list['goods_num'] = $entry['goods_num'];
                $list['goods_id'] = $entry['goods_id'];
                $list['goods_id'] = $entry['goods_id'];
                $list['state'] = jdy_entryModel::getStateName($entry['state']);
                $list['rpt_bill'] = ncPriceFormat($entry['rpt_bill']);
                $list['mj_bill'] = ncPriceFormat($entry['mj_bill']);
                $list['xs_bill'] = ncPriceFormat($entry['xs_bill']);
                $list['refund'] = ncPriceFormat($entry['refund']);
                $data['list'][$entry['id']] = $list;
            }
            exit(Tpl::flexigridXML($data));
        }
        else {
            ...
            $purchase_sap_status = array(
                '0' => '未进sap',
                '1' => '未进b1',
                '2' => '已进b1'
            );

            $send_sap_sap_status = array(
                '0' => '未进sap',
                '1' => '未进b1',
                '2' => '已进b1'
            );
            ...
            foreach ($order_list as $order_info) {
                ...
                if($order_info['purchase_sap']!==0){
                    $list['operation'].="<a target=\"_blank\" class=\"btn green\" href=\"index.php?act=bill&op=reset_sap&type=order&id={$order_info['order_id']}&ob_id={$ob_id}\"><i class=\"fa fa-list-alt\"></i>重推</a>";
                }
                $list['order_sn'] = $order_info['order_sn'];
                $list['order_amount'] = ncPriceFormat($order_info['order_amount']);
                $list['order_amount'] = $bill_info['ob_store_manage_type'] == 'platform' ? ncPriceFormat($order_info['order_amount']) : ncPriceFormat($order_info['cost_amount']);
                $list['check_result'] = $check_result_array[$order_info['check_result']];
                $list['shipping_fee'] = ncPriceFormat($order_info['shipping_fee']);
                $list['commis_amount'] = $commis;
                $list['rpt_bill'] = ncPriceFormat($order_info['rpt_bill']);
                $list['mj_bill'] = is_null($order_info['mj_bill']) ? '0.00' : $order_info['mj_bill'];
                $list['xs_bill'] = is_null($order_info['xs_bill']) ? '0.00' : $order_info['xs_bill'];
                $list['add_time'] = date('Y-m-d', $order_info['add_time']);
                ...
                $list['store_name'] = $order_info['store_name'];
                $list['store_id'] = $order_info['store_id'];
                $list['send_sap'] = $send_sap_sap_status[$order_info['send_sap']];
                $list['purchase_sap'] = $purchase_sap_status[$order_info['purchase_sap']];
                $list['errInf'] = $order_info['errInf'];
                $list['check_status'] = $order_info['check_status'];
                $data['list'][$order_info['order_id']] = $list;
            }
            exit(Tpl::flexigridXML($data));
        }
    ...
    public function add_attachmentOp()
    {
        ...
        if (chksubmit()) {
            ...
            if ($addRes)
                showMessage('操作完成', 'index.php?act=bill&op=show_bill&query_type=attachment&ob_id=' . $ob_id, 'html', 'succ');
            else
                showMessage('附件添加失败', 'index.php?act=bill&op=show_bill&query_type=attachment&ob_id=' . $ob_id, 'html', 'error');
        ...
        Tpl::output('ob_id', $ob_id);
        Tpl::setDirquna('shop');/*网 店 运 维shop wwi.com*/
        Tpl::showpage('bill_order_bill.upload', 'null_layout');
    }
    ...
    public function batch_editOp()
    {
        ...
        Tpl::output('ob_id', $ob_id);
        Tpl::setDirquna('shop');
        Tpl::showpage('bill_order_bill.batch_edit', 'null_layout');
    }
...
WHERE a.order_id = {$orderGoods['order_id']};");
    ...
    public function approve_hangoOp()
    {
        ...
        if ($bill->calcRealBill($billInfo) && $model_bill->editOrderBill(array('ob_state' => BILL_STATE_CEO), $condition)) {
...
//        $condition['ob_state'] = BILL_STATE_SYSTEM_CHECK;
    ...
    public function export_billOp()
    {
        ...
        $export_data = array();
        $export_data[0] = array('账单编号', '开始日期', '结束日期', '订单金额', '运费', '佣金金额', '退款金额', '退还佣金', '店铺费用', '本期应结', '出账日期', '账单状态', '店铺', '店铺ID','毛利','毛利率','备注');
        $ob_order_totals = 0;
    ...
    public function export_costOp()
    {
        ...
        if (!is_numeric($_GET['curpage'])) {
            ...
            if ($count > self::EXPORT_SIZE) {   //显示下载链接
                ...
                Tpl::output('list', $array);
                Tpl::output('murl', 'index.php?act=bill&op=show_bill&query_type=cost&ob_id=' . $ob_id);
                //网 店 运 维shop wwi.com
                Tpl::setDirquna('shop');
                Tpl::showpage('export.excel');
                exit();
    ...
    public function export_modifyOp()
    {
        ...
        if (!is_numeric($_GET['curpage'])) {
            ...
            if ($count > self::EXPORT_SIZE) {   //显示下载链接
                ...
                Tpl::output('list', $array);
                Tpl::output('murl', 'index.php?act=bill&op=show_bill&query_type=modify&ob_id=' . $ob_id);
                //网 店 运 维shop wwi.com
                Tpl::setDirquna('shop');
                Tpl::showpage('export.excel');
                exit();
    ...
    public function get_statis_xmlOp()
    {
        ...
        foreach ($bill_list as $bill_info) {
            $list = array();
            $list['operation'] = "<a target=\"_blank\" class=\"btn green\" href=\"index.php?act=bill&op=show_statis&os_month={$bill_info['os_month']}\"><i class=\"fa fa-list-alt\"></i>查看</a>";
    ...
    public function get_bill_xmlOp()
    {
        ...
        foreach ($bill_list as $bill_info) {
            ...
            if (in_array($bill_info['ob_state'], array(2, 3, 10, 11, 12, 13))) {
                $list['operation'] = "<a target=\"_blank\" class=\"btn orange\" href=\"index.php?act=bill&op=show_bill&ob_id={$bill_info['ob_id']}\"><i class=\"fa fa-gavel\"></i>处理</a>";
            } else {
                $list['operation'] = "<a target=\"_blank\" class=\"btn green\" href=\"index.php?act=bill&op=show_bill&ob_id={$bill_info['ob_id']}\"><i class=\"fa fa-list-alt\"></i>查看</a>";
            }
//            $list['operation']='<a target=\"_blank\" class=\"btn green\" href=\"index.php?act=bill&op=show_bill&ob_id={$bill_info[\'ob_id\']}\"><i class=\"fa fa-list-alt\"></i>查看</a>';
            ...
            $list['jdy_state'] = $model_bill->getJdyState($bill_info['jdy_state']);
            $list['jdy_push_time'] = $bill_info['jdy_push_time'] ? date('Y-m-d', $bill_info['jdy_push_time']) : '';
    ...
    public function jdy_pushOp() {
        $billModel = Model("bill");
        $billInfo = $billModel->getOrderBillInfo(array('ob_id'=>$_GET['id']));
        if (empty($billInfo)){
    ...
    private function getPlatformAmount($bill_info){
        $payable_amount_html = $gross_profit_html = '';
        if($bill_info['ob_store_manage_type']=='platform') {
            $payable_amount_html .= ncPriceFormat($bill_info['ob_result_totals']).
                " = ". ncPriceFormat($bill_info['ob_order_totals'])." (订单金额) - "
                .ncPriceFormat($bill_info['ob_commis_totals'])."(佣金金额) - "
                .ncPriceFormat($bill_info['ob_order_return_totals'])."(退单金额) + "
                .ncPriceFormat($bill_info['ob_commis_return_totals'])."(退还佣金) - "
                .ncPriceFormat($bill_info['ob_store_cost_totals'])."(店铺促销费用)";

            if (floatval($bill_info['ob_order_book_totals']) > 0) {
                $payable_amount_html .= "+ " . ncPriceFormat($bill_info['ob_order_book_totals']) . "(未退定金金额)";
            }
            $payable_amount_html .= "+ (".ncPriceFormat($bill_info['ob_rpt_amount']).") (平台红包)"
                ."- ( ".ncPriceFormat($bill_info['ob_rf_rpt_amount']).") (全部退款时应扣除的平台红包)";
        }else{
            $payable_amount_html = ncPriceFormat($bill_info['ob_result_totals'])." = "
                .ncPriceFormat($bill_info['ob_order_totals'])." (订单成本金额)"
                ."- ".ncPriceFormat($bill_info['ob_order_return_totals'])." (退单成本金额)"
                ."- ".ncPriceFormat($bill_info['ob_store_cost_totals'])." (店铺促销费用)";
            if (floatval($bill_info['ob_order_book_totals']) > 0) {
                $payable_amount_html .= "+ ".ncPriceFormat($bill_info['ob_order_book_totals'])." (未退定金金额)";
            }
        ...
        if($bill_info['ob_sales']>0){
            $gross_profit_html .= ncPriceFormat($bill_info['ob_sales']-$bill_info['ob_result_totals']) . ' = ';
            $gross_profit_html .= ncPriceFormat($bill_info['ob_sales']). '(订单销售额) - ';
            $gross_profit_html .= ncPriceFormat($bill_info['ob_result_totals']). '(账单应付金额) &nbsp; 【毛利率：';
            $gross_profit_html .= ncPriceFormat(($bill_info['ob_sales']-$bill_info['ob_result_totals'])*100/$bill_info['ob_sales']) . '% = ';
            $gross_profit_html .= ncPriceFormat($bill_info['ob_sales']-$bill_info['ob_result_totals']) . '(订单毛利) / ';
            $gross_profit_html .= ncPriceFormat($bill_info['ob_sales']). '(订单销售额) 】';
        }else{
            $gross_profit_html .= '毛利未计算';
        }
        return ['payable_amount'=>$payable_amount_html,'gross_profit'=>$gross_profit_html];
    }
}
