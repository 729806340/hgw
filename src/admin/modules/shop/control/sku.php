...
class skuControl extends SystemControl{
//    private $links = array(
...
//    );
    ...
    public function cost_historyOp(){
        ...
        $goodsList = $goodsModel->getGoodsList(array('goods_commonid'=>$goods_commid));
    ...
    public function cost_editOp()
    {
        ...
        $goodsList = $goodsModel->getGoodsList(array('goods_commonid'=>$_GET['goods_commonid']));
        if(chksubmit()){
    ...
    public function get_xmlOp() {
        ...
        if ($_GET['goods_id'] != '') {
            $condition['goods_id'] = array('like', '%' . $_GET['goods_id'] . '%');
        }
        if ($_GET['store_name'] != '') {
...
//        }
        ...
        if ($_GET['is_pyramid_goods'] != '') {
            ...
            $retail_goods_ids = array();
            if (!empty($retail_goods_list)) {
                $retail_goods_ids = array_column($retail_goods_list, 'retail_goods_id');
            }
            if ($_GET['is_pyramid_goods'] == 1 && !empty($retail_goods_ids)) {
                $condition['goods_id'] = array('in', $retail_goods_ids);
            }
            if ($_GET['is_pyramid_goods'] == 2 && !empty($retail_goods_ids)) {
                $condition['goods_id'] = array('not in', $retail_goods_ids);
            }
        }
        ...
        $order = '';
        $param = array('goods_id','goods_commonid', 'goods_name', 'goods_price', 'goods_state', 'goods_verify', 'goods_image', 'goods_jingle', 'gc_id'
        ...
        $condition['is_show_manage'] = 1;//SKU管理过滤掉is_show=1条件
        $goods_list = $model_goods->getGoodsList($condition, '*','',$order,15,$page);
        ...
        foreach ($goods_list as $value) {
            ...
            $operation .= "<li><a href='javascript:void(0);' onclick=\"fg_show(" . $value['goods_id'] . ",{$is_show_state})\">{$is_show_button}</a></li>";
...
//            $operation .= "<li><a href='javascript:void(0)' onclick=\"fg_sku('" . $value['goods_commonid'] . "')\">查看商品SKU</a></li>";
            ...
            $param['tax_input'] = $value['tax_input'];
            $param['tax_output'] = $value['tax_output'];
            $param['goods_cost_new'] = $value['goods_cost_new']?ncPriceFormat($value['goods_cost_new']):'--';
...
//        }
        ...
        if (!is_numeric($_GET['curpage'])){
            ...
            if ($count > self::EXPORT_SIZE ){   //显示下载链接
                ...
                Tpl::output('murl','index.php?act=sku&op=index');
								//网 店 运 维shop wwi.com
		Tpl::setDirquna('shop');
        ...
        $goods_list = $model_goods->getGoodsList($condition, '*', null, $order, $limit);
    ...
    private function createCsv($goods_list) {
        ...
        $data = array();
        $retail_goods_list = array();
        if (!empty($goods_list)) {
            /** @var retail_goodsModel $retail_goodsModel */
            $retail_goodsModel = Model('retail_goods');
            $retail_goods_list = $retail_goodsModel->getRetailGoodsList(array('retail_goods_id' => array('in', array_column($goods_list, 'goods_id'))));
...
//            $param['goods_url'] = urlShop('goods','index',array('goods_id'=>$storage_array[$value['goods_commonid']]['goods_id']));
             ...
             if ($_GET['is_pyramid_goods'] != '') {
                 $param['retail_one_return'] = $retail_goods_list[$value['goods_id']]['retail_one_return'];
                 $param['retail_two_return'] = $retail_goods_list[$value['goods_id']]['retail_two_return'];
                 $param['retail_three_return'] = $retail_goods_list[$value['goods_id']]['retail_three_return'];
             }
...
//                'goods_url' => '商品链接'
        ...
        if ($_GET['is_pyramid_goods'] != '') {
            $header['retail_one_return'] = '一级分销佣金';
            $header['retail_two_return'] = '二级分销佣金';
            $header['retail_three_return'] = '三级分销佣金';
        }
