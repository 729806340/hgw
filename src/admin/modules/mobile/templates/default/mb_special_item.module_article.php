...
<div class="explanation" id="explanation">
  ...
  <ul>
    ...
    <li>鼠标移动到已有文章上，会出现删除按钮可以对文章进行删除</li>
...
<div class="index_block goods-list">
  ...
  <div class="title">
    ...
    <div class="title">
        <?php if($item_edit_flag) { ?>
            <h5>样式：(请填写约定样式名称)</h5>
            <input id="home1_style" type="text" class="txt w200" name="item_data[style]" value="<?php echo $item_data['style'];?>">
        <?php } else { ?>
            <span><?php echo $item_data['style'];?></span>
        <?php } ?>
    </div>
    <div class="title">
        <?php if($item_edit_flag) { ?>
            <h5>图标：(请填写图标URL或者名称)</h5>
            <input id="home1_lcurl" type="text" class="txt w200" name="item_data[lcurl]" value="<?php echo $item_data['lcurl'];?>">
        <?php } else { ?>
  ...
  <div nctype="item_content" class="content">
    ...
    <div nctype="item_image" class="item">
      <div class="goods-pic"><img nctype="goods_image" src="<?php echo $item_value['article_image'];?>" alt=""></div>
      <div class="goods-name" nctype="goods_name"><?php echo $item_value['article_title'];?></div>
      <div class="goods-price" nctype="goods_price"><?php echo $item_value['article_author'];?></div>
      <?php if($item_edit_flag) { ?>
      <input nctype="goods_id" name="item_data[item][]" type="hidden" value="<?php echo $item_value['article_id'];?>">
...
<script type="text/javascript">
    $(document).ready(function(){
        $('#btn_mb_special_goods_search').on('click', function() {
            var url = '<?php echo urlAdminMobile('mb_special', 'article_list');?>';
