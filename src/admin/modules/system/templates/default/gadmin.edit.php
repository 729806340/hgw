      ...
      <dl class="row">
        ...
        <dd class="opt nobg nopd nobd nobs">
          ...
          <div class="ncap-account-container">
			...
			<!-- action设置 -->
			<?php if( isset($val['subop']) && !empty($val['subop']) ) {?>
			<p>
			<h4>子操作</h4>
			<ul class="ncap-account-container-list"><li>&nbsp;</li></ul>
			</p>
			<?php foreach ($val['subop'] as $index => $arr) {?>
			<p>
			<h4><?php echo $val['child'][$index];?></h4>
				<ul class="ncap-account-container-list">
					<?php foreach ($arr as $a) {?>
						<li>
						<input class="checkbox" type="checkbox" value="<?php echo $a['action'];?>" name="permission[action][<?php echo $key?>][<?php echo $index?>][]" <?php if (@in_array($a['action'], $output['ginfo']['limits']['action'][$key][$index])) { echo 'checked';}?> >
						<?php echo $a['name'];?>
						</li>
					<?php }?>
				</ul>
