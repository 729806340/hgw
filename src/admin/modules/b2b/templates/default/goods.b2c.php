...
$(function(){
    $("#flexigrid").flexigrid({
        url: 'index.php?act=goods&op=get_xml_b2c&type=<?php echo $output['type'];?>',
        colModel : [
    ...
    $('#ncsubmit').click(function(){
        $("#flexigrid").flexOptions({url: 'index.php?act=goods&op=get_xml_b2c&'+$("#formSearch").serialize(),query:'',qtype:''}).flexReload();
    });

    // 高级搜索重置
    $('#ncreset').click(function(){
        $("#flexigrid").flexOptions({url: 'index.php?act=goods&op=get_xml_b2c'}).flexReload();
        $("#formSearch")[0].reset();
    });
});
...
// 商品导入
function fg_import(id) {

        $.getJSON('index.php?act=goods_b2c&op=goods_import', {id:id}, function(data){
            if (data.state) {
                showSucc(data.msg)
            } else {
                showError(data.msg)
