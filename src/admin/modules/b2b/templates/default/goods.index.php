...
$(function(){
    ...
    $('#ncsubmit').click(function(){
        $("#flexigrid").flexOptions({url: 'index.php?act=supplier&op=get_xml&'+$("#formSearch").serialize(),query:'',qtype:''}).flexReload();
    });

    // 高级搜索重置
    $('#ncreset').click(function(){
        $("#flexigrid").flexOptions({url: 'index.php?act=supplier&op=get_xml'}).flexReload();
        $("#formSearch")[0].reset();
    });
});
...
//商品上架
function  fg_lonkup_on(ids){
        $.ajax({
            type: "GET",
            dataType: "json",
            url: "index.php?act=goods&op=goods_lockup_on",
            data: {"ids":ids},
