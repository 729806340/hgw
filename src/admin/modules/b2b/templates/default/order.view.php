...
<div class="ncap-order-flow">
    ...
    <div class="ncap-order-details">
      ...
      <div class="tabs-panels">
        ...
        <div class="addr-note" style="display: none;">
          <h4>购买/收货方信息</h4>
          <dl>
          ...
          <dl>
            ...
            <a href="javascript:;" onclick="ajax_form('edit_deliver', '修改收获地址', 'index.php?act=order&op=edit_address&order_id=<?php echo $output['order_info']['order_id'];?>', 640,0);">修改地址</a>
            </dd>
          </dl>
          <dl>
        ...
        <div class="goods-info">
          ...
          <table>
            ...
            <tbody>
              ...
              ?>
              <tr>
                <td class="w30"><div class="goods-thumb"><a href="<?php echo B2B_SITE_URL;?>/index.php?act=goods&goods_id=<?php echo $goods['goods_id'];?>" target="_blank"><img alt="<?php echo $lang['product_pic'];?>" src="<?php echo thumb($goods, 60);?>" /> </a></div></td>
                <td style="text-align: left;"><a href="<?php echo B2B_SITE_URL;?>/index.php?act=goods&goods_id=<?php echo $goods['goods_id'];?>" target="_blank"><?php echo $goods['goods_name'];?></a><span class="rec"><a style="display: none;" target="_blank" href="<?php echo urlShop('snapshot', 'index', array('rec_id' => $goods['rec_id']));?>">[交易快照]</a></span><br/><?php echo $goods['goods_spec'];?></td>
                ...
                <!--<td class="w60"><a class="nyroModal" href="index.php?act=order&op=show_goods_column&order_id=<?php /*echo $output['order_info']['order_id']; */?>&goods_id=<?php /*echo $goods['goods_id'];*/?>">更多</a></td>-->
                  ...
                  foreach ($output['order_info']['address_list'][$goods['rec_id']] as $address) { ?>
                      <tr>
                          <td colspan="1"></td>
                          <td colspan="1"><?php echo '<b>'.$address['buyer_name'].'</b> '.$address['address'] .' ('.$address['buyer_phone'].')'; ?></td>
                          <td><?php echo $address['rec_price']; ?></td>
                          <td>x <?php echo $address['rec_num']; ?></td>
                          <td>

                              <?php if($address['status']||!in_array($output['order_info']['order_state'],array(ORDER_STATE_PART_SEND,ORDER_STATE_PAY))){echo '已发货 ('.$address['logi_name'].':'.$address['logi_sn'].')';}else{?>
                                  <a href="javascript:;" onclick="ajax_form('address_send', '商品发货', 'index.php?act=order&op=address_send&address_id=<?php echo $address['id'];?>', 640,0);">发货</a>
                              <?php } ?></td>
                      </tr>
                  <?php } ?>
...
function fg_ajax(id,type) {
    ...
    $.ajax({
        type: "GET",
        dataType: "json",
        url: "index.php?act=order&op=address_send",
        data: "address_id="+id,
