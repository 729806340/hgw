      ...
      <dl class="row">
        <dd class="opt nopd nobd nobs">
          ...
          <div id="div_content_edit" class="tab-content" style="display:none;">
            <?php showEditor('special_content',$output['special_detail']['special_content']);?>
          </div>
        </dd>
      </dl>
    </div>
      ...
      <dl class="row">
        ...
        <dd class="opt">
          <table>
          	...
          	<td>
          		<select name="widgetpl_id" id="widgetpl_id">
          			<?php 
						if( !empty($output['tpl_list']) && is_array( $output['tpl_list'] ) ) {
          				foreach ($output['tpl_list'] as $id => $row)
          				{
          			?>
          	...
          	<tr><td>产品个数</td><td><input class="input-file" type="text" name="goods_num" id="goods_num" hidefocus="true" /></td></tr>
          	<tr>
	          	<td>产品ID</td>
	          	<td id="goods_input_list">
	          	</td>
          	</tr>
          	<tr><td colspan="2"><input id="submit_button" class="input-button" type="button" value="生成HTML" onClick="crtHtml()" /></td></tr>
          	<tr id="tr_new_html" style="display:none;"><td colspan="2"><textarea style="height:200px; width:400px;" id="new_html"></textarea></td></tr>
          </table>
          <p class="notic">拷贝代码到上方可视化编辑器中查看效果</p>
        </dd>
      </dl>
      <dl class="row">
...
$(document).ready(function(){
	$("#goods_num").blur(function(){
		var list = $("#goods_input_list");
		var nums = $("#goods_num").val(); 
		if( nums < 1 ) return false ; 
		list.empty();
		for(var i=1;i<=nums;i++ ){
			$('<input type="text" name="goods_id[]" class="goods_id">').appendTo(list);
		}
	}) ;
});
function crtHtml()
{
	...
	$(".goods_id").each(function(i){
		goods_ids[i] = this.value ;
	});
	var template_id = $("#widgetpl_id").val();
	$.post("index.php?act=cms_special&op=getGoodsTemplate",
	  {
