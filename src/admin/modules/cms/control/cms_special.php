...
//         showEditor('special_content',$special_detail['special_content']);
//         die;
        
        //模板列表
        $condition = array();
        $model = Model("cms_special_widgtpl") ;
        $tpl_list = $model -> where( $condition ) -> select();
...
Tpl::showpage('cms_special.detail', 'null_layout');
    ...
    public function getGoodsTemplateOp()
    {
    	header("Content-type: text/html; charset=utf-8");
    	$template_id = intval($_REQUEST['template_id']);
    	$model_special_tpl = Model('cms_special_widgtpl');
    	$condition = array();
    	$row = $model_special_tpl->where( array('id' => $template_id) )->find();
    	if( !$row ) die("没找到模板") ;
    	
    	$goods_ids = $_REQUEST['goods_ids'] ;
    	if( empty($goods_ids) ) die("没有商品ID") ;
    	$sql = "SELECT * FROM `shopwwi_goods` WHERE `goods_id` IN (". implode(",", $goods_ids) .")" ;
    	...
    	{
			$goods[$k] = $v ;
			$goods[$k]['goods_url'] = C("shop_site_url") . "/index.php?act=goods&op=index&goods_id=".$v['goods_id'] ;
			$goods[$k]['goods_pic'] = cthumb($v['goods_image'],360,$v['store_id']) ;
    	}
    	$template_content = htmlspecialchars_decode($row['content']) ;
    	$template_content = stripslashes( $template_content ) ;
    	$params = array(
    		'goods' => $goods	
    	) ;
    	$this -> parseWidgeTemplate($template_content, $params) ;
    }
    
	function parseWidgeTemplate($template, $params=array()) {
