...
class indexControl extends SystemControl{
    ...
    public function modify_emailOp(){
        ...
        if (chksubmit()){
            ...
            $update = array();
            $update['admin_email'] = trim($_POST['admin_email']);
            $update['admin_id'] = $admininfo['admin_id'];
            $result = $admin_model->updateAdmin($update);
            if ($result){
                showDialog('邮箱修改成功', '', 'succ');
            }else{
                showMessage('邮箱修改失败');
                showDialog('邮箱修改失败', '', '', 'CUR_DIALOG.click();');
            }
        }else{
            Tpl::output('admin',$admininfo);
            Language::read('admin');
            Tpl::showpage('admin.modify_email', 'null_layout');
        }
    }
