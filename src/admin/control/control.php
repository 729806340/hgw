...
class SystemControl{
    ...
    protected final function checkPermission($link_nav = null) {
        ...
        $act = $act == '' ? 'index' : $act;
        $op = $_REQUEST['op'] == '' ? 'index' : $_REQUEST['op'] ;
        $op = substr($op,-2) == 'Op' ? substr($op,0,-2) : $op ;
        ...
        if (is_array($permission[MODULE_NAME]) && in_array($act, $permission[MODULE_NAME])){
        	...
        	$validOpArr = isset( $permission['action'][MODULE_NAME][$act] ) ? $permission['action'][MODULE_NAME][$act] : array() ;
        	if( empty( $validOpArr ) ) {
            	return true;
        	} else if( in_array($op, $validOpArr)  ) {
        		return true;
        	} else {
    ...
    private final function getPermission() {
        if (empty($this->permission)) {
            ...
            $this->permission = $this -> _getActPermission( unserialize($permission) ) ;
        }
        return $this->permission;
    }
    private function _getActPermission( $permission )
    {
    	$actions = array() ;
    	foreach ( (array)$permission as $module => $controllers )
    	{
    		if( empty( $controllers ) ) continue ;
    
    		foreach ( $controllers as $ctl )
    		{
    			if( isset($permission['action'][$module][$ctl]) && !empty($permission['action'][$module][$ctl]) )
    			{
    				$ctls = $permission['action'][$module][$ctl] ;
    				foreach ( $ctls as $ct ) {
    					if( strpos($ct, "|") !== false ) {
    						$tmp = explode("|", $ct) ;
    						foreach ($tmp as $t) {
    							$actions[$module][$ctl][] = substr($t, -2) == "Op" ? substr($t, 0, -2) : $t ;
    						}
    					} else {
    						$actions[$module][$ctl][] = substr($ct, -2) == "Op" ? substr($ct, 0, -2) : $ct ;
    					}
    				}
    			}
    		}
