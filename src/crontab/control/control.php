...
class BaseCronControl {
    ...
    protected function _order_commis_rate_update() {
        ...
        for($i = 0; $i < 5000; $i++) {
            ...
            $goods_list = $model_order->getOrderGoodsList(array('commis_rate' => 200), 'rec_id,goods_id,store_id,gc_id,goods_pay_price', 100, null, '');
            $storeIds = array_column($goods_list,'store_id');
            if (!empty($goods_list)) {
                //$commis_rate_list : store_id => array(gc_id => commis_rate)
                $commis_rate_list = $store_bind_class->getStoreGcidCommisRateList($goods_list);
                $storeList = $storeModel->getStoreList(array('store_id'=>array('in',array_unique($storeIds))));
                ...
                foreach ($goods_list as $v) {
                    ...
                    // 更新平台商家成本值
                    $storeInfo = $storeList[$v['store_id']];
                    $upArray = array('commis_rate' => $commis_rate);
                    if($storeInfo['manage_type']=='platform'){
                        $upArray['goods_cost'] = $v['goods_pay_price']*(100-$commis_rate)/100;
                    }
                    $update = $model_order->editOrderGoods($upArray,array('rec_id' => $v['rec_id']));
                    v($model_order->getLastSql(),0);
                    if (!$update) {
