...
class dateControl extends BaseCronControl {
    ...
    private $_member_fenxiao;
    private $_channel_bill;
    ...
    public function indexOp() {
        //更新店铺销量、评分
        $this->_update_store_sales_and_evaluate() ;
        ...
        //更新退款申请超时处理
        Model('trade')->editRefundConfirm(0,0,0);
        ...
        //生成渠道的结算(月结)
        $this->_channel_real_order();

        //分销提现
        $this->_del_pyramid();
    }

    public function updateOrderGoodsCacheOp(){
        $this->_order_goods_cache();
    }
     ...
     * 统计每天支付渠道应收信息
     */
    public function paymentReceivableOp()
    {
        $this->_create_receivable();
    }
    public function pushPaymentOp()
    {
        $this->_push_receivable();
    }

    // 统计每天线下退款数据
    public function parseOfflineRefundOp(){
        // 统计线下退款
        $this->_parseOfflineRefund();

    }
    // 推送线下退款信息
    public function pushOfflineRefundOp(){
    ...
    private function _order_auto_complete() {
        ...
        //分销订单不自动收货
        $condition['order_from'] = array('neq', 3);
        ...
        for ($stime = $start_time; $stime < time(); $stime = $stime+86400){
            ...
            for ($i=0; $i<$countnum; $i+=100){//每次查询100条
                ...
                if ($orderlog_list){
                    ...
                    //查询订单数据
                    $field = 'order_id,order_sn,store_id,buyer_id,buyer_name,buyer_phone,add_time,payment_code,order_amount,shipping_fee,evaluation_state,order_state,refund_state,refund_amount,order_from';
    ...
    private function _member_stat(){
        ...
        //分销会员ID
        $fService = Service('Fenxiao') ;
        $fxMembers = $fService -> getFenxiaoMembers () ;
        $fxMemIds = array_keys ( $fxMembers ) ;
        ...
        for ($stime = $start_time; $stime < time(); $stime = $stime+86400){
            ...
            //过滤分销用户订单
            $where['orders.buyer_id'] = array('exp', "(orders.buyer_id NOT IN (". implode(",", $fxMemIds) .") )") ;
            $orderlist_tmp = array();
            ...
            //过滤分销用户订单
            $where['lg_member_id'] = array('exp', "(lg_member_id NOT IN (". implode(",", $fxMemIds) .") )") ;
            ...
            //过滤分销用户订单
            $where['pl_memberid'] = array('exp', "(pl_memberid NOT IN (". implode(",", $fxMemIds) .") )") ;
            ...
            foreach ((array)$update_arr as $k=>$v){
                foreach ($v as $m_k=>$m_v){
                    ...
                    //查询时间时间段内的下单商品件数
                    $where = array();
                    $where['orders.buyer_id'] = $m_k ; //add zxj
                    $where['orders.order_state'] = array(array('neq',ORDER_STATE_NEW), array('neq',ORDER_STATE_CANCEL),'and');//去除未支付和已取消订单
            ...
            if (is_array($store_list) && $store_list) {
                foreach ($store_list as $kk => $store_info) {
                    ...
                    if ($start_time !== 0) {
                        ...
                        //临时代码：取出共建商家，暂时不做结算处理
                        // $_store_info = $this->_model_store->getStoreInfoByID($store_info['store_id']);
                        // if ('platform' != $_store_info['manage_type'] ) {
                        // 	continue;
                        // }
    ...
    private function _create_bill_cycle_by_month($start_unixtime,$store_info,$flag_static) {
        ...
        while (($time = strtotime('-'.$i.' month',$current_time)) >= $start_unixtime) {
            if (date('Ym',$start_unixtime) == date('Ym',$time)) {
                //如果两个月份相等检查库是里否存在
                $order_statis = $this->_model_bill->getOrderStatisInfo(array('os_month'=>date('Ym',$start_unixtime),'ob_store_id'=>$store_info['store_id']));
                if ($order_statis) {
                    break;
                }
            }
    ...
    private function _create_real_order_bill($data){
        echo "创建账单:店铺ID：{$data['ob_store_id']}，时间：{$data['ob_start_date']}-{$data['ob_end_date']}<br />\n";
    ...
    private function _calc_real_order_bill($data_bill){
        ...
        $order = $this->_model_order->getOrderInfo($order_condition);
        $manageType = isset($order['manage_type'])?$order['manage_type']:'platform';
        if(method_exists($this,'_calc_real_order_bill_'.$manageType)){
           return call_user_func(array($this,'_calc_real_order_bill_'.$manageType),$data_bill);
        };
    }

    //刷新账单结算数据
    public function refreshOrderBillOp(){
        ...
        if($bill_info['ob_store_manage_type'] == 'co_construct'){
            $this->_calc_real_order_bill_co_construct($bill_info);
        } else if($bill_info['ob_store_manage_type'] == 'platform'){
            $this->_calc_real_order_bill_platform($bill_info);
        }
    }
    /**
     * 共建商家结算
     ...
     * @return mixed
     */
    private function _calc_real_order_bill_co_construct($data_bill){
        return Service("Bill") -> _calcRealBillCo_construct ( $data_bill ) ;
    }
     ...
     * @return mixed
     */
    private function _calc_real_order_bill_platform($data_bill){
        return Service("Bill") -> _calcRealBillPlatform ( $data_bill ) ;
    }
     ...
     * 更新店铺销售量和评分
     */
    private function _update_store_sales_and_evaluate()
    {
        /** @var storeModel $model_store */
        $model_store = Model('store');
        $condition = array();
        $stores = $model_store->getStoreList($condition, 1000, '', 'store_id');
        $store_ids = array_column($stores, 'store_id');

        //获取销量
        /** @var goodsModel $model_goods $condition */
        $model_goods = Model('goods');
        $condition = array();
        $condition['store_id'] = array('in', $store_ids) ;
        $fields = "store_id,SUM(goods_salenum) AS goods_salenum";
        $sales_list = $model_goods->field($fields)->where($condition)->group('store_id')->select();

        //更新
        /** @var evaluate_storeModel $model_evaluate_store */
        $model_evaluate_store = Model('evaluate_store');
        foreach ($sales_list as $sales) {
            $evaluate = $model_evaluate_store->getEvaluateStoreInfoByStoreID($sales['store_id']);

            $condition = array();
            $condition['store_id'] = $sales['store_id'] ;
            $update = array();
            $update['store_sales'] = $sales['goods_salenum'] ;
            $update['store_credit'] = $evaluate['store_credit_percent'];
            $update['store_desccredit'] = $evaluate['store_credit']['store_desccredit']['credit'];
            $update['store_servicecredit'] = $evaluate['store_credit']['store_servicecredit']['credit'];
            $update['store_deliverycredit'] = $evaluate['store_credit']['store_deliverycredit']['credit'];
            $model_store -> editStore($update, $condition) ;
        }
    }


    public function channel_billOp() {
        $this->_channel_real_order();
    }
    /*生成渠道订单*/
    private  function _channel_real_order(){
        $this->_member_fenxiao=Model('member_fenxiao');
        $this->_channel_bill=Model('channel_bill');
        $this->_model_order = Model('order');
        $con['filter_store_id']=array('gt','0');
        $count=$this->_member_fenxiao->getMemberFenxiaoCount($con);
        $step_num=10;
        for($i=0;$i<$count;$i=$i+$step_num){
            $channel_list=$this->_member_fenxiao->getMembeFenxiaoList($con,"{$i},{$step_num}");
            if(is_array($channel_list) && $channel_list){
                foreach($channel_list as $kk=>$channel_info) {
                    $start_time=$this->get_channel_start_data($channel_info['member_id']);
                    if($start_time<strtotime("2017-09-01")){
                        $start_time=strtotime("2017-09-01");
                     }
                    if($start_time!=0){
                        $this->_create_channel_bill_cycle_by_month($start_time,$channel_info);
                    }
                }
            }
        }
    ...
    private function get_channel_start_data($channel_id){
        $channel_bill_info=$this->_channel_bill->getOrderBillInfo(array('ob_channel_id'=>$channel_id),'max(ob_end_date) as stime');
        $start_unixtime=0;
        if($channel_bill_info['stime']){
            $start_unixtime=$channel_bill_info['stime']+1;
        }else{
            $condition=array();
            $condition['order_state']=ORDER_STATE_SUCCESS;
            $condition['buyer_id']=$channel_id;
            $condition['finnshed_time']=array('gt',0);
    ...
    private  function _create_channel_bill_cycle_by_month($start_unixtime,$channel_info,$flag_static){
        $i = 1;
        $start_unixtime = strtotime(date('Y-m-01 00:00:00', $start_unixtime));
        ...
        while (($time = strtotime('-'.$i.' month',$current_time)) >= $start_unixtime) {
            if (date('Ym',$start_unixtime) == date('Ym',$time)) {
                //如果两个月份相等检查库是里否存在
                $order_statis = $this->_channel_bill->getOrderStatisInfo(array('os_month'=>date('Ym',$start_unixtime)));
                if ($order_statis) {
                    break;
                }
            }
            ...
            try {
                ...
                //生成单个店铺月订单出账单
                $data = array();
                $data['ob_channel_id'] = $channel_info['member_id'];
                $data['ob_start_date'] = $first_day_unixtime;
                $data['ob_end_date'] = $last_day_unixtime;
                $this->_create_real_channel_bill($data);
                if ($flag_static === 0) {
                    $data = array();
                    $data['os_month'] = $os_month;
                    $data['os_year'] = date('Y',$first_day_unixtime);
                    $data['os_start_date'] = $first_day_unixtime;
                    $data['os_end_date'] = $last_day_unixtime;
                    $this->_channel_bill->addOrderStatis($data);
                }
                $this->_model_order->commit();
            } catch (Exception $e) {
                $this->log('实物账单:'.$e->getMessage());
     ...
     * @throws Exception
     */
    private function _create_real_channel_bill($data){
        //echo "创建账单:渠道ID：{$data['ob_channel_id']}，时间：{$data['ob_start_date']}-{$data['ob_end_date']}<br />\n";
        echo "创建账单:渠道ID：{$data['ob_channel_id']}，时间：".date('Y-m-d H:i:s',$data['ob_start_date'])."-".date('Y-m-d H:i:s',$data['ob_end_date'])."<br />\n";
        $data_bill['ob_start_date'] = $data['ob_start_date'];
        $data_bill['ob_end_date'] = $data['ob_end_date'];
        $data_bill['ob_state'] = 0;
        $data_bill['ob_channel_id'] = $data['ob_channel_id'];
        if (!$this->_channel_bill->getOrderBillInfo(array('ob_channel_id'=>$data['ob_channel_id'],'ob_start_date'=>$data['ob_start_date']))) {
            $insert = $this->_channel_bill->addOrderBill($data_bill);
            if (!$insert) {
                throw new Exception('生成账单失败');
            }
            //对已生成空账单进行销量、退单、佣金统计
            $data_bill['ob_id'] = $insert;
            $update = $this->_calc_real_channel_bill($data_bill);
            if (!$update){
                throw new Exception('更新账单失败');
            }
        }
    ...
    private  function  _calc_real_channel_bill($data_bill){
        return  Service("Bill")->_calChannelBill($data_bill);
    }

    public function  change_stat_orderOp(){
        $model_stat=Model('stat');
        $where1['buyer_phone']=array('exp','buyer_phone IS NULL');
        $count=$model_stat->getStatOrderCount($where1);
        $step_num=300;
        $arr=array();
        $num=0;
        for ($i = 0; $i <= $count; $i = $i + $step_num){
            //每次取出300条信息
            $order_list=Model('stat_order')->where($where1)->field('order_id')->limit("{$i},{$step_num}")->select();
            $order_ids=array_column($order_list,'order_id');
            $where['order_id']=array("in",implode(',',$order_ids));
            $orders=Model("orders")->where($where)->field('order_id,buyer_phone')->select();
            $num=$num+count($orders);
            if(count($orders)>0) {
                foreach ($orders as $item) {
                    $condition['order_id'] = $item['order_id'];
                    $data['buyer_phone'] = $item['buyer_phone'];
                    $res = Model('stat_order')->where($condition)->update($data);
                    if (!$res) {
                        $arr[] = $item['order_id'];
                    }
                }
        ...
        echo "stat_order总数量:".$count;
        echo "<br/>";
        echo "orders查询数量:".$num;
        echo "<br/>";
        echo "出错的order_ids".implode(',',$arr);
    }


    public function _create_receivable(){
        // 获取支付渠道
        // 循环处理支付渠道信息
        // 推送数据到供应链
        import("Curl");
        $curl = new \Curl();
        $url = "https://apisupplier.hangomart.com/receivable/create";
        $payments = array(
            'wxpay' => array('wx_jsapi','wx_saoma','微信支付'),
            'alipay' => array('alipay'),
            'bestpay' => array('bestpay'),
            'yeepay' => array('yeepay'),
        );
        ...
        /** @var payment_billModel $payment_billModel */
        $payment_billModel = model('payment_bill');
        $today = strtotime(date('Y-m-d'));
        if ($_GET['date']){
            $today = strtotime($_GET['date'])+86400;
        }
        if (C('ON_DEV')) {
            $today = strtotime(date('Y-m-d',1556006118));
            $today = $today+86400; // 测试环境指定的数据
            $url = "http://api.supplier.supplier.com/receivable/create";
        }
        $yesterday = $today - 86400;
        $today = $today-1;
        foreach ($payments as $payment => $paymentCodes){
            // 统计订单信息
            $item = $payment_billModel->getItemInfo(array(
                'date' => $yesterday,
                'payment' => $payment,
            ));
            if ($item) continue;
            $condition = array();
            $condition['payment_time'] = array('between',array($yesterday,$today));
            $condition['payment_code'] = array('in',$paymentCodes);
            $fields = " SUM(order_amount) as order_amount, SUM(rcb_amount) as rcb_amount, SUM(pd_amount) as pd_amount";
            $info = $orderModel->getOrderInfo($condition,array(),$fields);
            $orderAmount = $info["order_amount"]?$info["order_amount"]:0;
            $rcbAmount = $info["rcb_amount"]?$info["rcb_amount"]:0;
            $pdAmount = $info["pd_amount"]?$info["pd_amount"]:0;
            $order = $orderAmount-$rcbAmount-$pdAmount;
            // 统计退款信息
            $condition = array();
            $condition['pay_time'] = array('between',array($yesterday,$today));
            $condition['refund_code'] = array('in',$paymentCodes);
            $fields = " SUM(pay_amount) as pay_amount";
            $info = $refundModel->getDetailInfo($condition,$fields);
            $refund = $info['pay_amount']?$info['pay_amount']:0;
            // 统计充值信息
            $condition = array();
            $condition['pdr_payment_time'] = array('between',array($yesterday,$today));
            $condition['pdr_payment_code'] = array('in',$paymentCodes);
            $condition['pdr_payment_state'] = 1;
            $fields = " SUM(pdr_amount) as pdr_amount";
            $info = $predepositModel->getPdRechargeInfo($condition,$fields);
            $predeposit = $info['pdr_amount']?$info['pdr_amount']:0;
            $amount = $order-$refund+$predeposit;

            $payment_billModel->addItem(array(
                'date'=>$yesterday,
                'payment'=>$payment,
                'amount'=>$amount,
            ...
            continue;
            $memo = "订单在线支付收入：{$order}，退款：{$refund}，预存款充值：{$predeposit}";
            // 推送到供应链
            $param = array(
                'api_key'=>'c1dca569396ba260fe6a7d552b6b7d75',
                'finance_time'=>$yesterday,
                'type'=>30,
                'finance_type'=>$paymentTypeCode[$payment],
                'platform_id'=>1,
                'entity'=>$paymentName[$payment],
                'amount'=>$amount,
                'memo'=>$memo
            );
            $curl->post($url,$param);
            if ($curl->error) {
                Log::record('curl错误' . $url. ';Error:' . $curl->errorCode . ': ' . $curl->errorMessage);
                Log::record(json_encode($param));
                throw new Exception('curl错误' . $url . ';Error:' . $curl->errorCode . ': ' . $curl->errorMessage);
            }
    ...
    public function _push_receivable(){
        // 获取支付渠道
        // 循环处理支付渠道信息
        // 推送数据到供应链
        import("Curl");
        ...
        $items = $payment_billModel->getList(array(
            'push_state'=>array('in',array(payment_billModel::STATE_CREATED,payment_billModel::STATE_PUSHING)),
            'pushed_at'=>array('lt',time()-30*60),
        ));
        if (!$items) return;
        $payment_billModel->editItem(
            array('push_state'=>payment_billModel::STATE_PUSHING,'pushed_at'=>time()),
            array('id'=>array('in',array_column($items,'id'))));
        foreach ($items as $item){
            $order = $item['order_amount'];
            $refund = $item['order_amount'];
            $predeposit = $item['order_amount'];
            $amount = $item['amount'];
            $yesterday = $item['date'];
            ...
            if ($result->code==200){
                $payment_billModel->editItem(
                    array('push_state'=>payment_billModel::STATE_PUSHED),
                    array('id'=>$item['id'])
                );
                ...
                Log::record(json_encode($result));
                $payment_billModel->editItem(
                    array('push_state'=>payment_billModel::STATE_ERROR,'msg'=>$result['error']),
                    array('id'=>$item['id']));
            }
        }
    }
     ...
     * 获取支付渠道，按每日处理、把明细传过去
     */
    public function getPaymentChannelsOp(){
        $refundModel = model('refund_return');
        ...
        $today = strtotime(date('Y-m-d'));
        if (C('ON_DEV')) {//开发测试数据
            $today = strtotime('2019-05-24');
        }
        //指定日期-根据指定日期导出明细
        if ($_GET['date']){
            $today = strtotime($_GET['date'])+86400;
        }
        $yesterday_start = $today-86400;//头一天0点
        $yesterday_end = $today-1;//头一天23:59:59
        $condition = [
            'refund_way'=>['in',['offline','fenxiao']],//渠道为分销、线下两种
            'seller_state'=>2,//商家已同意
            'refund_state'=>3,//退款已完成
            'admin_time'=>['between',[$yesterday_start,$yesterday_end]]
        ];
        $payment_list = $refundModel->getRefundGroupBusiness($condition,'*',$group='',$order = 'refund_id desc');
        ...
        foreach ($payment_list as $k=>$v){
            $payment_list[$k]['price'] = $v['goods_num'] == 0?ncPriceFormat($v['refund_amount']):ncPriceFormat($v['refund_amount']/$v['goods_num']);
            $payment_list[$k]['signatory'] = '';
        }

        //要发送的全部字段列表
        $fields = ['order_id','member_cn_code','refund_amount','refund_way','payment_method','refund_account','refund_name','reason_info','store_name','goods_name','goods_num','price','seller_message','fx_order_id','admin_name','signatory'];

        //===关联表的信息整理
        //member_cn_code
        $buyer_id_list = array_column($payment_list,'buyer_id');
        $code_list = $member_fenxiaoModel->getMembeFenxiaoList(['member_id'=>['in',$buyer_id_list]]);
        $member_cn_code_mapping = array_column($code_list,'member_cn_code','member_id');//member_id=>member_cn_code
        //fx_order_id
        $order_id_list = array_column($payment_list,'order_id');
        $fx_order_list = $orderModel->getOrderList(['order_id'=>['in',$order_id_list]],'','order_id,fx_order_id,payment_code');
        $fx_order_id_mapping = array_column($fx_order_list,'fx_order_id','order_id');//order_id=>fx_order_id
        $payment_method_mapping = array_column($fx_order_list,'payment_code','order_id');//order_id=>payment_code

        //===model属性关联整理
        $refund_way = $member_fenxiaoModel::$refund_way;

        $export_data = [];
        foreach ($payment_list as $k=>$v){
            foreach ($fields as $field){
                //'member_cn_code'=>'buyer_id',//根据buyer_id从member_fenxiao表找
                //'fx_order_id'=>'order_id',//orders表fx_order_id
                //'payment_method'=>'order_id',//orders表fx_order_id
                if($field == 'member_cn_code'){
                    $export_data[$k][$field] = isset($member_cn_code_mapping[$v['buyer_id']])?$member_cn_code_mapping[$v['buyer_id']]:'';
                }elseif($field == 'fx_order_id'){
                    $export_data[$k][$field] = $fx_order_id_mapping[$v['order_id']];
                }elseif($field == 'payment_method'){//model属性关联
                    $export_data[$k][$field] = $refund_way[$payment_method_mapping[$v['order_id']]];
                }elseif($field == 'refund_way'){//model属性关联
                    $export_data[$k][$field] = isset($payment_method_mapping[$v['order_id']])?$refund_way[$payment_method_mapping[$v['order_id']]]:'';
                    $export_data[$k][$field] = $export_data[$k][$field]?$export_data[$k][$field]:'';//model中未定义完全的会显示NULL
                }else{
                    $export_data[$k][$field] = $v[$field];
                }
            }
        }

        //2.调供应链接口，发送前一天数据、及明细情况；
        import("Curl");
        $curl = new \Curl();
        $url = "https://apisupplier.hangomart.com/payable/savePaymentChannels?platform_id=1&date=".$_GET['date'];
        if (C('ON_DEV')) {
            $url = "http://api.hangoshop.com/payable/savePaymentChannels?platform_id=1&date=".$_GET['date'];
        }
        ...
        if ($result->code==200){
            ...
            Log::record('推送错误');
            Log::record(json_encode($result));
            echo $result->datas->error;
        }
    }

    public function _del_pyramid() {

        /** @var PyramidService $PyramidService */
        $PyramidService = Service("Pyramid");
        //查询完成时间复合的订单
        $condition_str = "orders.lock_state = 0";//订单未锁定
        $condition_str .= " AND orders.order_state = 40";//订单已完成
        $condition_str .= " AND pyramid_order_log.lg_status = 20";//

        $param = array() ;
        $param['table'] = 'pyramid_order_log,orders' ;
        $param['field'] = 'orders.*';
        $param['join_type'] = 'LEFT JOIN';
        $param['join_on'] = array('pyramid_order_log.order_id = orders.order_id');
        $param['where'] = $condition_str;
        $param['order'] = "orders.order_id desc";
        $order_list = DB::select($param);
        $result_list = array();
        foreach ($order_list as $order_data) {
            if (array_key_exists($order_data['order_id'], $result_list)) {
                continue;
            }
            $result_list[$order_data['order_id']] = $order_data;
        }
        foreach ($result_list as $order_info) {
            $PyramidService->getRealPyramidMoney($order_info);
        }
        return true;
    }
}