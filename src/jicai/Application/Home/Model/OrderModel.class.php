...
class OrderModel extends Model {
    ...
    public function getOrderList($pagesize = 6, $page = 1, $status, $id, $begintime = '', $endtime = '') {
        $conditions = array();
        if ($status != '') {
        ...
        if ($begintime && $endtime)
            $conditions['created_at'] = array(array('gt', strtotime($begintime)), array('lt', strtotime($endtime)));
        if (!$begintime && $endtime)
            $conditions['created_at'] = array(array('gt', 0), array('lt', strtotime($endtime)));
        if ($begintime && !$endtime)
            $conditions['created_at'] = array(array('gt', strtotime($begintime)), array('lt', time()));
        $offset = 0;
        if ($page > 1)
            $offset = ($page - 1) * $pagesize;
        $totalrows = $this->where($conditions)->count();
        $result = $this->order('id desc')->limit($offset, $pagesize)->where($conditions)->field('*,FROM_UNIXTIME(created_at,"%Y-%m-%d %H:%i:%S") as datetime ')->select();
